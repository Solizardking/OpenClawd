...
> **Last update:** 2026-04-30 — pAGENT family rollout 4 of 6 packages live · ClawdHub on Convex `third-bobcat-386` · SolanaOS → OpenClawd brand sweep complete · gateway HTTP server + `/src` bridge + `/console` route shipping.
...
- **SolanaOS → OpenClawd brand sweep** — 50+ files in `clawdhub/src/`, plus `convex/`, `server/`, `netlify/`, `live_chess-main/`, `e2e/`. Domain rewrites: `solanaos.net` → `openclawd.net`, `seeker.solanaos.net` → `seeker.openclawd.net`. Solana (the chain) preserved.
