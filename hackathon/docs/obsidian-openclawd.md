...
6. Export the active note to `.openclawd/membrain-ingest.jsonl` and `.openclawd/honcho-session.jsonl`.

## Why It Matters

OpenClawd already has a hosted notes app, Membrain typed memory, and Honcho-style persistent sessions. The Obsidian plugin makes that stack available in the place many operators already keep research: a local markdown vault.
