...
`openclawd.invoke --each`) without adding new OpenClawd code per workflow.
...
This extension depends on OpenClawd internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenClawd extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openclawd/extensions` as a standalone plugin directory.
