...
// NOTE: This extension is intended to be bundled with OpenClawd.
// When running from source (tests/dev), OpenClawd internals live under src/.
...
export function createLlmTaskTool(api: OpenClawdPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via openclawd.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawd-llm-task-"));
