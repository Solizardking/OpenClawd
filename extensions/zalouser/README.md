...
OpenClawd extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
openclawd channels login --channel zalouser
...
```bash
openclawd message send --channel zalouser --target <threadId> --message "Hello from OpenClawd!"
```
...
```bash
openclawd channels login --channel zalouser              # Login via QR
openclawd channels login --channel zalouser --account work
openclawd channels status --probe
openclawd channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
openclawd directory self --channel zalouser
openclawd directory peers list --channel zalouser --query "name"
openclawd directory groups list --channel zalouser --query "work"
openclawd directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
openclawd message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
openclawd message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `openclawd channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
openclawd channels login --channel zalouser --account work
openclawd message send --channel zalouser --account work --target <id> --message "Hello"
