import type { PluginRuntime } from "openclawd/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENCLAWD_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawd-nostr-"));
  process.env.OPENCLAWD_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OPENCLAWD_STATE_DIR?.trim() || env.OPENCLAWD_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".openclawd");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      delete process.env.OPENCLAWD_STATE_DIR;
    } else {
      process.env.OPENCLAWD_STATE_DIR = previous;
    }
