...
- Hub + dynamic endpoints: `https://solanaclawd.com`
  ...
  "agents": [
    {
      ...
      "a2a": "https://solanaclawd.com/api/agents/a2a",
      "source": "repo",
      "onchainAddress": null
    },
    {
...
```text
GET https://solanaclawd.com/api/agents/registration-template.json
GET https://solanaclawd.com/api/agents/nft-metadata.json?identifier=<agent-id>
```
...
// Send an A2A message
const res = await fetch("https://solanaclawd.com/api/agents/a2a", {
  method: "POST",
...
curl https://solanaclawd.com/api/agents/hosted | jq '.agents[].identifier'
```
...
- **Dynamic hub** (`solanaclawd.com`): 60 req/min per IP on public endpoints; higher for authenticated clients.
