...
export class OpenRouterService {
  ...
  private requireKey(): string {
    ...
    if (!key) {
      throw new Error(
        'OpenRouter key not set. Either export OPENROUTER_API_KEY or call openrouter.setUserKey(key).',
      )
    }
    return key
  }
  ...
  async generateText(prompt: string, opts: GenerateTextOptions = {}): Promise<string> {
    ...
    if (opts.instructions) messages.push({ role: 'system', content: opts.instructions })
  ...
  async listModels(opts: { modality?: 'text' | 'image'; query?: string } = {}): Promise<ModelInfo[]> {
    const key = this.requireKey()
...
function aspectRatioToSize(ratio: GenerateImageOptions['aspectRatio']): string {
  switch (ratio) {
    case '1:1':
      return '1024x1024'
    case '4:3':
