...
export class MemoryService {
    ...
    recall(opts = {}) {
        ...
        for (const entry of this.entries.values()) {
            if (tiers && !tiers.includes(entry.tier))
                continue;
            if (tags && !entry.tags.some((t) => tags.has(t)))
                continue;
            if (newerThan != null && entry.createdAt < newerThan)
                continue;
    ...
    prune(minConfidence = 0.1, now = Date.now()) {
        let dropped = 0;
    ...
    stats() {
        const byTier = { KNOWN: 0, LEARNED: 0, INFERRED: 0 };
