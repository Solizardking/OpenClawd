...
export class PageAgentCore extends EventTarget {
    ...
    constructor(config) {
        ...
        this.#llm.addEventListener('error', (e) => {
            ...
            if (error?.rawError?.name === 'AbortError')
                return;
    ...
    async execute(task) {
        if (this.disposed)
            throw new Error('PageAgent has been disposed. Create a new instance.');
        if (!task)
    ...
    #packMacroTool() {
        ...
        return {
            ...
            execute: async (input) => {
                ...
                if (input.memory)
                    reflectionLines.push(`💾: ${input.memory}`);
                if (input.next_goal)
                    reflectionLines.push(`🎯: ${input.next_goal}`);
    ...
    #getSystemPrompt() {
        ...
        const systemPrompt = SYSTEM_PROMPT.replace(/Default working language: \*\*.*?\*\*/, `Default working language: **${targetLanguage}**`);
        return systemPrompt;
    }
    /**
    ...
    async #getInstructions() {
        ...
        if (!systemInstructions && !pageInstructions && !llmsTxt)
            return '';
    ...
    async #handleObservations(step) {
        ...
        if (this.#states.totalWaitTime >= 3) {
            this.pushObservation(`You have waited ${this.#states.totalWaitTime} seconds accumulatively. ` +
        ...
        if (remaining === 5) {
            this.pushObservation(`⚠️ Only ${remaining} steps remaining. ` +
                `Consider wrapping up or calling done with partial results.`);
        }
        else if (remaining === 2) {
            this.pushObservation(`⚠️ Critical: Only ${remaining} steps left! You must finish the task or call done immediately.`);
        }
...
//# sourceMappingURL=PageAgentCore.js.map