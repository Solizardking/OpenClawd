 ...
 * - etc.
 */
export function normalizeResponse(response, tools) {
...
function validateAction(action, tools) {
    ...
    if (schema instanceof z.ZodObject && value !== null && typeof value !== 'object') {
        const requiredKey = Object.keys(schema.shape).find((k) => !schema.shape[k].safeParse(undefined).success);
        if (requiredKey) {
    ...
    if (!result.success) {
        throw new InvokeError(InvokeErrorType.INVALID_TOOL_ARGS, `Invalid input for action "${toolName}": ${z.prettifyError(result.error)}`);
    }
