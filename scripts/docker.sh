...
case "$1" in
    build)
        docker build --platform linux/amd64 -t openclawd .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=openclawd)" ]; then
            echo "Container 'openclawd' is already running. Stopping it first."
            docker stop openclawd
            docker rm openclawd
        fi
        ...
        CMD="$CMD --name openclawd openclawd"

        # Execute the command
        eval $CMD
        ;;
    start)
        docker start openclawd
        ;;
    bash)
        # Check if the container is running before executing bash
        if [ "$(docker ps -q -f name=openclawd)" ]; then
            docker exec -it openclawd bash
        else
            echo "Container 'openclawd' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
