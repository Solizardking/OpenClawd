---
summary: 'ClawdHub feature spec: import a skill from a public GitHub URL (auto-detect SKILL.md, selective file upload, provenance).'
read_when:
  - Adding GitHub import to ClawdHub (web + API)
  - Reviewing safety limits (SSRF/zip-bombs) on ClawdHub uploads
  - Implementing provenance + canonical-claim flows for ClawdHub skills
---

# ClawdHub: GitHub Import (public repos)

Goal: paste a GitHub URL → ClawdHub auto-detects skill → preview files → publish (selective) → persist provenance on the ClawdHub registry.

Non-goal (v1): private repos (no OAuth/PAT support).

Related ClawdHub docs:

- [`skill-format.md`](skill-format.md) — what counts as a ClawdHub skill; text-only limits.
- [`api.md`](api.md) / [`http-api.md`](http-api.md) — REST patterns + auth.

## UX

ClawdHub upload page: "Import from GitHub" mode.

Flow:

1. URL input.
2. Detect skill candidates (any folder containing `SKILL.md`).
...
ClawdHub downloads the archive:

- `https://github.com/<owner>/<repo>/archive/<ref>.zip`
- Follow redirects. The final redirect usually pins a commit via `codeload.github.com/.../zip/<sha-or-branch>`.

Unzip server-side (Node or ClawdHub Convex node action). Scan for skill candidates.
...
- Return candidate list to ClawdHub UI: `{ path, frontmatter.name, frontmatter.description }`.
- User chooses one.
...
- Prefer selecting only within the chosen skill folder; allow "include out-of-folder refs" if explicitly toggled.
...
ClawdHub publishes using its existing pipeline:

- Text-only enforced (see [`skill-format.md`](skill-format.md)).
...
- `slug`: sanitize folder basename; if collision on ClawdHub, suffix (`-2`, `-3`, …).
- `version`: if new ClawdHub skill → `0.1.0`; if updating own existing skill → bump patch.
- `tags`: default `latest`.

## Provenance (persist source)

Persist on each ClawdHub-published version (server-side injection; no mutation of imported files):
...
- It's already optional and stored with each ClawdHub version.
- No schema churn for v1.

Future: canonical-claim

- "claim canonical" can key off `{ kind:'github', repo, path }` on ClawdHub.
- Prefer commit-pinned provenance for auditability; allow ClawdHub UI to show "Imported from …".

## API sketch (internal ClawdHub actions)
...
- Validate `selectedPaths` is a subset of the fetched archive manifest.
...
- Huge repo → clean "too large" error from ClawdHub.
- Redirect pinning → ClawdHub import stores commit sha in provenance.
