---
summary: 'ClawdHub skill version diffing mode (Monaco-backed).'
read_when:
  - Implementing the ClawdHub skill diff UI
  - Adding version comparisons on hub.solanaclawd.com
---
...
- Compare any file between two ClawdHub-published versions.
...
- Public access — no ClawdHub login needed to view diffs.

## UX

- Diff card on the ClawdHub skill detail page.
...
- Use SemVer precedence to sort ClawdHub-stored versions.
- `previous` = immediate predecessor of `latest` by SemVer.
- If `latest` is missing or only one version exists:
  - Disable `previous` and show empty-state copy.

## Data sources

- Versions: ClawdHub `api.skills.listVersions` (all, not just latest 10).
- Tags: `skill.tags` map.
...
- `skills.getFileText({ versionId, path }) -> { text, size, sha256 }`
  ...
  - Enforce size <= 200KB (both in the action and the client).
  - Use `fetchText` from [`../convex/lib/skillPublish.ts`](../convex/lib/skillPublish.ts).

Optional helper action:

- `skills.getVersionFiles({ versionId }) -> files[]`
  - For lightweight fetch without the full version object.

## Client flow

1. Fetch versions + tags from ClawdHub.
...
4. For the selected file:
   - Fetch left/right text from ClawdHub (guard by size).
   - Feed into the Monaco diff editor.
...
- Apply theme on load + when the ClawdHub theme changes (`data-theme`).
...
- File removed/added: show empty buffer on the missing side + a label.
- Non-text file should not exist on ClawdHub (upload rejects), but still guard.
...
- Limit concurrent ClawdHub fetches to 2.
