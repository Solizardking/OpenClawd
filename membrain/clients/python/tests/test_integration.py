...
def daemon_env(daemon_binary: str):
    repo_root = Path(__file__).resolve().parents[3]
    test_auth_key = "membrane-python-integration-test-key"
    addr = f"127.0.0.1:{_free_port()}"

    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = os.path.join(tmpdir, "membrane.db")
        env = os.environ.copy()
        env["MEMBRANE_API_KEY"] = test_auth_key
        ...
        try:
            _wait_for_ready(proc, addr, test_auth_key)
            yield {"addr": addr, "api_key": test_auth_key}
        finally:
            _stop_process(proc)
