...
/**
 * OpenClawd CLI Delegation Debug Tool
 *
 * This script helps diagnose issues with OpenClawd CLI local delegation.
 * When you run `openclawd` commands, the CLI should automatically detect
 ...
 *
 * Common Issues:
 *   1. No local @openclawdsolana/cli installation
...
function displayHelp(): void {
  console.log(`${colors.cyan}OpenClawd CLI Delegation Debug Tool${colors.reset}`);
  ...
  console.log(`  This tool diagnoses why OpenClawd CLI local delegation might not be working.`);
...
function checkProblematicEnvVars(): string[] {
  const envVarsToCheck = [
    'NODE_ENV',
    'OPENCLAWD_TEST_MODE',
    'OPENCLAWD_CLI_TEST_MODE',
    'OPENCLAWD_SKIP_LOCAL_CLI_DELEGATION',
    'OPENCLAWD_DISABLE_LOCAL_CLI_DELEGATION',
    'BUN_TEST',
    'VITEST',
    'JEST_WORKER_ID',
    'npm_lifecycle_event',
    ...
    'APPVEYOR',
    'CODEBUILD_BUILD_ID',
    '_OPENCLAWD_CLI_DELEGATION_DEPTH',
  ];
  ...
  envVarsToCheck.forEach((envVar) => {
    ...
    if (value !== undefined) {
      ...
      if (
        ...
        (envVar === 'OPENCLAWD_TEST_MODE' && (value === 'true' || value === '1')) ||
        (envVar === 'OPENCLAWD_CLI_TEST_MODE' && value === 'true') ||
        (envVar === 'OPENCLAWD_SKIP_LOCAL_CLI_DELEGATION' && value === 'true') ||
        (envVar === 'OPENCLAWD_DISABLE_LOCAL_CLI_DELEGATION' && value === 'true') ||
        ...
        (envVar === '_OPENCLAWD_CLI_DELEGATION_DEPTH' && parseInt(value, 10) > 0)
      ) {
...
function analyzePackageJson(): void {
  ...
  if (existsSync(packageJsonPath)) {
    try {
      ...
      console.log(
        `   ${colors.bright}Has openclawd dependency:${colors.reset} ${packageJson.dependencies && packageJson.dependencies['@openclawdsolana/core'] ? `${colors.green}✅${colors.reset}` : `${colors.red}❌${colors.reset}`}`
      );
      console.log(
        `   ${colors.bright}Has openclawd dev dependency:${colors.reset} ${packageJson.devDependencies && packageJson.devDependencies['@openclawdsolana/core'] ? `${colors.green}✅${colors.reset}` : `${colors.red}❌${colors.reset}`}`
      );
    } catch (error) {
...
/**
 * Install @openclawdsolana/cli locally using bun
 */
async function installLocalCli(): Promise<void> {
  console.log(`${colors.yellow}→ Installing @openclawdsolana/cli locally...${colors.reset}`);

  try {
    const result: ExecResult = await bunExecInherit('bun', ['install', '@openclawdsolana/cli'], {
      cwd: process.cwd(),
    });

    if (result.success) {
      console.log(`${colors.green}✅ Successfully installed @openclawdsolana/cli${colors.reset}`);
    } else {
      throw new PackageInstallError(
        `Failed to install @openclawdsolana/cli: exit code ${result.exitCode}`,
        result.stderr
      );
    }
  } catch (error) {
    if (error instanceof PackageInstallError) {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.log(`${colors.red}❌ Failed to install @openclawdsolana/cli: ${errorMessage}${colors.reset}`);
    }
    throw error;
  }
}
...
function displayRecommendations(debugResult: DebugResult): void {
  ...
  if (!debugResult.hasLocalCli && !shouldFix) {
    console.log(
      `   ${colors.bright}•${colors.reset} Install @openclawdsolana/cli locally: ${colors.cyan}bun install @openclawdsolana/cli${colors.reset}`
    );
    console.log(
    ...
    console.log(
      `   ${colors.bright}•${colors.reset} Test with: ${colors.cyan}openclawd --help${colors.reset} (should show "Using local @openclawdsolana/cli installation")`
    );
...
function displayQuickTest(debugResult: DebugResult): void {
  if (
    ...
    console.log(`   Run: ${colors.cyan}openclawd --help${colors.reset}`);
    console.log(
      `   Expected: Should show "${colors.green}Using local @openclawdsolana/cli installation${colors.reset}" message`
    );
  }
}
