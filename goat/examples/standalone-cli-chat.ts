/**
 * OpenClawd Interactive Chat Interface
 *
 * An interactive command-line chat interface using OpenClawd agents.
 * Similar to AI SDK's streamText but using OpenClawd runtime and plugins.
 *
 * Usage:
 *   LOG_LEVEL=fatal OPENAI_API_KEY=your_key bun run standalone.ts
 */

// MUST be set before any imports to suppress OpenClawd logs
...
import sqlPlugin, { DatabaseMigrationService, createDatabaseAdapter } from '@openclawdsolana/plugin-sql';
...
class AgentInitializer {
  private static createCharacter(): Character {
    return {
      name: 'OpenClawd',
      username: 'openclawd',
      bio: 'A helpful AI assistant powered by OpenClawd.',
  ...
  static async initialize(): Promise<ChatSession> {
    ...
    try {
      task.start('Initializing OpenClawd...');
      ...
      task.stop('✅ OpenClawd initialized successfully');
...
class ChatInterface {
  ...
  private displayWelcome(): void {
    clack.intro('🤖 OpenClawd Interactive Chat');
    clack.note(
