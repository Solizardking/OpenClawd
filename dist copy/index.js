...
import { cloneAgent, cloneAll } from './agents/clone.js';
import { MonitorAgent } from './agents/monitor.js';
import { createRuntime, describeRuntime, getRuntime, } from './agents/runtime.js';
...
const KNOWN_AGENTS = ['trader', 'scanner', 'analyst', 'monitor'];
const args = process.argv.slice(2);
if (args[0] === 'agent') {
    const agentType = (args[1] || 'trader');
...
async function listExtensions() {
    ...
    for (const p of report.loaded) {
        console.log(`  ✓ ${p.id.padEnd(28)} tools=${p.tools.length} channels=${p.channels.length} providers=${p.providers.length}`);
    }
...
async function runAgent(type) {
    ...
    console.log(`🦞 Birthing ${type} clone — OpenRouter ${summary.hasOpenRouterKey ? 'ready' : 'no-key'}, ${enabled} skills injected.`);
    const agent = cloneAgent(type, { runtime });
    await agent.run();
}
