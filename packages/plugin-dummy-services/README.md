# @openclawdsolana/plugin-dummy-services

Mock/dummy service implementations for testing OpenClawd agents without external API dependencies.

## Overview

This plugin provides dummy implementations of common OpenClawd services, enabling developers to test agent functionality without requiring external API keys, network connections, or third-party services. All services return predictable mock data, making them ideal for unit tests, integration tests, and local development.

## Installation

```bash
bun add @openclawdsolana/plugin-dummy-services
```
...
```typescript
import { dummyServicesPlugin } from '@openclawdsolana/plugin-dummy-services';
...
import {
  ...
  DummyWalletService,
  DummyBrowserService,
} from '@openclawdsolana/plugin-dummy-services';
...
const suggestions = await searchService.autocomplete('openclawd');
...
```typescript
import { dummyServicesScenariosSuite } from '@openclawdsolana/plugin-dummy-services';
