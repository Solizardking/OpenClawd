...
This is an OpenClawd plugin built with the official plugin starter template.

## Getting Started

```bash
# Create a new plugin (automatically adds "plugin-" prefix)
openclawd create --type plugin solana
...
OpenClawd uses a dual testing approach that combines Bun's native test runner for component tests with a custom E2E test runner for integration testing within a live OpenClawd runtime.
...
#### 2. E2E Tests (OpenClawd Test Runner)

- **Purpose**: Test plugin behavior within a real OpenClawd runtime
- **Location**: `src/__tests__/e2e/*.ts`
- **Runner**: OpenClawd custom test runner
- **Command**: `openclawd test --type e2e`
- **Features**: Real runtime, real database, full integration

```typescript
// Example: src/__tests__/e2e/starter-plugin.ts
import { type TestSuite } from '@openclawdsolana/core';
...
bun test
# or
openclawd test --type component

# Run only E2E tests (slower, full integration)
openclawd test --type e2e
```
...
| --------------- | -------------------- | ----------------------- |
| **Runner**      | Bun test             | OpenClawd TestRunner      |
...
2. **GitHub Repository**
   ...
   - Add the 'openclawd-plugins' topic to the repository
...
- Submit your plugin to the OpenClawd registry for discoverability

### Continuous Development & Updates

**Important**: After your initial publish with `openclawd publish`, all future updates should be done using standard npm and git workflows, not the OpenClawd CLI.
...
1. **Make Changes**
   ...
   openclawd dev  # Test locally with hot-reload
   ```
...
2. **Test Your Changes**
   ...
   openclawd test component  # Component tests only
   openclawd test e2e       # E2E tests only
   ```
...
- **Automatic registry updates**: The OpenClawd registry automatically syncs with npm, so no manual registry updates needed
