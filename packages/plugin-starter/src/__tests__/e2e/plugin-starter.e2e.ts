...
/**
 * E2E (End-to-End) Test Suite for OpenClawd Plugins
 * ================================================
 *
 * This file contains end-to-end tests that run within a real OpenClawd runtime environment.
 ...
 * HOW E2E TESTS WORK:
 * -------------------
 * 1. Tests are executed by the OpenClawd test runner using `openclawd test e2e`
 ...
 *
 * For more details, see the OpenClawd documentation.
 */
  ...
  tests: [
     ...
     */
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (runtime.character.name !== 'OpenClawd') {
          throw new Error(
            `Expected character name to be "OpenClawd" but got "${runtime.character.name}"`
          );
        }
