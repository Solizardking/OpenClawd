...
This directory contains end-to-end tests for the OpenClawd plugin starter template.

## OpenClawd Testing Philosophy
...
E2E tests run in a real OpenClawd runtime environment, allowing you to test your plugin's behavior as it would work in production. Unlike component tests, E2E tests provide access to a fully initialized runtime with all services, actions, and providers available.
...
3. **Test Discovery**: The OpenClawd test runner automatically finds and executes tests from the plugin's `tests` array
...
- The test runner may look for tests on other plugins (e.g., @openclawdsolana/plugin-sql) instead of the current plugin
