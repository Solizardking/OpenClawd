...
// Custom command to set OPENCLAWD_CONFIG
Cypress.Commands.add('setOpenClawdConfig', (config) => {
  cy.window().then((win) => {
    (win as any).OPENCLAWD_CONFIG = config;
  });
});
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      setOpenClawdConfig(config: { agentId: string; apiBase?: string }): Chainable<Window>;
