...
The OpenClawd CLI provides a comprehensive set of commands to manage your OpenClawd projects and plugins, from local development to cloud deployment.
...
The OpenClawd CLI requires [Bun](https://bun.sh) as its package manager. If Bun is not installed when you run the CLI, it will attempt to automatically install it for you.
...
openclawd --no-auto-install create my-project

# Environment variable
OPENCLAWD_NO_AUTO_INSTALL=true openclawd create my-project
```

## Global Options

The following options are available for all OpenClawd CLI commands:
...
openclawd --no-auto-install --no-emoji create my-project
...
Below is a comprehensive reference for all OpenClawd CLI commands, including their options, arguments, and subcommands. For the most up-to-date usage, run `openclawd [command] --help`.

### `openclawd create [name]`
...
openclawd dev --character file1.json file2.json
...
openclawd dev --character assistant     # .json extension added automatically

# URLs are also supported
openclawd dev --character https://example.com/characters/assistant.json
```

### Environment Management

#### `openclawd env <subcommand>`
...
Clone OpenClawd monorepo from a specific branch (defaults to develop).

- **Options:**
  - `-b, --branch <branch>`: Branch to install (default: `develop`)
  - `-d, --dir <directory>`: Destination directory (default: `./openclawd`)
...
#### `openclawd plugins <subcommand>`

Manage an OpenClawd plugin.
...
#### `openclawd agent <subcommand>`

Manage OpenClawd agents.
...
- **Subcommands:**
  ...
  - `stop` (alias: `st`): Stop an agent
    - Options:
      - `-n, --name <name>`: Agent id, name, or index number from list
      - `--all`: Stop all running OpenClawd agents locally
      - `-r, --remote-url <url>`: URL of the remote agent runtime
      - `-p, --port <port>`: Port to listen on
...
The `openclawd publish` command is designed for **initial plugin publishing only**. After initial publishing, use standard npm and git workflows for updates:
...
The OpenClawd registry automatically syncs with npm updates.

### Agent/Server Management

#### `openclawd start`

Start the OpenClawd agent with configurable plugins and services.
...
openclawd start --character file1.json file2.json
...
openclawd start --character character1

# URLs are supported
openclawd start --character https://example.com/characters/assistant.json
```

If any character files fail to load, OpenClawd will:

- Log errors for the failed characters
- Continue starting with any successfully loaded characters
- Fall back to the default OpenClawd character if no characters loaded successfully

### Testing

#### `openclawd test`

Run tests for OpenClawd agent plugins and projects.
...
#### `openclawd tee phala <subcommand>`

Manage TEE deployments using the official [Phala Cloud CLI](https://docs.phala.com/phala-cloud/references/phala-cloud-cli/phala/docker). This integration provides seamless access to Phala's decentralized TEE cloud infrastructure directly through the OpenClawd CLI.
...
```bash
openclawd tee phala <command> [options]
```

##### Main Commands

- **`openclawd tee phala help`** - Display help for all commands
- **`openclawd tee phala join` (alias: `free`)** - Join Phala Cloud! Get an account and deploy a CVM for FREE
- **`openclawd tee phala demo`** - Launch demo applications on Phala Cloud (Jupyter Notebook, HTTPBin)

##### Authentication Commands (`openclawd tee phala auth`)

- **`openclawd tee phala auth login [api-key]`** - Set the API key for authentication
...
- **`openclawd tee phala auth logout`** - Remove the stored API key

- **`openclawd tee phala auth status`** - Check authentication status
  - Displays whether you're logged in and which account is active

##### Cloud Virtual Machine Management (`openclawd tee phala cvms`)

- **`openclawd tee phala cvms list` (alias: `ls`)** - List all CVMs

  - Options:
    - `-j, --json` - Output in JSON format

- **`openclawd tee phala cvms create`** - Create a new CVM
...
- **`openclawd tee phala cvms get [app-id]`** - Get details of a CVM

  - Options:
    - `-j, --json` - Output in JSON format

- **`openclawd tee phala cvms start [app-id]`** - Start a stopped CVM

  - Interactive selection if app-id not provided

- **`openclawd tee phala cvms stop [app-id]`** - Stop a running CVM

  - Interactive selection if app-id not provided

- **`openclawd tee phala cvms restart [app-id]`** - Restart a CVM

  - Interactive selection if app-id not provided

- **`openclawd tee phala cvms delete [app-id]`** - Delete a CVM

  - Options:
    - `-f, --force` - Skip confirmation prompt

- **`openclawd tee phala cvms upgrade [app-id]`** - Upgrade a CVM to a new version
...
- **`openclawd tee phala cvms resize [app-id]`** - Resize resources for a CVM
...
- **`openclawd tee phala cvms attestation [app-id]`** - Get attestation information for a CVM
  - Provides cryptographic proof that your application is running in a secure TEE
  - Interactive selection if app-id not provided

##### Docker Management (`openclawd tee phala docker`)

- **`openclawd tee phala docker login`** - Login to Docker Hub

  - Configure Docker Hub credentials for pushing images

- **`openclawd tee phala docker build`** - Build a Docker image

  - Options:
    - `--image <image>` - Docker image name
    - `--tag <tag>` - Tag for the Docker image

- **`openclawd tee phala docker push`** - Push a Docker image to Docker Hub

  - Options:
    - `--image <image>` - Docker image name
    - `--tag <tag>` - Tag to push

- **`openclawd tee phala docker generate`** - Generate a Docker Compose file
  - Options:
...
##### TEE Simulator (`openclawd tee phala simulator`)

- **`openclawd tee phala simulator start`** - Start the TEE simulator
...
- **`openclawd tee phala simulator stop`** - Stop the TEE simulator
...
2. **Authenticate**:
   ...
   ```bash
   openclawd tee phala auth login <your-api-key>
   openclawd tee phala auth status
   ```

3. **Deploy your first OpenClawd Agent**:

   ```bash
   # Create a TEE project starter template
   openclawd create --type tee tee-agent
   ...
   openclawd tee phala docker generate --template openclawd

   # Create and deploy a CVM
   openclawd tee phala cvms create --name openclawd -c <docker-compose file> -e <path to .env>

   # Check deployment status
   openclawd tee phala cvms list

   # Upgrade existing deployment
   openclawd tee phala cvms upgrade -c <docker-compose file> -e <path to .env (optional)>
   ```

4. **Verify TEE attestation**:

   ```bash
   openclawd tee phala cvms attestation <app-id>
   ```
...
- **Command Help**: `openclawd tee phala help` or `openclawd tee phala <command> --help`
...
Update OpenClawd CLI and project dependencies to the latest versions.
...
Deploy OpenClawd projects to AWS ECS (Elastic Container Service) with automatic multi-architecture support.
...
- **Options:**
  ...
  - `-k, --api-key <key>`: OpenClawd Cloud API key
  - `-u, --api-url <url>`: OpenClawd Cloud API URL
...
openclawd deploy --platform linux/arm64
...
To stop all running OpenClawd agents locally, use:

```bash
openclawd agent stop --all
```

This command uses `pkill` to terminate all OpenClawd processes. For stopping individual agents, see the [Agent Management](#openclawd-agent-subcommand) section.

## Development Guide

### Developing Agents

Agents are character definitions for OpenClawd bots.

1. **Create a new agent character**:

   ```bash
   openclawd create my-assistant --type agent
   ```
...
2. **Start an agent with the character**:
   ...
   ```bash
   openclawd agent start --path my-assistant.json
   ```

### Developing Plugins

Plugins extend the functionality of OpenClawd agents by providing additional capabilities or integrations.

1. **Create a new plugin**:

   ```bash
   openclawd create my-plugin --type plugin
...
3. **Implement your plugin**:
   ...
   ```typescript
   import { type Plugin, type IAgentRuntime } from '@openclawdsolana/core';
   import { z } from 'zod';
...
4. **Configure package.json**:
   ...
   ```json
   "agentConfig": {
     "pluginType": "openclawd:plugin:1.0.0",
     "pluginParameters": {
       "API_KEY": {
         "type": "string",
...
1. **Create a new project**:
   ...
   ```bash
   openclawd create my-agent-project
   cd my-agent-project
   ```
...
4. **Add plugins to your project**:
   ...
   ```bash
   openclawd plugins add @openclawdsolana/plugin-openai
   ```
...
7. **Test your project**:
   ...
   openclawd test --port 4000 --name specific-test
   ```
...
8. **Development workflow**:
   ...
   openclawd dev  # Development mode with hot-reload
...
For contributing to the OpenClawd CLI, please clone the monorepo using:
