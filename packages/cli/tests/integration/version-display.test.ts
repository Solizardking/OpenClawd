...
describe('CLI version display integration tests', () => {
  describe('version flag behavior', () => {
    ...
    it('should not show update notification when in monorepo context', async () => {
      ...
      expect(result.stdout).not.toContain('bun i -g @openclawdsolana/cli@latest');
      expect(result.code).toBe(0);
    });
  });
  ...
  describe('non-monorepo context behavior', () => {
    ...
    beforeEach(async () => {
      ...
      tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'openclawd-cli-test-'));
  ...
  describe('local dist behavior', () => {
    it.skip('should display "monorepo" when running from dist folder outside monorepo', async () => {
      ...
      const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'openclawd-cli-dist-test-'));
...
async function runCLI(
  ...
  const proc = Bun.spawn(['bun', cliPath, ...args], {
    ...
    env: {
      ...
      OPENCLAWD_NO_AUTO_INSTALL: 'true',
    },
    stdout: 'pipe',
    stderr: 'pipe',
  });
