...
@test "e2e: complete project creation and execution workflow" {
  # Create project
  run run_cli "dist" create my-openclawd-project --no-install
  assert_success
  assert_output --partial "Creating new OpenClawd project"
  
  cd my-openclawd-project
...
@test "e2e: plugin creation and testing workflow" {
  ...
  assert_file_contain "package.json" "@openclawdsolana/plugin-my-plugin"
