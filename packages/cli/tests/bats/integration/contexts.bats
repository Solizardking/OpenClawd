...
@test "context: CLI shows help from dist" {
  ...
  assert_cli_success
  assert_output --partial "Usage: openclawd"
...
@test "context: CLI commands work from monorepo root" {
  ...
  assert_cli_success
  assert_output --partial "Create a new OpenClawd project"
}

@test "context: Environment variables are respected" {
  export OPENCLAWD_NO_AUTO_INSTALL=true
  export NO_COLOR=1
  export OPENCLAWD_LOG_LEVEL=debug
...
@test "context: CLI works after npm link" {
  ...
  cd "$TEST_DIR"
  run npm link @openclawdsolana/cli
  assert_success
  
  run openclawd --version
  assert_cli_success
  assert_output --regexp "[0-9]+\.[0-9]+\.[0-9]+"
  
  # Cleanup
  npm unlink @openclawdsolana/cli
}
