...
describe('selection utilities', () => {
  describe('hasEmbeddingSupport', () => {
    it('should return true for models with embedding support', () => {
      expect(hasEmbeddingSupport('openclawdcloud')).toBe(true);
  ...
  describe('AI model selection flow', () => {
    it('should identify which models need separate embedding providers', () => {
      ...
      const modelsWithEmbeddings = ['openclawdcloud', 'local', 'openai', 'google', 'openrouter'];
  ...
  describe('OpenClawd Cloud model option', () => {
    it('should include openclawdcloud as the first model option', () => {
      const models = getAvailableAIModels();
      expect(models[0].value).toBe('openclawdcloud');
    });

    it('should have openclawdcloud marked as recommended', () => {
      const models = getAvailableAIModels();
      const openclawdcloud = models.find((m) => m.value === 'openclawdcloud');
      expect(openclawdcloud).toBeDefined();
      expect(openclawdcloud!.title).toContain('Recommended');
    });

    it('should have openclawdcloud with embedding support', () => {
      expect(hasEmbeddingSupport('openclawdcloud')).toBe(true);
    });
  });
});
