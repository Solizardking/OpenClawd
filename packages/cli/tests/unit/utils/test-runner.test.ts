...
import type { IAgentRuntime, Plugin, ProjectAgent, Character } from '@openclawdsolana/core';
...
describe('TestRunner Plugin Isolation', () => {
  ...
  const originalEnv = process.env.OPENCLAWD_TESTING_PLUGIN;
  ...
  afterEach(() => {
    ...
    if (originalEnv) {
      process.env.OPENCLAWD_TESTING_PLUGIN = originalEnv;
    } else {
      delete process.env.OPENCLAWD_TESTING_PLUGIN;
    }
  });
  ...
  describe('Plugin Identification', () => {
    it('should identify plugin under test when OPENCLAWD_TESTING_PLUGIN is set', () => {
      ...
      const projectAgent: ProjectAgent = {
        character: { name: 'OpenClawd', bio: 'Test bio' } as Character,
    ...
    it('should not identify as plugin test when OPENCLAWD_TESTING_PLUGIN is not set', () => {
