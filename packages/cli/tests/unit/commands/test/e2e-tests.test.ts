...
const mockServerStartAgents = mock().mockResolvedValue([
  {
    character: { name: 'OpenClawd' },
    plugins: [],
  },
]);

mock.module('@openclawdsolana/server', () => ({
...
mock.module('../../../../src/characters/openclawd', () => ({
...
describe('E2E Tests Plugin Isolation', () => {
  let originalOpenClawdTestingPlugin: string | undefined;
  let runE2eTests: any;
  let loadProject: any;
  ...
  beforeEach(async () => {
    ...
    // Save original OPENCLAWD_TESTING_PLUGIN value specifically
    originalOpenClawdTestingPlugin = process.env.OPENCLAWD_TESTING_PLUGIN;
  ...
  afterEach(() => {
    ...
    if (originalOpenClawdTestingPlugin !== undefined) {
      process.env.OPENCLAWD_TESTING_PLUGIN = originalOpenClawdTestingPlugin;
    } else {
      delete process.env.OPENCLAWD_TESTING_PLUGIN;
    }
  });
  ...
  describe('Plugin Test Environment Variable', () => {
    it('should set OPENCLAWD_TESTING_PLUGIN=true when testing a plugin', async () => {
      ...
      const originalDescriptor = Object.getOwnPropertyDescriptor(
        process.env,
        'OPENCLAWD_TESTING_PLUGIN'
      );

      // Simple spy on environment variable
      Object.defineProperty(process.env, 'OPENCLAWD_TESTING_PLUGIN', {
      ...
      try {
        ...
        if (originalDescriptor) {
          Object.defineProperty(process.env, 'OPENCLAWD_TESTING_PLUGIN', originalDescriptor);
        } else {
          delete process.env.OPENCLAWD_TESTING_PLUGIN;
        }
      }
    ...
    it('should not set OPENCLAWD_TESTING_PLUGIN when testing a project', async () => {
      ...
      expect(process.env.OPENCLAWD_TESTING_PLUGIN).toBeUndefined();
    ...
    it('should clean up OPENCLAWD_TESTING_PLUGIN after tests complete', async () => {
      // Setup mock to return a plugin using test fixtures
      loadProject.mockResolvedValue(TestFixtures.mockPluginProject);
      ...
      TestRunnerMock.mockImplementation(() => ({
        runTests: mock().mockImplementation(() => {
          ...
          envDuringTest = process.env.OPENCLAWD_TESTING_PLUGIN;
    ...
    it('should clean up OPENCLAWD_TESTING_PLUGIN even if tests fail', async () => {
  ...
  describe('Plugin vs Project Detection', () => {
    ...
    it('should correctly identify and test projects', async () => {
      ...
      const projectInfo = {
        ...TestFixtures.mockDirectoryInfo.project,
        openclawdPackageCount: 3, // Override for this test
      };
