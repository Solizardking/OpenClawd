...
describe('OpenClawd Dev Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  const spawnDevProcess = (
    ...
    const defaultEnv = {
      ...
      PGLITE_DATA_DIR: join(testTmpDir, 'openclawddb'),
      OPENCLAWD_TEST_MODE: 'true',
      // Ensure non-interactive mode to prevent hanging
      OPENCLAWD_NONINTERACTIVE: 'true',
      NODE_ENV: 'test',
    };
  ...
  beforeAll(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-dev-'));
      ...
      JSON.stringify(
        {
          ...
          dependencies: {
            ...
            '@openclawdsolana/server': '^1.0.0',
            '@openclawdsolana/plugin-sql': '^1.0.0',
  ...
  afterAll(async () => {
    ...
    if (testTmpDir && testTmpDir.includes('openclawd-test-dev-')) {
      try {
  ...
  const startDevAndWait = async (
    ...
    await mkdir(join(testTmpDir, 'openclawddb'), { recursive: true });

    const commandStr = `openclawd dev ${args}`;
    console.log(`[DEBUG] Running command: ${commandStr}`);

    try {
      const devProcess = spawnDevProcess(['openclawd', 'dev', ...args.split(' ')], {
        cwd: cwd || projectDir,
        env: {
  ...
  it('dev --help shows usage', () => {
    const result = bunExecSync(`openclawd dev --help`, { encoding: 'utf8' });
    expect(result).toContain('Usage: openclawd dev');
  ...
  it.skipIf(process.platform === 'win32' && process.env.CI === 'true')(
    ...
    async () => {
      ...
      const devProcess = spawnDevProcess(['openclawd', 'dev', '--port', testServerPort.toString()], {
        env: {
          LOG_LEVEL: 'info',
        },
      });
      ...
      if (output && output.length > 0) {
        expect(output).toMatch(
          /(OpenClawd project|project mode|Identified as|Starting|development|dev mode|project|error|info)/i
        );
      } else {
  ...
  it('dev command handles non-openclawd directory gracefully', async () => {
    // Create a non-OpenClawd project directory
    const nonOpenClawdDir = join(testTmpDir, 'non-openclawd');
    await mkdir(nonOpenClawdDir, { recursive: true });
    await writeFile(
      join(nonOpenClawdDir, 'package.json'),
      JSON.stringify({ name: 'not-openclawd', version: '1.0.0' })
    );

    console.log(`[DEBUG] Testing non-OpenClawd directory handling with port ${testServerPort}`);
    ...
    console.log(`[NON-OPENCLAWD DIR TEST] Final output: "${output}"`);

    // Verify process started and detected non-OpenClawd directory
    expect(devProcess.pid).toBeDefined();

    if (output && output.length > 0) {
      expect(output).toMatch(
        /(not.*recognized|standalone mode|not.*OpenClawd|non.*openclawd|external|independent|error|info|Starting)/i
      );
    } else {
      console.log('[NON-OPENCLAWD DIR TEST] No output but process started successfully');
    }
  ...
  it('dev command validates port parameter', () => {
    ...
    try {
      bunExecSync(`openclawd dev --port abc`, {
        encoding: 'utf8',
        stdio: 'pipe',
  ...
  it.skipIf(process.platform === 'win32' && process.env.CI === 'true')(
    ...
    async () => {
      ...
      // Ensure openclawddb directory exists
      await mkdir(join(testTmpDir, 'openclawddb'), { recursive: true });
      ...
      try {
        ...
        const devProcess = spawnDevProcess(['openclawd', 'dev'], {
          env: {
            FORCE_COLOR: '0',
            LOG_LEVEL: 'debug',
            OPENCLAWD_NONINTERACTIVE: 'true',
            // Don't unset OPENCLAWD_TEST_MODE as it's needed for proper test isolation
          },
        });
  ...
  it.skipIf(process.platform === 'win32' && process.env.CI === 'true')(
    ...
    async () => {
      ...
      const helpResult = bunExecSync(`openclawd dev --help`, { encoding: 'utf8' });
      ...
      const devProcess = spawnDevProcess(['openclawd', 'dev', '--port', specifiedPort.toString()], {
        env: {
          FORCE_COLOR: '0',
  ...
  it(
    ...
    async () => {
      ...
      const { pluginDir, cleanup } = await cloneAndSetupPlugin(
        'https://github.com/OpenClawd-plugins/plugin-openai.git',
        '1.x'
      );
      ...
      try {
        ...
        const devProcess = Bun.spawn(['openclawd', 'dev', '--port', testServerPort.toString()], {
          cwd: pluginDir,
          env: {
            ...process.env,
            LOG_LEVEL: 'info',
