...
describe('OpenClawd Start Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  let testTmpDir: string;
  let openclawdPath: string;
  let originalCwd: string;
  let testServerPort: number;
  let processManager: TestProcessManager;
  let originalOpenClawdTestMode: string | undefined;
  ...
  beforeEach(async () => {
    ...
    // Store original OPENCLAWD_TEST_MODE
    originalOpenClawdTestMode = process.env.OPENCLAWD_TEST_MODE;
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-start-'));

    // Setup CLI path
    const scriptDir = join(__dirname, '..');
    openclawdPath = join(scriptDir, '../dist/index.js');
    ...
    process.env.OPENCLAWD_CLI_TEST_MODE = 'true';
  });
  ...
  afterEach(async () => {
    ...
    delete process.env.OPENCLAWD_CLI_TEST_MODE;

    // Restore original OPENCLAWD_TEST_MODE
    if (originalOpenClawdTestMode !== undefined) {
      process.env.OPENCLAWD_TEST_MODE = originalOpenClawdTestMode;
    } else {
      delete process.env.OPENCLAWD_TEST_MODE;
    }
    ...
    if (testTmpDir && testTmpDir.includes('openclawd-test-start-')) {
      try {
  ...
  it('start command shows help', async () => {
    const { stdout: result } = await bunExecSimple('bun', [openclawdPath, 'start', '--help'], {
    ...
    expect(result).toContain('Usage: openclawd start');
        ...
        for (let i = 0; i < maxRetries; i++) {
          try {
            ...
            const { stdout } = await bunExecSimple(
              'bun',
              [openclawdPath, 'agent', 'list', '--remote-url', `http://localhost:${testServerPort}`],
              {
                timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
                env: {
                  ...process.env,
  ...
  it(
    ...
    async () => {
      ...
      await mkdir(join(testTmpDir, 'openclawddb2'), { recursive: true });
      ...
      const serverProcess = processManager.spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            LOG_LEVEL: 'debug',
            PGLITE_DATA_DIR: join(testTmpDir, 'openclawddb2'),
            NODE_ENV: 'test',
            OPENCLAWD_TEST_MODE: 'true',
            BUN_TEST: 'true',
            OPENCLAWD_CLI_TEST_MODE: 'true',
  ...
  it('multiple character formats parse', async () => {
    ...
    for (const fmt of formats) {
      const { stdout: result } = await bunExecSimple(
        'bun',
        [openclawdPath, 'start', '--character', `${adaPath}${fmt}${adaPath}`, '--help'],
        {
          timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
          env: {
            ...process.env,
  ...
  it('graceful acceptance of invalid character file list (dry)', async () => {
    ...
    const { stdout: result } = await bunExecSimple(
      'bun',
      [openclawdPath, 'start', '--character', `${adaPath},does-not-exist.json`, '--help'],
      {
        timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
        env: {
          ...process.env,
  ...
  it('build option flag accepted', async () => {
    const { stdout: result } = await bunExecSimple(
      'bun',
      [openclawdPath, 'start', '--build', '--help'],
      {
        timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
        env: {
          ...process.env,
  ...
  it(
    ...
    async () => {
      ...
      await mkdir(join(testTmpDir, 'openclawddb3'), { recursive: true });
      ...
      const serverProcess = processManager.spawn(
        ...
        [join(__dirname, '..', '../dist/index.js'), 'start', '--configure', '--character', adaPath],
        {
          env: {
            ...process.env,
            LOG_LEVEL: 'debug',
            PGLITE_DATA_DIR: join(testTmpDir, 'openclawddb3'),
            NODE_ENV: 'test',
            OPENCLAWD_TEST_MODE: 'true',
            BUN_TEST: 'true',
            OPENCLAWD_CLI_TEST_MODE: 'true',
