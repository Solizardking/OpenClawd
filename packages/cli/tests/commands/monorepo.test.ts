...
describe('OpenClawd Monorepo Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  it('monorepo --help shows usage', async () => {
    const result = bunExecSync('openclawd monorepo --help', { encoding: 'utf8' });
  ...
  it('monorepo fails when directory is not empty', async () => {
    ...
    try {
      ...
      bunExecSync('openclawd monorepo --dir not-empty-dir', { encoding: 'utf8' });
