...
describe('OpenClawd Create Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  beforeEach(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-'));
  ...
  it('create --help shows usage', () => {
    const result = bunExecSync(
      `openclawd create --help`,
      getPlatformOptions({ encoding: 'utf8' })
    ) as string;
    expect(result).toContain('Usage: openclawd create');
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync(
        'openclawd create my-default-app --yes',
        getPlatformOptions({
          encoding: 'utf8',
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd create my-plugin-app --yes --type plugin', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      }) as string;
  ...
  it('create agent succeeds', async () => {
    ...
    const result = bunExecSync('openclawd create my-test-agent --yes --type agent', {
      encoding: 'utf8',
    }) as string;
  ...
  it('rejects creating project in existing directory', async () => {
    ...
    try {
      const output = bunExecSync('openclawd create existing-app --yes', {
        encoding: 'utf8',
      }) as string;
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd create . --yes', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      }) as string;
  ...
  it('rejects invalid project name', () => {
    let result: { status: number; output: string };
    try {
      const output = bunExecSync('openclawd create Invalid-Name! --yes', {
        encoding: 'utf8',
      }) as string;
      throw new Error(`Command should have failed but succeeded with output: ${output}`);
    } catch (e: any) {
  ...
  it('rejects invalid project type', () => {
    let result: { status: number; output: string };
    try {
      const output = bunExecSync('openclawd create bad-type-proj --yes --type bad-type', {
        encoding: 'utf8',
      }) as string;
      throw new Error(`Command should have failed but succeeded with output: ${output}`);
    } catch (e: any) {
  ...
  it(
    ...
    async () => {
      ...
      bunExecSync('openclawd create long-flag-test --yes --type plugin', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      });
      ...
      bunExecSync('openclawd create short-flag-test -y -t plugin', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      });
  ...
  it(
    ...
    async () => {
      ...
      bunExecSync('openclawd create yes-long-flag-test --yes', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      });
      ...
      bunExecSync('openclawd create yes-short-flag-test -y', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      });
  ...
  // create-openclawd parity tests
  it('create-openclawd default project succeeds', async () => {
    // Use cross-platform directory removal
    await crossPlatform.removeDir('my-create-app');

    // Skip this test - create-openclawd command not available
    console.warn('Skipping create-openclawd test - command not available');
  }, 60000);

  it('create-openclawd plugin project succeeds', async () => {
    // Use cross-platform directory removal
    await crossPlatform.removeDir('plugin-my-create-plugin');

    // Skip this test - create-openclawd command not available
    console.warn('Skipping create-openclawd plugin test - command not available');
  }, 60000);

  it('create-openclawd agent succeeds', async () => {
    // Use cross-platform file removal
    await crossPlatform.removeFile('my-create-agent.json');

    // Skip this test - create-openclawd command not available
    console.warn('Skipping create-openclawd agent test - command not available');
  }, 60000);
  ...
  describe('AI Model Selection', () => {
    ...
    it('maintains core AI model options', () => {
      ...
      // Only test for essential/core providers (openclawdcloud is now the recommended default)
      const CORE_PROVIDERS = ['openclawdcloud', 'local', 'openai', 'claude', 'openrouter'];
      CORE_PROVIDERS.forEach((provider) => {
        expect(values).toContain(provider);
      });
    });

    it('has openclawdcloud as the first/default option', () => {
      const models = getAvailableAIModels();
      expect(models[0].value).toBe('openclawdcloud');
  ...
  describe('CLAUDE.md File Creation', () => {
    it(
      ...
      async () => {
        ...
        const result = bunExecSync(
          'openclawd create claude-md-test-project --yes',
          getPlatformOptions({
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          })
        ...
        expect(claudeMdContent).toContain('OpenClawd Agent Project Development Guide for Claude');
        expect(claudeMdContent).toContain(
          '| **Project Type**    | OpenClawd Agent Project         |'
        ...
        expect(claudeMdContent).toContain('openclawd dev');
        expect(claudeMdContent).toContain('openclawd start');
      },
      TEST_TIMEOUTS.INDIVIDUAL_TEST
    );
    ...
    it(
      ...
      async () => {
        ...
        const result = bunExecSync(
          'openclawd create claude-md-test --yes --type plugin',
          getPlatformOptions({
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          })
        ...
        expect(claudeMdContent).toContain('OpenClawd Plugin Development Guide for Claude');
        expect(claudeMdContent).toContain('| **Project Type**    | OpenClawd Plugin        |'); // Match actual template format
  ...
  describe('TEE Project Creation', () => {
    it(
      ...
      async () => {
        ...
        const result = bunExecSync(
          'openclawd create my-tee-project --yes --type tee',
          getPlatformOptions({
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          })
    ...
    it(
      ...
      async () => {
        ...
        const result = bunExecSync(
          'openclawd create tee-guide-test --yes --type tee',
          getPlatformOptions({
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          })
    ...
    it(
      ...
      async () => {
        ...
        const createProcess = Bun.spawn(
          ['openclawd', 'create', pluginName, '--type', 'plugin', '--yes'],
          {
            stdout: 'ignore',
            stderr: 'ignore',
            stdin: 'ignore',
  ...
  describe('--dir Flag Removal (Breaking Change)', () => {
    it('rejects --dir flag with helpful error message', () => {
      let result: { status: number; output: string };
      try {
        const output = bunExecSync('openclawd create my-project --dir /some/path', {
          encoding: 'utf8',
        }) as string;
        throw new Error(`Command should have failed but succeeded with output: ${output}`);
      } catch (e: unknown) {
    ...
    it('rejects -d shorthand flag', () => {
      let result: { status: number; output: string };
      try {
        const output = bunExecSync('openclawd create my-project -d /some/path', {
          encoding: 'utf8',
        }) as string;
        throw new Error(`Command should have failed but succeeded with output: ${output}`);
      } catch (e: unknown) {
    ...
    it(
      ...
      async () => {
        ...
        try {
          const result = bunExecSync('openclawd create my-current-dir-project --yes', {
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          }) as string;
    ...
    it(
      ...
      async () => {
        ...
        // Before: openclawd create my-project --dir /path/to/directory
        // After: cd /path/to/directory && openclawd create my-project
        ...
        try {
          ...
          const result = bunExecSync('openclawd create migrated-project --yes', {
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          }) as string;
  ...
  it(
    'does not hoist PGLITE database to parent .openclawd directory',
    async () => {
      // Create a parent directory that looks like an OpenClawd project
      const parentDir = await mkdtemp(join(tmpdir(), 'openclawd-parent-'));
      const parentOpenClawdDir = join(parentDir, '.openclawd');
      await mkdir(parentOpenClawdDir, { recursive: true });

      // Create a marker file to verify we're not using parent's .openclawd
      await writeFile(join(parentOpenClawdDir, 'parent-marker.txt'), 'parent');
      ...
      try {
        ...
        const result = bunExecSync('openclawd create test-no-hoist --yes', {
          encoding: 'utf8',
          timeout: TEST_TIMEOUTS.PROJECT_CREATION,
        }) as string;
        ...
        // Verify project has its own .openclawd directory
        const projectOpenClawdDir = join('test-no-hoist', '.openclawd');
        expect(existsSync(projectOpenClawdDir)).toBe(true);

        // Verify the project's .openclawd directory has its own database directory
        const projectDbDir = join(projectOpenClawdDir, '.openclawddb');
        expect(existsSync(projectDbDir)).toBe(true);

        // Verify the parent marker file is NOT in the project's .openclawd directory
        const projectMarkerPath = join(projectOpenClawdDir, 'parent-marker.txt');
        ...
        // Verify it points to the project's .openclawd/.openclawddb, not parent's
        expect(pgliteDataDir).toContain(join('test-no-hoist', '.openclawd', '.openclawddb'));
        // Ensure it's not pointing to parent's .openclawd directory (without the project name)
        const sep = path.sep.replace(/\\/g, '\\\\'); // Escape backslashes for regex
        expect(pgliteDataDir).not.toMatch(
          new RegExp(`openclawd-parent-[^${sep}]+${sep}\\.openclawd${sep}\\.openclawddb$`)
        );
      } finally {
        process.chdir(originalDir);
        // Cleanup
