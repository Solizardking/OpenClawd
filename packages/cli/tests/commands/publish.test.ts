...
describe('OpenClawd Publish Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  beforeEach(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-publish-'));
    ...
    // Mock OpenClawd data directory to avoid credential prompts
    const openclawdDataDir = join(testTmpDir, '.openclawd');
    process.env.OPENCLAWD_DATA_DIR = openclawdDataDir;
    await mkdir(openclawdDataDir, { recursive: true });

    // Create mock credentials file
    await writeFile(
      join(openclawdDataDir, 'credentials.json'),
      JSON.stringify({
        github: {
    ...
    await writeFile(
      join(openclawdDataDir, 'registry.json'),
      JSON.stringify({
        registryUrl: 'https://github.com/openclawd/registry',
...
case "$1" in
  ...
  *)
  ...
  afterEach(async () => {
    ...
    if (testTmpDir && testTmpDir.includes('openclawd-test-publish-')) {
      try {
  ...
  it('publish --help shows usage', () => {
    const result = bunExecSync(`openclawd publish --help`, { encoding: 'utf8' });
    expect(result).toContain('Usage: openclawd publish');
  ...
  it('publish command integrates with CLI properly', () => {
    ...
    const helpResult = bunExecSync(`openclawd --help`, { encoding: 'utf8' });
    expect(helpResult).toContain('publish');

    // Test that publish command can be invoked
    const publishHelpResult = bunExecSync(`openclawd publish --help`, { encoding: 'utf8' });
  ...
  it('publish dry-run flag works', () => {
    ...
    const result = bunExecSync(`openclawd publish --dry-run --help`, { encoding: 'utf8' });
    expect(result).toContain('dry-run');
  });
});
...
describe('npmPackage Scoping', () => {
  ...
  beforeEach(async () => {
    originalCwd = process.cwd();
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-npmpackage-'));
    process.chdir(testTmpDir);
  });

  afterEach(async () => {
    safeChangeDirectory(originalCwd);
    if (testTmpDir && testTmpDir.includes('openclawd-test-npmpackage-')) {
      try {
        await rm(testTmpDir, { recursive: true });
      } catch (e) {
        // Ignore cleanup errors
