...
export async function setupTestEnvironment(): Promise<TestContext> {
  ...
  const testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-'));
...
export async function cleanupTestEnvironment(context: TestContext): Promise<void> {
  ...
  if (context.testTmpDir && context.testTmpDir.includes('openclawd-test-')) {
    try {
...
/**
 * Helper to create a basic OpenClawd project for testing
 */
export async function createTestProject(projectName: string): Promise<void> {
  try {
    await bunExecSimple(`openclawd create ${projectName} --yes`);
...
export function expectHelpOutput(
  ...
  if (!output.includes(`Usage: openclawd ${command}`)) {
    throw new Error(`Expected help output to contain 'Usage: openclawd ${command}', got: ${output}`);
  }
...
export async function createTestPluginStructure(pluginName: string): Promise<void> {
  ...
  const packageJson = {
    name: `@openclawdsolana/plugin-${pluginName}`,
    version: '1.0.0',
    type: 'module',
    main: 'dist/index.js',
    openclawdPlugin: true,
  };
  ...
  while (Date.now() - startTime < maxWaitTime) {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${port}${endpoint}`, {
        ...
        headers: {
          'User-Agent': 'OpenClawd-Test-Client/1.0',
...
export async function cloneAndSetupPlugin(
  ...
  const pluginTestDir = await mkdtemp(join(tmpdir(), 'openclawd-plugin-test-'));
  ...
  console.log(`[PLUGIN SETUP] Linking @openclawdsolana/core and @openclawdsolana/server in plugin...`);
  // Link globally installed packages so the plugin can find them
  await spawnCommand('bun', ['link', '@openclawdsolana/core'], {
    cwd: pluginDir,
  });
  await spawnCommand('bun', ['link', '@openclawdsolana/server'], {
    cwd: pluginDir,
  });
  ...
  const cleanup = async () => {
    if (pluginTestDir && pluginTestDir.includes('openclawd-plugin-test-')) {
      try {
