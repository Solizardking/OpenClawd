...
This directory contains TypeScript tests for the OpenClawd CLI, converted from the original BATS test files.
...
- **`createTestProject()`** - Creates a test OpenClawd project
...
// Execute a command
const result = bunExecSync('openclawd [command]', { encoding: 'utf8' });

// With platform-specific options
import { getPlatformOptions } from './test-utils';
const result = bunExecSync('openclawd [command]', getPlatformOptions({ encoding: 'utf8' }));
```
...
describe('OpenClawd Command Tests', () => {
  ...
  it('command shows help', async () => {
    const result = bunExecSync('openclawd command --help', { encoding: 'utf8' });
...
1. **Replace direct paths**: Replace `bun "/path/to/cli/index.js"` with `openclawd` command
2. **Use temp directories**: Replace manual directory creation with `setupTestEnvironment()`
3. **Use bunExecSync**: Replace `execSync` or wrapper calls with `bunExecSync('openclawd [command]', { encoding: 'utf8' })`
...
2. **Long-running processes:**
   ...
   ```typescript
   const proc = Bun.spawn(['openclawd', 'start', ...args], {
     cwd: process.cwd(),
     env: { ...process.env },
     stdout: 'pipe',
     stderr: 'pipe',
...
3. **Error handling:**
   ```typescript
   try {
     bunExecSync('openclawd [command]', { encoding: 'utf8' });
   } catch (e: any) {
...
- **Reliability**: Works with global `openclawd` command via `bun link`
