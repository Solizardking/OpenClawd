...
describe('OpenClawd Update Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  beforeEach(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-update-'));
  ...
  afterEach(async () => {
    ...
    if (testTmpDir && testTmpDir.includes('openclawd-test-update-')) {
      try {
  ...
  const makeProj = async (name: string) => {
    bunExecSync(`openclawd create ${name} --yes`, { encoding: 'utf8' });
    process.chdir(join(testTmpDir, name));
  };

  // --help
  it('update --help shows usage and options', async () => {
    const result = bunExecSync('openclawd update --help', { encoding: 'utf8' });
    expect(result).toContain('Usage: openclawd update');
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd update', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd update --check', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd update --skip-build', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd update --packages', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      const result = bunExecSync('openclawd update --cli', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('openclawd update --cli --packages', { encoding: 'utf8' });
  ...
  it.skipIf(process.env.CI === 'true' || process.env.GITHUB_ACTIONS === 'true')(
    ...
    async () => {
      ...
      expect(result).toMatch(
        /(Project successfully updated|Update completed|already up to date|No updates available|create a new OpenClawd project|This appears to be an empty directory|Version: monorepo|Version: 1\.[2-9]\.\d+|CLI is already at the latest version)/
      );
    },
    TEST_TIMEOUTS.STANDARD_COMMAND
  );
  ...
  it(
    ...
    async () => {
      ...
      expect(result).toContain("This directory doesn't appear to be an OpenClawd project");
    },
    TEST_TIMEOUTS.STANDARD_COMMAND
  );

  it(
    'update --packages shows helpful message in non-openclawd project',
    async () => {
      // Create a non-OpenClawd package.json
      await writeFile(
        'package.json',
        JSON.stringify(
          {
      ...
      expect(result).toContain('openclawd create');
    },
    TEST_TIMEOUTS.STANDARD_COMMAND
  );

  it(
    'update --packages works in openclawd project with dependencies',
    async () => {
      await makeProj('update-openclawd-project');

      // Add some OpenClawd dependencies to make it a valid project
      await writeFile(
        'package.json',
        JSON.stringify(
          {
            name: 'test-openclawd-project',
            version: '1.0.0',
            dependencies: {
      ...
      const result = bunExecSync('openclawd update --packages --check', { encoding: 'utf8' });

      expect(result).toContain('OpenClawd');
    },
    TEST_TIMEOUTS.INDIVIDUAL_TEST
  );

  it(
    'update --packages shows message for project without openclawd dependencies',
    async () => {
      await makeProj('update-no-deps-project');

      // Create package.json without OpenClawd dependencies
      await writeFile(
        'package.json',
        JSON.stringify(
          {
      ...
      expect(result).toContain('No OpenClawd packages found');
    },
    TEST_TIMEOUTS.INDIVIDUAL_TEST
  );
  ...
  it.skipIf(process.env.CI === 'true' || process.env.GITHUB_ACTIONS === 'true')(
    ...
    async () => {
      ...
      const tmpDir = mkdtempSync(join(tmpdir(), 'openclawd-test-'));
  ...
  describe('bunx/npx detection', () => {
    it.skip('update --cli shows warning when running via bunx', () => {
      ...
      const result = bunExecSync('openclawd update --cli', {
        encoding: 'utf8',
        env: {
          ...process.env,
      ...
      expect(result).toContain('bun install -g @openclawdsolana/cli');
    });
    ...
    it.skip('update --cli works with global bun installation', () => {
      ...
      process.argv = [
        ...
        '/Users/user/.bun/install/global/@openclawdsolana/cli/dist/index.js',
        'update',
        '--cli',
      ];
      process.env = {};
    ...
    it.skip('update --cli works with global npm installation', () => {
      ...
      process.argv = [
        'node',
        '/usr/local/lib/node_modules/@openclawdsolana/cli/dist/index.js',
        'update',
        '--cli',
      ];
      process.env = {};
    ...
    it(
      ...
      async () => {
        ...
        const result = bunExecSync('openclawd update --packages', {
          encoding: 'utf8',
          env: {
            ...process.env,
            BUN_INSTALL_CACHE_DIR: '/Users/user/.bun/install/cache',
    ...
    it.skip(
      ...
      async () => {
        ...
        // Simulate bunx execution by setting environment variable
        const result = bunExecSync('openclawd update', {
          encoding: 'utf8',
          env: {
            ...process.env,
            BUN_INSTALL_CACHE_DIR: '/Users/user/.bun/install/cache',
        ...
        expect(result).toContain('CLI update is not available when running via npx or bunx');
        expect(result).toMatch(
          /(Project successfully updated|Update completed|already up to date|No updates available|Found.*OpenClawd package)/
        );
      },
      TEST_TIMEOUTS.INDIVIDUAL_TEST
    );
