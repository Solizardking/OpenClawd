...
describe('OpenClawd Agent Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  beforeAll(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'openclawd-test-agent-'));
    ...
    // Start the OpenClawd server with a default character
    console.log(`[DEBUG] Starting OpenClawd server on port ${testServerPort}`);
      ...
      const proc = Bun.spawn(
        ['bun', cliPath, 'start', '--port', testServerPort, '--character', defaultCharacter],
        {
          env: {
            ...process.env,
            LOG_LEVEL: 'debug',
            PGLITE_DATA_DIR: `${testTmpDir}/openclawddb`,
...
describe('OpenClawd Agent Stop All - Final Cleanup', { timeout: TEST_TIMEOUTS.INDIVIDUAL_TEST }, () => {
  it('agent stop --all works for stopping all agents', async () => {
    ...
    try {
      ...
      expect(result).toMatch(/(All OpenClawd agents stopped|stopped successfully)/);
    } catch (e: any) {
      // The command may succeed even if no agents are running
      // Handle case where stdout/stderr might be undefined
      const output = (e.stdout || '') + (e.stderr || '') + (e.message || '');
      expect(output).toMatch(
        /(stopped|All OpenClawd agents stopped|Windows|WSL|requires Unix-like commands)/
      );
    }
  });
});
