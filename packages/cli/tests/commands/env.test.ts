...
describe('OpenClawd Env Commands', { timeout: TEST_TIMEOUTS.SUITE_TIMEOUT }, () => {
  ...
  it('env --help shows usage', async () => {
    const result = bunExecSync('openclawd env --help', { encoding: 'utf8' });
    expectHelpOutput(result, 'env');
  });

  it('env list shows environment variables', async () => {
    // First call: no local .env file present
    let result = bunExecSync('openclawd env list', { encoding: 'utf8' });
    ...
    result = bunExecSync('openclawd env list', { encoding: 'utf8' });
  ...
  it('env list --local shows only local environment', async () => {
    ...
    const result = bunExecSync('openclawd env list --local', { encoding: 'utf8' });
  ...
  it('env reset shows all necessary options', async () => {
    ...
    const result = bunExecSync('openclawd env reset --yes', { encoding: 'utf8' });
