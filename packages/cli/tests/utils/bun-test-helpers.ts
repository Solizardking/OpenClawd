/**
 * Centralized test utilities for OpenClawd CLI tests
 *
...
/**
 * Run OpenClawd CLI command synchronously
 * Specialized helper for testing CLI commands
 *
 * @param args - CLI arguments (without 'openclawd' prefix)
 * @param options - Execution options
...
export function runCliCommandSync(args: string[], options: BunExecSyncOptions = {}): string {
  // Use global openclawd command
  const command = `openclawd ${args.map((arg) => (arg.includes(' ') ? `"${arg}"` : arg)).join(' ')}`;
