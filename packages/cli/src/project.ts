...
import { stringToUuid } from '@openclawdsolana/core';
...
export async function loadProject(dir: string): Promise<Project> {
  ...
  if (!main) {
    ...
    // Create a fallback project with the default OpenClawd character
    // Use deterministic UUID based on character name to match runtime behavior
    const defaultCharacterName = 'OpenClawd (Default)';
    const openclawdCharacter = getDefaultCharacter(); // Get the filtered character based on env vars
    const defaultAgent: ProjectAgent = {
      ...
      init: async () => {
        logger.info({ src: 'cli', util: 'project' }, 'Initializing default OpenClawd character');
      },
    };
  ...
  if (moduleIsPlugin) {
    ...
    // Use the OpenClawd character as our test agent
    // Use deterministic UUID based on character name to match runtime behavior
    const characterName = 'OpenClawd (Test Mode)';
    const openclawdCharacter = getDefaultCharacter(); // Get the filtered character based on env vars
    const testCharacter: Character = {
      ...openclawdCharacter,
      id: stringToUuid(characterName) as UUID,
      name: characterName,
      system: `${openclawdCharacter.system} Testing the plugin: ${completePlugin.name}.`,
    };

    logger.info(
      { src: 'cli', util: 'project', pluginName: completePlugin.name },
      'Using OpenClawd character as test agent for plugin'
    );
    ...
    const testAgent: ProjectAgent = {
      ...
      init: async () => {
        logger.info(
          { src: 'cli', util: 'project', pluginName: completePlugin.name },
          'Initializing OpenClawd test agent for plugin'
        );
        // The plugin will be registered automatically in runtime.initialize()
      },
    };

    // Since we're in test mode, OpenClawd (our test agent) needs to already exist in the database
