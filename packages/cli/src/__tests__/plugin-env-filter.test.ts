...
describe('Plugin Environment Variable Filtering', () => {
  ...
  describe('scanPluginsForEnvDeclarations', () => {
    ...
    it('should scan plugin with env var declarations', () => {
      ...
      const nodeModulesPath = path.join(tempDir, 'node_modules', '@openclawd', 'plugin-test');
      ...
      expect(result.pluginsWithDeclarations).toContain('@openclawdsolana/plugin-test');
      expect(result.varInfo.get('TEST_API_KEY')?.plugin).toBe('@openclawdsolana/plugin-test');
    });

    it('should detect plugins without declarations', () => {
      // Create a mock plugin without env var declarations
      const nodeModulesPath = path.join(tempDir, 'node_modules', '@openclawd', 'plugin-empty');
      fs.mkdirSync(nodeModulesPath, { recursive: true });

      const packageJson = {
        name: '@openclawdsolana/plugin-empty',
        version: '1.0.0',
        agentConfig: {
          pluginType: 'openclawd:plugin:1.0.0',
          // No pluginParameters
      ...
      expect(result.pluginsWithoutDeclarations).toContain('@openclawdsolana/plugin-empty');
    });

    it('should scan multiple plugins', () => {
      // Create two mock plugins
      const plugin1Path = path.join(tempDir, 'node_modules', '@openclawd', 'plugin-one');
      const plugin2Path = path.join(tempDir, 'node_modules', '@openclawd', 'plugin-two');
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: '@openclawdsolana/plugin-one',
          version: '1.0.0',
          agentConfig: {
            pluginType: 'openclawd:plugin:1.0.0',
            pluginParameters: {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: '@openclawdsolana/plugin-two',
          version: '1.0.0',
          agentConfig: {
            pluginType: 'openclawd:plugin:1.0.0',
            pluginParameters: {
    ...
    it('should search parent directories for node_modules', () => {
      ...
      const parentNodeModules = path.join(tempDir, 'node_modules', '@openclawd', 'plugin-parent');
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: '@openclawdsolana/plugin-parent',
          version: '1.0.0',
          agentConfig: {
            pluginType: 'openclawd:plugin:1.0.0',
            pluginParameters: {
  ...
  describe('warnAboutMissingDeclarations', () => {
    it('should return true when official plugins are missing declarations', () => {
      const plugins = ['@openclawdsolana/plugin-missing', '@openclawdsolana/plugin-also-missing'];
  ...
  describe('Integration: Shell Leakage Prevention', () => {
    it('should demonstrate complete flow of filtering shell vars', () => {
      ...
      const nodeModulesPath = path.join(tempDir, 'node_modules', '@openclawd', 'plugin-openrouter');
      fs.mkdirSync(nodeModulesPath, { recursive: true });

      fs.writeFileSync(
        path.join(nodeModulesPath, 'package.json'),
        JSON.stringify({
          name: '@openclawdsolana/plugin-openrouter',
          version: '1.0.0',
          agentConfig: {
            pluginType: 'openclawd:plugin:1.0.0',
            pluginParameters: {
