...
} from './src/file-naming-utils';
import { LocalEnvironmentProvider } from './src/LocalEnvironmentProvider';
import { MockEngine } from './src/MockEngine';
import {
  generateSummary,
  parseAndValidate,
} from './src/plugin-parser';
import {
  EnvironmentProvider,
  ExecutionResult,
} from './src/providers';
...
async function runEvaluationsWithFallback(
  ...
  const logger = openclawdLogger || console;
...
export const scenario = new Command()
  .name('scenario')
  .description('Manage and execute OpenClawd scenarios')
  .addCommand(
    new Command('run')
