# The OpenClawd Scenario System: A Definition

A **Scenario** is a declarative, human-readable definition of a task for an OpenClawd agent. It serves a dual purpose:
...
Scenarios are defined in `.yaml` files and are executed by a specialized **Scenario Runner** within the `openclawd` CLI. They are designed to bridge the gap between low-level unit/integration tests and high-level, goal-oriented agent behavior.
...
- **Environment Agnostic**: Scenarios define _what_ the agent should do, not _where_. The same scenario can be executed against a local environment or deployed to cloud containers via `openclawd deploy`.
...
- **Plugins**: A list of the required OpenClawd plugins the agent needs to run the scenario (e.g., `@openclawdsolana/plugin-github`).
- **Environment**: Defines the execution environment for the agent (currently only 'local' is supported; use `openclawd deploy` for cloud container deployments).
...
- **Run**: Defines the task(s) for the agent. This can be:
  - **Single-turn**: A single, high-level input leveraging OpenClawd's action chaining capabilities
...
Scenarios are executed via a dedicated top-level `openclawd scenario` command, which clearly separates scenario execution from standard testing (`openclawd test`) or running the agent server (`openclawd start`).

The runner supports two primary modes of operation on the same scenario file:

- **Test Mode**:
  - Production: `openclawd scenario run <scenario_file.yaml>`
...
- **Live Mode**:
  - Production: `openclawd scenario run <scenario_file.yaml> --live`
...
This definition of the Scenario System is the result of careful consideration of both the unique challenges of testing AI agents and the current architecture of OpenClawd.

### Working with OpenClawd's Architecture

The Scenario System is designed to test both:

1. **Goal-oriented tasks** accomplished by OpenClawd's existing **linear action chaining** capability
...
The conversation system uses existing OpenClawd infrastructure:
...
The dynamic prompting system extends OpenClawd scenarios without breaking changes:
...
This comprehensive conversation system enables sophisticated agent testing while maintaining full backward compatibility with existing OpenClawd scenarios.
