...
This document provides a simple specification for creating `scenario.yaml` and `matrix.yaml` files for OpenClawd testing.
...
- `environment.type`: Must be "local" (use `openclawd deploy` for cloud container deployments)
...
- parameter: 'plugins[1].name'
  values: ['@openclawdsolana/plugin-github', '@openclawdsolana/plugin-sql']
```
...
- `@openclawdsolana/plugin-sql` - Database operations
- `@openclawdsolana/plugin-bootstrap` - Core functionality
- `@openclawdsolana/plugin-openai` - LLM operations
...
8. Always include required plugins (`@openclawdsolana/plugin-bootstrap` is usually needed)
