# OpenClawd Scenario Testing & Matrix Runner Guide

This guide covers the comprehensive scenario testing and matrix runner functionality implemented through Epic [#5781](https://github.com/OpenClawd/openclawd/issues/5781) and associated tickets.

## Overview

The OpenClawd Scenario System provides powerful testing capabilities for agents in both local and sandboxed environments. Each scenario is defined in YAML and can include:
...
openclawd scenario run <scenario-file> [options]

# Run matrix testing with parameter combinations
openclawd scenario matrix <matrix-config> [options]

# Generate comprehensive reports
openclawd report generate <input-directory> [options]
```
...
openclawd scenario run packages/cli/src/commands/scenario/examples/simple-test.scenario.yaml

# Run action tracking test
openclawd scenario run packages/cli/src/commands/scenario/examples/action-tracking-test.scenario.yaml

# Run evaluation test
openclawd scenario run packages/cli/src/commands/scenario/examples/evaluation-test.scenario.yaml

# Run basic conversation test
openclawd scenario run packages/cli/src/commands/scenario/examples/basic-conversation.scenario.yaml
```
...
openclawd scenario run packages/cli/src/commands/scenario/examples/simple-mock-test.scenario.yaml

# Run full mock test
openclawd scenario run packages/cli/src/commands/scenario/examples/mock-test.scenario.yaml
```
...
openclawd scenario run packages/cli/src/commands/scenario/examples/llm-judge-test.scenario.yaml

# Run LLM judgment failure test
openclawd scenario run packages/cli/src/commands/scenario/examples/llm-judge-failure-test.scenario.yaml
```
...
```bash
# Run matrix testing with parameter combinations
openclawd scenario matrix packages/cli/src/commands/scenario/examples/github-issue-analysis.matrix.yaml

# Run simple matrix test
openclawd scenario matrix packages/cli/src/commands/scenario/examples/simple-test.matrix.yaml
```
...
openclawd scenario run packages/cli/src/commands/scenario/examples/customer-support.scenario.yaml
openclawd scenario run packages/cli/src/commands/scenario/examples/technical-interview.scenario.yaml
```
...
openclawd scenario run packages/cli/src/commands/scenario/examples/multi-step.scenario.yaml

# Run mixed results test
openclawd scenario run packages/cli/src/commands/scenario/examples/mixed-results.scenario.yaml

# Run trajectory test
openclawd scenario run packages/cli/src/commands/scenario/examples/trajectory-test.scenario.yaml
```
...
openclawd report generate packages/cli/src/commands/scenario/_logs_

# Generate specific format only
openclawd report generate packages/cli/src/commands/scenario/_logs_ --format json
openclawd report generate packages/cli/src/commands/scenario/_logs_ --format html
openclawd report generate packages/cli/src/commands/scenario/_logs_ --format pdf
```
...
openclawd scenario run packages/cli/src/commands/scenario/examples/*.scenario.yaml
```
...
1. **Ensure you're running from the project root directory**
   ...
   **Production Commands:**
   ...
   openclawd scenario run <scenario-file>
   ```
...
2. **If plugins still fail to load, add them as dependencies**
   ...
   bun add openclawd/plugin-name
   ```

   Common plugin examples:

   ```bash
   bun add openclawd/plugin-bootstrap
   bun add openclawd/plugin-sql
   bun add openclawd/plugin-quick-starter
   ```

3. **Verify plugin installation**
   ```bash
   # Check if the plugin is listed in package.json dependencies
   cat package.json | grep "openclawd/"
   ```

This approach ensures that the plugin is properly installed and available for the OpenClawd runtime to load during scenario execution.
...
1. **YAML Parser** ([#5574](https://github.com/OpenClawd/openclawd/issues/5574))
   - Validates scenario file structure
   - Provides type-safe scenario configuration

2. **Environment Providers**
   - Local ([#5575](https://github.com/OpenClawd/openclawd/issues/5575))

3. **Mock Engine** ([#5577](https://github.com/OpenClawd/openclawd/issues/5577))
   - Service call interception
   - Response mocking

4. **Evaluation Engine** ([#5578](https://github.com/OpenClawd/openclawd/issues/5578))
   - Action tracking
   - Response validation
   - Trajectory analysis
...
5. **Final Judgment** ([#5579](https://github.com/OpenClawd/openclawd/issues/5579))
   - LLM-based judgment
   - User-facing reports
...
3. **Plugin Loading Issues**: If plugins are not being loaded correctly:
   ...
   - If plugins still fail to load, run `bun add openclawd/plugin-name` from the root directory
   - This will add the plugin as a dependency and ensure it's available for loading
   - Example: `bun add openclawd/plugin-bootstrap` or `bun add openclawd/plugin-sql`
...
This comprehensive system was implemented through Epic [#5781](https://github.com/OpenClawd/openclawd/issues/5781) with the following components:
...
- [CLI Command Implementation](https://github.com/OpenClawd/openclawd/issues/5573)
- [YAML Parser](https://github.com/OpenClawd/openclawd/issues/5574)
- [Local Environment](https://github.com/OpenClawd/openclawd/issues/5575)
- [Mock Engine](https://github.com/OpenClawd/openclawd/issues/5577)
- [Evaluation Engine](https://github.com/OpenClawd/openclawd/issues/5578)
- [Final Judgment](https://github.com/OpenClawd/openclawd/issues/5579)

### **New Features (Epic #5781)**

- [Matrix Runner Implementation](https://github.com/OpenClawd/openclawd/issues/5782)
- [Reporting System](https://github.com/OpenClawd/openclawd/issues/5783)
- [Trajectory Analysis](https://github.com/OpenClawd/openclawd/issues/5784)
- [Enhanced Evaluations](https://github.com/OpenClawd/openclawd/issues/5785)
- [Data Aggregation](https://github.com/OpenClawd/openclawd/issues/5786)
- [Resource Monitoring](https://github.com/OpenClawd/openclawd/issues/5787)
- [Progress Tracking](https://github.com/OpenClawd/openclawd/issues/5788)
- [Run Isolation](https://github.com/OpenClawd/openclawd/issues/5789)
- [Error Handling](https://github.com/OpenClawd/openclawd/issues/5790)
