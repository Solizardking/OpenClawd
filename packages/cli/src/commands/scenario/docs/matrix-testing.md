...
This document explains how to use scenario matrix testing to systematically evaluate OpenClawd agents across multiple parameter combinations.

## 🎉 Matrix Testing is Now Available!

The `openclawd scenario matrix` command is fully implemented and ready to use! You can now:
...
openclawd scenario matrix packages/cli/src/commands/scenario/examples/github-issue-analysis.matrix.yaml --dry-run
```
...
matrix:
  ...
  - parameter: 'run[0].input'
    values:
      - 'List open issues for OpenClawd/openclawd'
      - 'Find current issues for the openclawd/openclawd repo'
      - "Show me what's open in the OpenClawd/openclawd GitHub."
```
...
```yaml
- parameter: 'plugins[0].name'
  values: ['@openclawdsolana/plugin-bootstrap', '@openclawdsolana/plugin-sql']
- parameter: 'plugins[0].enabled'
  values: [true, false]
...
matrix:
  ...
  - parameter: 'plugins'
    values:
      - ['@openclawdsolana/plugin-bootstrap']
      - ['@openclawdsolana/plugin-bootstrap', '@openclawdsolana/plugin-sql']
```
...
The `openclawd scenario matrix` command is now available and provides comprehensive matrix testing capabilities.
...
openclawd scenario matrix path/to/config.matrix.yaml

# Dry run mode - analysis only, no execution
openclawd scenario matrix path/to/config.matrix.yaml --dry-run

# Verbose mode - show detailed information
openclawd scenario matrix path/to/config.matrix.yaml --verbose

# Filter combinations by pattern
openclawd scenario matrix path/to/config.matrix.yaml --filter "gpt-4"

# Specify parallel execution (for future implementation)
openclawd scenario matrix path/to/config.matrix.yaml --parallel 4
```
...
When you run `openclawd scenario matrix config.matrix.yaml --dry-run`, you'll see:
...
🎯 Matrix Structure:
   Axis 1: character.llm.model
     ...
     Count: 2
   Axis 2: run[0].input
     Values: ["List open issues for OpenClawd/openclawd", "Find current issues...", "Show me what's open..."]
     Count: 3

✅ Base scenario file found: /path/to/base-scenario.yaml

🔀 Parameter Combinations:
   1. {"character.llm.model":"gpt-4-turbo","run[0].input":"List open issues for OpenClawd/openclawd"}
   2. {"character.llm.model":"gpt-4-turbo","run[0].input":"Find current issues for the openclawd/openclawd repo"}
   3. {"character.llm.model":"gpt-4-turbo","run[0].input":"Show me what's open in the OpenClawd/openclawd GitHub."}
   4. {"character.llm.model":"gpt-3.5-turbo","run[0].input":"List open issues for OpenClawd/openclawd"}
   5. {"character.llm.model":"gpt-3.5-turbo","run[0].input":"Find current issues for the openclawd/openclawd repo"}
   6. {"character.llm.model":"gpt-3.5-turbo","run[0].input":"Show me what's open in the OpenClawd/openclawd GitHub."}
...
- `openclawd scenario matrix` CLI command
...
openclawd scenario matrix --help

# Show help for all scenario commands
openclawd scenario --help
```
...
This matrix testing system enables systematic, scientific evaluation of OpenClawd agents across multiple dimensions, providing the data needed to optimize agent performance and reliability.
