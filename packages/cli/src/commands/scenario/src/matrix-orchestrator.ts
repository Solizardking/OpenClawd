...
import { IAgentRuntime, UUID, logger } from '@openclawdsolana/core';
...
export async function executeMatrixRuns(
  ...
  try {
    ...
    const defaultPlugins = ['@openclawdsolana/plugin-sql', '@openclawdsolana/plugin-bootstrap'];
    ...
    const finalPlugins = Array.from(
      new Set([...scenarioPlugins, ...defaultPlugins, '@openclawdsolana/plugin-openai'])
    );
...
async function executeScenarioWithTimeout(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      process.env = {
        ...originalEnv,
        OPENCLAWD_DB_PATH: context.dbPath,
        OPENCLAWD_LOG_PATH: context.logPath,
        OPENCLAWD_TEMP_DIR: context.tempDir,
      };
