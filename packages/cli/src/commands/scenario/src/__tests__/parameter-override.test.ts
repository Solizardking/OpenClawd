...
describe('Parameter Override System', () => {
  ...
  beforeEach(() => {
    ...
    baseScenario = {
      ...
      plugins: [
        { name: '@openclawdsolana/plugin-bootstrap', enabled: true },
        { name: '@openclawdsolana/plugin-sql', enabled: false, config: { apiKey: 'default-key' } },
      ],
      environment: {
        type: 'local',
        settings: {
  ...
  describe('Real-World Scenario Examples', () => {
    it('should handle matrix parameter combinations', () => {
      const combination1: ParameterOverride[] = [
        ...
        { path: 'run[0].input', value: 'List open issues for OpenClawd/openclawd' },
      ];

      const combination2: ParameterOverride[] = [
        { path: 'character.llm.model', value: 'gpt-3.5-turbo' },
        { path: 'run[0].input', value: 'Find current issues for the openclawd/openclawd repo' },
      ];
      ...
      expect(result1.run[0].input).toBe('List open issues for OpenClawd/openclawd');

      expect(result2.character.llm.model).toBe('gpt-3.5-turbo');
      expect(result2.run[0].input).toBe('Find current issues for the openclawd/openclawd repo');
