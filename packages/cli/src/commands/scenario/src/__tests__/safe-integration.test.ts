...
describe('Safe Integration with Fallback (Ticket #5783)', () => {
  ...
  beforeEach(() => {
    ...
    originalEnv = process.env.OPENCLAWD_ENHANCED_EVALUATIONS;
  ...
  afterEach(() => {
    ...
    if (originalEnv !== undefined) {
      process.env.OPENCLAWD_ENHANCED_EVALUATIONS = originalEnv;
    } else {
      delete process.env.OPENCLAWD_ENHANCED_EVALUATIONS;
    }
  });
  ...
  async function runEvaluationsWithFallback(
    ...
    const useEnhanced = process.env.OPENCLAWD_ENHANCED_EVALUATIONS === 'true';
  ...
  describe('Feature Flag Disabled', () => {
    beforeEach(() => {
      process.env.OPENCLAWD_ENHANCED_EVALUATIONS = 'false';
    });
  ...
  describe('Feature Flag Enabled - Success Path', () => {
    beforeEach(() => {
      process.env.OPENCLAWD_ENHANCED_EVALUATIONS = 'true';
    });
