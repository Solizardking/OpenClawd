import { Character, UUID, IAgentRuntime, stringToUuid } from '@openclawdsolana/core';
import { AgentServer } from '@openclawdsolana/server';
import { OpenClawdClient } from '@openclawdsolana/api-client';
import type { Message } from '@openclawdsolana/api-client';
import { ChannelType, stringToUuid as stringToUuidCore } from '@openclawdsolana/core';
  ...
  while (retryCount < maxRetries) {
    try {
      if (existingServer) {
        ...
        // Prefer unique directory per scenario run under PGLite root (env or default .openclawd/.openclawddb)
        const pgliteRoot =
          process.env.PGLITE_DATA_DIR || path.join(process.cwd(), '.openclawd', '.openclawddb');
...
export async function askAgentViaApi(
  ...
  try {
    ...
    const port =
      ...
      3000;
    const client = OpenClawdClient.create({ baseUrl: `http://localhost:${port}` });
