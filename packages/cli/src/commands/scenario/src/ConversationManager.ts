...
import { IAgentRuntime, UUID, ModelType } from '@openclawdsolana/core';
...
export class ConversationManager {
  ...
  private async createChannelDirectly(): Promise<UUID> {
    const { OpenClawdClient } = await import('@openclawdsolana/api-client');
    const { ChannelType, stringToUuid: stringToUuidCore } = await import('@openclawdsolana/core');

    const port = this.serverPort;
    const client = new OpenClawdClient({ baseUrl: `http://localhost:${port}` });
