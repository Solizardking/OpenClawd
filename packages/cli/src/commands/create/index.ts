...
export const create = new Command('create')
  .description('Create a new OpenClawd project, plugin, agent, or TEE project')
  ...
  .action(async (name?: string, opts?: Record<string, unknown>) => {
    ...
    try {
      ...
      if (
        process.env.OPENCLAWD_NONINTERACTIVE === '1' ||
        process.env.OPENCLAWD_NONINTERACTIVE === 'true' ||
      ...
      if (!projectName) {
          ...
          const selectedType = await clack.select({
            ...
            options: [
              ...
              },
              {
                ...
                value: 'plugin',
                hint: 'Extend OpenClawd functionality with custom plugins',
              },
              {
        ...
        if (!isNonInteractive) {
          ...
          clack.intro(colors.inverse(` Creating OpenClawd ${introType} `));
        }
        ...
        if (!isNonInteractive) {
          ...
          throw new Error(`Project name is required. Usage: openclawd create [name]`);
        }
      }
