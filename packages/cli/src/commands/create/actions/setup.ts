...
import {
  ...
  promptAndStoreOpenClawdCloudKey,
  setupPgLite,
} from '@/src/utils';
...
export async function createProjectDirectories(targetDir: string): Promise<void> {
  await ensureOpenClawdDir(targetDir);
}
...
export async function setupAIModelConfig(
  ...
  try {
    switch (aiModel) {
      case 'openclawdcloud': {
        // Configure OpenClawd Cloud for multi-model AI
        if (isNonInteractive) {
          let content = '';
          ...
          content += 'OPENCLAWD_API_KEY=your_openclawd_api_key_here\n';
          content +=
            '# Available models: gpt-4o-mini, gpt-4o, claude-3-5-sonnet, gemini-2.0-flash\n';
          content += '# OPENCLAWD_CLOUD_SMALL_MODEL=gpt-4o-mini\n';
          content += '# OPENCLAWD_CLOUD_LARGE_MODEL=gpt-4o\n';

          await fs.writeFile(envFilePath, content, 'utf8');
        } else {
          // Interactive mode - prompt for OpenClawd Cloud API key or login
          await promptAndStoreOpenClawdCloudKey(envFilePath);
        }
        break;
      }
...
function resolveModelToPlugin(modelName: string): string | null {
  const modelToPlugin: Record<string, string> = {
    openclawdcloud: 'openclawdcloud', // OpenClawd Cloud multi-model AI plugin
    openai: 'openai',
    claude: 'anthropic',
    anthropic: 'anthropic',
...
export async function setupProjectEnvironment(
  ...
  // This is required because the default OpenClawd character always includes it
  // Skip for OpenClawd Cloud since it provides all functionality including embeddings
  if (aiModel !== 'local' && aiModel !== 'openclawdcloud' && embeddingModel !== 'local') {
