...
export const eigenCliCommand = new Command('eigen')
  ...
  .action(async (...commandArgs) => {
    ...
    if (!resolvedBinary) {
      ...
      const autoInstall = process.env.OPENCLAWD_TEE_EIGEN_AUTO_INSTALL === 'true';
    ...
    if (process.env.OPENCLAWD_TEE_EIGEN_EXTRA_PATHS) {
      const extraPaths = process.env.OPENCLAWD_TEE_EIGEN_EXTRA_PATHS.split(path.delimiter)
        .map((p) => p.trim())
        .filter((p) => p.length > 0);
  ...
  .on('--help', () => {
    ...
    console.log('  $ openclawd tee eigen help');
    console.log('  $ openclawd tee eigen app list');
    console.log('  $ openclawd tee eigen app deploy myregistry/myapp:v1.0');
    console.log('');
