import { logger, asUUID } from '@openclawdsolana/core';
...
import { AgentsService } from '@openclawdsolana/api-client';
      ...
      if (errorMsg.startsWith('AGENT_NOT_FOUND:')) {
        ...
        try {
          ...
          if (agents.length > 0) {
            ...
            console.error(
              `\nYou can create a new agent with: openclawd create --type agent ${agentName.toLowerCase()}`
            );
          }
        } catch (error) {
...
export async function stopAgent(opts: OptionValues): Promise<void> {
  ...
  try {
    ...
    if (!hasValidName && !opts.all) {
      ...
      console.error('  openclawd agent stop --name openclawd');
      console.error('  openclawd agent stop --all');
      process.exit(1);
    }

    // If --all flag is provided, stop all local OpenClawd processes
    if (opts.all) {
      logger.info({ src: 'cli', command: 'agent-stop' }, 'Stopping all OpenClawd agents');
      ...
      try {
        ...
        // Support both node and bun executables, and look for common OpenClawd patterns
        const patterns = [
          '(node|bun).*openclawd',
          '(node|bun).*openclawd.*start',
        ...
        logger.success(
          { src: 'cli', command: 'agent-stop' },
          'All OpenClawd agents stopped successfully'
        );
      } catch (error) {
        logger.error(
          {
