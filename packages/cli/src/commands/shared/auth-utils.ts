...
import type { ApiClientConfig } from '@openclawdsolana/api-client';
...
export function getAuthHeaders(opts: OptionValues): Record<string, string> {
  ...
  const authToken = opts.authToken || process.env.OPENCLAWD_API_KEY;
 ...
 * @returns ApiClientConfig for use with @openclawdsolana/api-client
 */
