...
export const update = new Command()
  .name('update')
  .description('Update OpenClawd CLI and project dependencies')
    ...
    try {
      ...
      if (updateCli) {
        ...
        if (isNpx || isBunx) {
          ...
          console.info(' bun install -g @openclawdsolana/cli');
      ...
      if (updatePackages) {
        ...
        if (!directoryInfo) {
          ...
          console.info('To create a new OpenClawd project, use: openclawd create <project-name>');
          return;
        }
        ...
        const isPlugin = directoryInfo.type === 'openclawd-plugin';

        if (directoryInfo.openclawdPackageCount === 0) {
          console.info('No OpenClawd packages found in this project.');
          console.info(
            "This might be a new project that hasn't installed OpenClawd dependencies yet."
          );
          console.info('Consider adding OpenClawd packages first, such as: bun add @openclawdsolana/core');
          return;
        }

        console.info(
          `Found ${directoryInfo.openclawdPackageCount} OpenClawd package(s) to check for updates`
        );
        ...
        if (options.check) {
          ...
          console.log(`${projectType} successfully updated to the latest OpenClawd packages`);
        }
      }
    } catch (error) {
      handleError(error);
