 ...
 *
 * Updates OpenClawd dependencies in a project or plugin, with support for dry-run mode, major version confirmation, and optional build step.
 */
export async function updateDependencies(
  ...
  if (!hasUpdates) {
    console.log('All OpenClawd packages are up to date! [✓]');
    return;
  }
  ...
  if (dryRun) {
    console.log('\nTo apply these updates, run: openclawd update');
    return;
  }
