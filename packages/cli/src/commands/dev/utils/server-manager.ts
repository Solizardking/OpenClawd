...
function setupEnvironment(): Record<string, string> {
  ...
  // Preserve OPENCLAWD_TEST_MODE for test environments
  if (process.env.OPENCLAWD_TEST_MODE) {
    env.OPENCLAWD_TEST_MODE = process.env.OPENCLAWD_TEST_MODE;
  }
...
async function startServerProcess(args: string[] = []): Promise<void> {
  ...
  if (localCliPath) {
    console.info('Using local @openclawdsolana/cli installation');
  ...
  const isTestMode = process.env.OPENCLAWD_TEST_MODE === 'true';
