...
import { ensureOpenClawdCli } from '@/src/utils/dependency-manager';
...
async function startClientDevServer(cwd: string): Promise<void> {
  ...
  if (fs.existsSync(path.join(monorepoClientPath, 'package.json'))) {
    ...
    if (fs.existsSync(path.join(parentClientPath, 'package.json'))) {
      ...
      // Check for installed @openclawdsolana/client
      const installedClientPath = path.join(cwd, 'node_modules', '@openclawd', 'client');
      ...
      // BUT: prevent recursive execution when running openclawd dev from the same directory
      if (!clientDir) {
        const localViteTs = path.join(cwd, 'vite.config.ts');
        const localViteJs = path.join(cwd, 'vite.config.js');
        if (fs.existsSync(localViteTs) || fs.existsSync(localViteJs)) {
          ...
          if (fs.existsSync(packageJsonPath)) {
            try {
              ...
              // If the dev script would run openclawd dev, skip to prevent recursion
              if (devScript && devScript.includes('openclawd dev')) {
                console.warn(
                  'Detected potential recursive openclawd dev execution in local Vite config. Skipping client dev server to prevent infinite loop.'
                );
                return;
              }
            } catch (error) {
  ...
  const isInstalledClient = clientDir.includes(path.join('node_modules', '@openclawd', 'client'));
  if (isInstalledClient) {
    console.info(
      'Detected installed @openclawdsolana/client. Using server static UI instead of running Vite.'
    );
    return;
  }
...
async function getClientPort(cwd: string): Promise<number | null> {
  const possibleClientDirs = [
    ...
    path.join(cwd, 'node_modules', '@openclawd', 'client'),
  ];
...
export async function startDevMode(options: DevOptions): Promise<void> {
  ...
  // Auto-install @openclawdsolana/cli as dev dependency using bun (for non-monorepo projects)
  await ensureOpenClawdCli(cwd);

  const context = createDevContext(cwd);
  const serverManager = getServerManager();

  const { directoryType } = context;
  const isProject = directoryType.type === 'openclawd-project';
  const isPlugin = directoryType.type === 'openclawd-plugin';
  const isMonorepo = directoryType.type === 'openclawd-monorepo';
  const inStandalone = !isProject && !isPlugin && !isMonorepo;

  // Log project type
  if (isProject) {
    console.info('Identified as an OpenClawd project package');
  } else if (isPlugin) {
    console.info('Identified as an OpenClawd plugin package');
  } else if (isMonorepo) {
    console.info('Identified as an OpenClawd monorepo');
  } else {
    console.warn(
      `Not in a recognized OpenClawd project, plugin, or monorepo directory. Current directory is: ${directoryType.type}. Running in standalone mode.`
    );
  }
  ...
  if (!inStandalone) {
    if (process.env.OPENCLAWD_TEST_MODE === 'true') {
    ...
    await new Promise((resolve) => setTimeout(resolve, 2000));
  } else if (process.env.OPENCLAWD_TEST_MODE === 'true') {
