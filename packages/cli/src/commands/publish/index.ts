...
export const publish = new Command()
  ...
  .action(async (opts: PublishOptions) => {
    try {
      ...
      if (directoryInfo.type === 'openclawd-plugin') {
        detectedType = 'plugin';
        console.info('Detected OpenClawd plugin using standardized directory detection');
      } else if (directoryInfo.type === 'openclawd-project') {
        detectedType = 'project';
        console.info('Detected OpenClawd project using standardized directory detection');
      } else {
        ...
        } else if (packageJson.openclawd?.type === 'project') {
          detectedType = 'project';
          console.info('Detected project from package.json openclawd.type (legacy format)');
      ...
      if (!packageJson.agentConfig) {
        packageJson.agentConfig = {
          pluginType: detectedType === 'plugin' ? 'openclawd:plugin:1.0.0' : 'openclawd:project:1.0.0',
          pluginParameters: {},
        };
      } else if (!packageJson.agentConfig.pluginType) {
        // Ensure pluginType is set based on detection
        packageJson.agentConfig.pluginType =
          detectedType === 'plugin' ? 'openclawd:plugin:1.0.0' : 'openclawd:project:1.0.0';
      }

      // For plugin type, validate naming convention
      if (detectedType === 'plugin' && !packageJson.name.includes('plugin-')) {
        console.warn(
          "This doesn't appear to be an OpenClawd plugin. Package name should include 'plugin-'."
        );
      ...
      const placeholderReplacements: Record<string, PlaceholderReplacement> = {
        ...
        'openclawd-plugin-starter': {
          check: () => packageJson.name === '@openclawdsolana/plugin-starter',
          replace: () => {
        ...
        '${PLUGINDESCRIPTION}': {
          ...
          replace: () => {
            ...
            packageJson.description = `OpenClawd plugin for ${simpleName}`;
      ...
      console.log('   Use standard npm and git workflows, not the OpenClawd CLI:');
      ...
      console.log('\n   The OpenClawd registry will automatically sync with npm updates.');
      console.log('   Only use "openclawd publish" for initial publishing of new plugins.');
    } catch (error) {
      handleError(error);
    }
  });
