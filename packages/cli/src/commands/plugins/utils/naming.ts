...
export const findPluginPackageName = (
  ...
  // Potential package names to check (prioritize @openclawdsolana/ over @openclawdsolana/)
  const possibleNames = [
    pluginInput, // Check the raw input first
    `@openclawdsolana/plugin-${normalizedBase}`, // Prioritize @openclawdsolana/ scope
    `@openclawdsolana/${normalizedBase}`, // Might be needed if input was 'plugin-abc' -> base 'abc' -> check '@openclawdsolana/abc'
    `@openclawdsolana/plugin-${normalizedBase}`, // Check alternative scope
    `plugin-${normalizedBase}`,
    `@openclawdsolana/${normalizedBase}`,
  ];
...
export const extractPackageName = (pluginInput: string): string => {
  ...
  if (githubUrlMatch) {
    ...
    // e.g., github:openclawd-plugins/plugin-discord -> @openclawdsolana/plugin-discord
    return `@${owner}/${repo}`;
  }
