...
export const extractPluginEnvRequirements = async (
  ...
  try {
    ...
    const possiblePaths = [
      ...
      // Scoped package path (e.g., @openclawdsolana/plugin-discord)
      path.join(cwd, 'node_modules', packageName.replace('/', path.sep), 'package.json'),
    ];

    // If the package name doesn't start with @openclawdsolana/, also try with that prefix
    if (!packageName.startsWith('@openclawdsolana/')) {
      possiblePaths.push(
        path.join(cwd, 'node_modules', '@openclawd', packageName, 'package.json'),
        path.join(cwd, 'node_modules', '@openclawd', `plugin-${packageName}`, 'package.json')
      );
    }
    ...
    if (!packageJsonPath) {
      ...
      if (existsSync(nodeModulesPath)) {
        ...
        for (const pkg of packages) {
          if (pkg.includes(packageName.replace('@openclawdsolana/', '').replace('plugin-', ''))) {
...
export const promptForPluginEnvVars = async (packageName: string, cwd: string): Promise<void> => {
  ...
  // If no requirements found and package doesn't start with @openclawdsolana/, try with that prefix
  if (Object.keys(envRequirements).length === 0 && !packageName.startsWith('@openclawdsolana/')) {
    // Try with @openclawdsolana/ prefix
    const openclawdPackageName = `@openclawdsolana/${packageName.replace('plugin-', '')}`;
    envRequirements = await extractPluginEnvRequirements(openclawdPackageName, cwd);

    // Also try with @openclawdsolana/plugin- prefix
    if (Object.keys(envRequirements).length === 0) {
      const openclawdPluginPackageName = `@openclawdsolana/plugin-${packageName.replace('plugin-', '')}`;
      envRequirements = await extractPluginEnvRequirements(openclawdPluginPackageName, cwd);
    }
  }
  ...
  if (Object.keys(envRequirements).length === 0) {
    ...
    const possiblePackages = [
      packageName,
      `@openclawdsolana/${packageName.replace('plugin-', '')}`,
      `@openclawdsolana/plugin-${packageName.replace('plugin-', '')}`,
    ];
