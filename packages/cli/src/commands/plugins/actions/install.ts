...
export async function installPluginFromRegistry(
  ...
  if (!cachedRegistry || !cachedRegistry.registry) {
    logger.error(
      ...
      'Plugin registry cache not found. Please run "openclawd plugins update" first'
    );
    process.exit(1);
  }
...
export async function addPlugin(pluginArg: string, opts: AddPluginOptions): Promise<void> {
  ...
  if (!pluginArg || !pluginArg.trim()) {
    ...
    logger.info(
      { src: 'cli', command: 'plugins-install' },
      'Please provide a valid plugin name (e.g., "openai", "plugin-anthropic", "@openclawdsolana/plugin-sql")'
    );
    process.exit(1);
  }
  ...
  if (!directoryInfo || !directoryInfo.hasPackageJson) {
    logger.error(
      ...
      'Command must be run inside an OpenClawd project directory'
    );
    process.exit(1);
  }
