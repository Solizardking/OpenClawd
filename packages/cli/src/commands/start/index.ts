...
import { AgentServer, loadCharacterTryPath } from '@openclawdsolana/server';
...
export const start = new Command()
  .name('start')
  .description('Build and start the OpenClawd agent server')
  ...
  .action(async (options: StartOptions & { character?: string[] }) => {
    try {
      ...
      const isMonorepo = dirInfo.type === 'openclawd-monorepo';

      if (!isMonorepo && !process.env.OPENCLAWD_TEST_MODE) {
        try {
      ...
      } else if (dirInfo.hasPackageJson && dirInfo.type !== 'non-openclawd-dir') {
        try {
