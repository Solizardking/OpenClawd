...
const OPENCLAWD_API_KEY_ENV = 'OPENCLAWD_API_KEY';

/**
 * Validates an OpenClawd Cloud API key format
 * @param key The API key to validate
 * @returns True if the key appears valid (starts with 'openclawd_' and has sufficient length)
 */
function isValidOpenClawdCloudKey(key: string): boolean {
  if (!key || typeof key !== 'string') return false;
  // OpenClawd Cloud keys start with 'openclawd_' and should have reasonable length
...
export async function handleLogin(options: LoginOptions): Promise<void> {
  console.log(colors.bold('\n🔐 OpenClawd Cloud Authentication\n'));
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Could not connect to OpenClawd Cloud at ${cloudUrl}. Please check the URL and try again.`
      );
    }
    ...
    if (isNaN(timeoutSeconds) || timeoutSeconds <= 0) {
      ...
      console.log(colors.dim('Example: openclawd login --timeout 600\n'));
      process.exit(1);
    }
    ...
    if (!isValidOpenClawdCloudKey(authResult.apiKey)) {
      spinner.fail('Invalid API key format received');
      console.log(
        colors.yellow(
          '\n⚠️  The API key received does not match the expected format (openclawd_xxxxx).'
        )
      );
      console.log(colors.yellow('Please try again or contact support.\n'));
      throw new Error('Invalid API key format: expected openclawd_xxxxx');
    }
...
async function writeApiKeyToEnv(apiKey: string, envFilePath?: string): Promise<void> {
  ...
  try {
    ...
    envVars[OPENCLAWD_API_KEY_ENV] = apiKey;
    ...
    console.log(colors.dim(`${OPENCLAWD_API_KEY_ENV}=${apiKey}\n`));
    throw error;
  }
}
...
function displaySuccessMessage(authResult: SessionStatusResponse): void {
  console.log(colors.green('\n✨ You are now authenticated with OpenClawd Cloud!\n'));
  ...
  console.log('  • You can now use OpenClawd Cloud features');
