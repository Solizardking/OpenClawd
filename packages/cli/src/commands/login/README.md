# OpenClawd Cloud Login Command

The `openclawd login` command provides secure authentication with OpenClawd Cloud, generating an API key that enables CLI users to access cloud-hosted features and services.
...
2. Opens browser to OpenClawd Cloud authentication page
...
openclawd login --cloud-url http://localhost:3000

# Don't open browser (manual authentication)
openclawd login --no-browser

# Increase timeout for slow connections
openclawd login --timeout 600
```

## Options

- `--cloud-url <url>` - URL of OpenClawd Cloud (default: `$OPENCLAWD_CLOUD_URL` or `https://www.openclawdcloud.ai`)
...
```bash
OPENCLAWD_API_KEY=openclawd_<random_hex_string>
```

You can also set a default cloud URL:

```bash
export OPENCLAWD_CLOUD_URL=https://www.openclawdcloud.ai
```
