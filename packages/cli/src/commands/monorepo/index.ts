...
/**
 * Create the monorepo command that clones OpenClawd from a specific branch
 */
export const monorepo = new Command()
  .name('monorepo')
  .description('Clone OpenClawd monorepo from a specific branch, defaults to develop')
  .option('-b, --branch <branch>', 'Branch to install', 'develop')
  .option('-d, --dir <directory>', 'Destination directory', './openclawd')
  .action(async (options: MonorepoOptions) => {
    try {
      const repo = 'OpenClawd/openclawd';
      const branch = options.branch || 'develop';
      const dir = options.dir || './openclawd';
