...
import { logger, type IAgentRuntime, type ProjectAgent } from '@openclawdsolana/core';
import { getDefaultCharacter } from '@/src/characters/openclawd';
import { AgentServer } from '@openclawdsolana/server';
...
export async function runE2eTests(
  ...
  if (!options.skipBuild) {
    try {
      ...
      const isPlugin = projectInfo.type === 'openclawd-plugin';
  ...
  try {
    ...
    const openclawdDir = path.join(process.cwd(), '.openclawd');
    // Create unique database directory for each test run to avoid conflicts
    const packageName = path.basename(process.cwd());
    const timestamp = Date.now();
    const uniqueDbDir = path.join(process.cwd(), '.openclawddb-test', `${packageName}-${timestamp}`);
    const openclawdDbDir = uniqueDbDir;
    const envInfo = await UserEnvironment.getInstanceInfo();
    const envFilePath = envInfo.paths.envFilePath;

    console.info('Setting up environment...');
    console.info(`OpenClawd directory: ${openclawdDir}`);
    console.info(`Database directory: ${openclawdDbDir}`);
    ...
    if (fs.existsSync(openclawdDbDir)) {
      console.info(`Cleaning up existing database directory: ${openclawdDbDir}`);
      try {
        fs.rmSync(openclawdDbDir, { recursive: true, force: true });
    ...
    console.info(`Creating fresh database directory: ${openclawdDbDir}`);
    fs.mkdirSync(openclawdDbDir, { recursive: true });
    console.info(`Created database directory: ${openclawdDbDir}`);

    // Set the database directory in environment variables to ensure it's used
    process.env.PGLITE_DATA_DIR = openclawdDbDir;
    console.info(`Set PGLITE_DATA_DIR to: ${openclawdDbDir}`);
    ...
    try {
      ...
      // For plugins, it's OK to have no agents defined (will use default OpenClawd)
      ...
      if (!project.isPlugin && (!project.agents || project.agents.length === 0)) {
        logger.warn(
          { src: 'cli', command: 'test-e2e' },
          'No agents found in project configuration; falling back to default OpenClawd character for tests'
        );
      }
      ...
      try {
        ...
        // When testing a plugin, import and use the default OpenClawd character
        ...
        if (project.isPlugin || (project.agents?.length || 0) === 0) {
          ...
          process.env.OPENCLAWD_TESTING_PLUGIN = 'true';

          logger.info(
            { src: 'cli', command: 'test-e2e' },
            'Using default OpenClawd character as test agent'
          );
          try {
            ...
            const defaultOpenClawdCharacter = getDefaultCharacter();
            ...
            const startedRuntimes = await server.startAgents(
              [
                {
                  character: defaultOpenClawdCharacter,
        ...
        for (let i = 0; i < runtimes.length; i++) {
          ...
          const results = await testRunner.runTests({
            ...
            skipPlugins: currentDirInfo.type !== 'openclawd-plugin',
            // Only run project tests if we're actually in a project directory
            skipProjectTests: currentDirInfo.type !== 'openclawd-project',
        ...
        // Clean up the OPENCLAWD_TESTING_PLUGIN environment variable
        if (process.env.OPENCLAWD_TESTING_PLUGIN) {
          delete process.env.OPENCLAWD_TESTING_PLUGIN;
        }

        // Clean up database directory after tests complete
        try {
          if (fs.existsSync(openclawdDbDir)) {
            console.info(`Cleaning up test database directory: ${openclawdDbDir}`);
            fs.rmSync(openclawdDbDir, { recursive: true, force: true });
            console.info(`Successfully cleaned up test database directory`);
          }
          // Also clean up the parent test directory if it's empty
          const testDir = path.dirname(openclawdDbDir);
