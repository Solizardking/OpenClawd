...
import { logger, type Plugin } from '@openclawdsolana/core';
...
export async function loadPluginDependencies(projectInfo: DirectoryInfo): Promise<Plugin[]> {
  if (projectInfo.type !== 'openclawd-plugin') {
    return [];
  }
  ...
  if (
    ...
    const projectPluginsPath = path.join(process.cwd(), '.openclawd', 'plugins');
