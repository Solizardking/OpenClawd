...
export async function deleteContainerAction(
  ...
  try {
    const apiKey = options.apiKey || process.env.OPENCLAWD_API_KEY;
    const apiUrl = options.apiUrl || 'https://www.openclawdcloud.ai';

    if (!apiKey) {
      logger.error(
        { src: 'cli', command: 'containers-delete' },
        'API key is required. Use --api-key or set OPENCLAWD_API_KEY environment variable'
      );
      process.exit(1);
    }
    ...
    if (!targetContainerId) {
      ...
      if (!matchingContainer) {
        ...
        logger.info(
          { src: 'cli', command: 'containers-delete' },
          'Run "openclawd containers list" to see all containers'
        );
        process.exit(1);
      }
