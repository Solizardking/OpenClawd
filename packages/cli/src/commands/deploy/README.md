# OpenClawd Deploy Command

Deploy OpenClawd projects to AWS ECS (Elastic Container Service) with a single command.

## Usage

```bash
openclawd deploy [options]
```
...
1. **Set your API key**:
   ...
   ```bash
   export OPENCLAWD_API_KEY="your-api-key-here"
   ```
...
| `-k, --api-key <key>`     | API key                                           | $OPENCLAWD_API_KEY          |
| `-u, --api-url <url>`     | API URL                                           | https://www.openclawdcloud.ai |
...
export OPENCLAWD_DOCKER_PLATFORM=linux/arm64
openclawd deploy
```
...
- `--platform` flag: `openclawd deploy --platform linux/arm64`
- `OPENCLAWD_DOCKER_PLATFORM` environment variable: `export OPENCLAWD_DOCKER_PLATFORM=linux/amd64`
...
3. **Requests ECR credentials** - Gets authentication token and repository from OpenClawd Cloud
...
- OpenClawd Cloud API key
- Valid OpenClawd project with package.json
- Docker installed and running
- Network access to OpenClawd Cloud API
...
- Verify the OpenClawd Cloud API is accessible
...
- [OpenClawd Cloud Dashboard](https://www.openclawdcloud.ai/dashboard/containers)
- [API Documentation](https://www.openclawdcloud.ai/docs/api)
