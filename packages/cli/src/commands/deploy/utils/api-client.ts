/**
 * OpenClawd Cloud API Client
 * Handles communication with the OpenClawd Cloud backend for deployments
 */
...
export class CloudApiClient {
  ...
  async waitForDeployment(
    ...
    return {
      success: false,
      error: `Deployment timeout after ${totalMinutes} minutes. Container may still be deploying - check "openclawd containers list" for current status.`,
    };
  }
...
export function getApiCredentials(): {
  ...
  const apiUrl = process.env.OPENCLAWD_API_URL || 'https://www.openclawdcloud.ai';
