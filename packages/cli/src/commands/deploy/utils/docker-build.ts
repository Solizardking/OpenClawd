...
export interface DockerBuildOptions {
  ...
  // Can also be set via OPENCLAWD_DOCKER_PLATFORM environment variable
  platform?: string;
}
...
export async function buildDockerImage(options: DockerBuildOptions): Promise<DockerBuildResult> {
  try {
    ...
    // 2. OPENCLAWD_DOCKER_PLATFORM environment variable
    // 3. Host platform (auto-detected)
    const hostPlatform = detectHostPlatform();
    const platform = options.platform || process.env.OPENCLAWD_DOCKER_PLATFORM || hostPlatform;
    ...
    if (platform !== hostPlatform) {
      ...
      logger.info(
        ...
        'Tip: Set OPENCLAWD_DOCKER_PLATFORM to use native platform'
      );
    }
