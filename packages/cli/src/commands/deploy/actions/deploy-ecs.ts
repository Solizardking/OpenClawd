 ...
 * Deploys OpenClawd projects using Docker containers to AWS ECS
 */
...
export async function deployWithECS(options: DeployOptions): Promise<DeploymentResult> {
  try {
    ...
    logger.info(
      ...
      'Starting OpenClawd deployment (Docker + AWS ECS)'
    );
    ...
    if (!credentials && !options.apiKey) {
      return {
        success: false,
        error: 'No API key found. Set OPENCLAWD_API_KEY environment variable or use --api-key flag.',
      };
    }
    ...
    const imageBuildResponse = await apiClient.requestImageBuild({
      ...
      metadata: {
        openclawdVersion: packageJson.dependencies?.['@openclawdsolana/core'] || 'unknown',
    ...
    const containerConfig: ContainerConfig = {
      ...
      description: packageJson.description || `OpenClawd project: ${containerName}`,
    ...
    logger.info(
      {
        src: 'cli',
        command: 'deploy-ecs',
        url: `https://www.openclawdcloud.ai/dashboard/containers/${containerId}`,
      },
      'Track deployment'
    );
    ...
    if (!deploymentResponse.success) {
      ...
      logger.error(
        { src: 'cli', command: 'deploy-ecs' },
        '1. Check container status: openclawd containers list'
      );
      logger.error(
        { src: 'cli', command: 'deploy-ecs' },
        '2. View container logs: openclawd containers logs'
      );
      logger.error(
        { src: 'cli', command: 'deploy-ecs' },
...
async function getDetectedPlatform(platformOverride?: string): Promise<string> {
  ...
  if (process.env.OPENCLAWD_DOCKER_PLATFORM) {
    return process.env.OPENCLAWD_DOCKER_PLATFORM;
  }
