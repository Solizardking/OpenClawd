import { buildCharacterPlugins, type Character } from '@openclawdsolana/core';

/**
 * Base OpenClawd character configuration
 * Contains personality, style, and conversation templates
 */
const baseOpenClawdCharacter: Character = {
  name: 'OpenClawd',
...
/**
 * Get the OpenClawd character with plugins configured based on environment variables.
 * Uses buildCharacterPlugins from @openclawdsolana/core to determine which plugins to load.
 *
 * @param env - Environment object to check for API keys (defaults to process.env)
 * @returns The OpenClawd character with appropriate plugins for the current environment
 */
export function getDefaultCharacter(
  env: Record<string, string | undefined> = process.env
): Character {
  return {
    ...baseOpenClawdCharacter,
