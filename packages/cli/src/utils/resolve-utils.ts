 ...
 * 4. `./.openclawd/.openclawddb` relative to the current working directory.
 *
...
export async function resolvePgliteDir(
  ...
  // The fallbackDir passed from getOpenClawdDirectories will be monorepoRoot + '.openclawddb' or similar.
  // If fallbackDir is not provided (e.g. direct call to resolvePgliteDir),
  // then we construct the default path using projectRoot.
  const defaultBaseDir = path.join(projectRoot, '.openclawd', '.openclawddb');

  const base = dir ?? process.env.PGLITE_DATA_DIR ?? fallbackDir ?? defaultBaseDir;

  // Resolve and migrate legacy default (<projectRoot>/.openclawddb) if detected
  const resolved = expandTildePath(base, projectRoot);
  const legacyPath = path.join(projectRoot, '.openclawddb');
  if (resolved === legacyPath) {
    const newPath = path.join(projectRoot, '.openclawd', '.openclawddb');
