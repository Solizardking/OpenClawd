...
export class TestRunner {
  ...
  constructor(runtime: IAgentRuntime, projectAgent?: ProjectAgent) {
    ...
    // 4. If OPENCLAWD_TESTING_PLUGIN environment variable is set
    ...
    const testingPlugin = process.env.OPENCLAWD_TESTING_PLUGIN === 'true';
    ...
    if (testingPlugin && projectAgent?.plugins) {
      ...
      const corePlugins = ['@openclawdsolana/plugin-sql', '@openclawdsolana/plugin-bootstrap', 'bootstrap'];
