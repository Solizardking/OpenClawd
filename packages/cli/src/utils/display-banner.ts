...
export function showUpdateNotification(currentVersion: string, latestVersion: string) {
  ...
  console.log(
    `${blue}│${orange} Run ${green}${bold}bun i -g @openclawdsolana/cli@latest${reset}${orange} to get the latest features${' '.repeat(width - 2 - ` Run bun i -g @openclawdsolana/cli@latest to get the latest features`.length)}${blue}│${reset}`
  );
  console.log(border);
  console.log('');
}
