...
describe('dependency-manager integration', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = await fs.promises.mkdtemp(path.join(tmpdir(), 'openclawd-dep-test-'));
  ...
  describe('real file system operations', () => {
    it('should correctly detect existing @openclawdsolana/cli in dependencies', async () => {
      const packageJson = {
        name: 'test-project',
        dependencies: {
      ...
      const result = hasOpenClawdCli(packageJsonPath);
      expect(result).toBe(true);
    });

    it('should correctly detect existing @openclawdsolana/cli in devDependencies', async () => {
      const packageJson = {
        name: 'test-project',
        devDependencies: {
          '@openclawdsolana/cli': '^1.0.0',
    ...
    it('should correctly detect missing @openclawdsolana/cli', async () => {
      const packageJson = {
        name: 'test-project',
        dependencies: {
          express: '^4.0.0',
    ...
    it('should handle missing package.json gracefully', () => {
      ...
      const result = hasOpenClawdCli(nonExistentPath);
      expect(result).toBe(false);
    });
  ...
  describe('package.json creation', () => {
    ...
    it('should handle directory names with only special characters', async () => {
      ...
      const packageJsonPath = path.join(projectDir, 'package.json');
      const packageJson = JSON.parse(await fs.promises.readFile(packageJsonPath, 'utf8'));
      expect(packageJson.name).toBe('openclawd-project'); // Should fallback to default
    });
  });

  describe('environment variable interactions', () => {
    it('should respect OPENCLAWD_NO_AUTO_INSTALL flag', () => {
  ...
  describe('installation scenarios', () => {
    it('should simulate successful installation', async () => {
      ...
      const result = await installOpenClawdCli(testDir);
      expect(result).toBe(true);
  ...
  describe('simplified end-to-end functionality', () => {
    it('should handle ensureOpenClawdCli without errors', async () => {
      ...
      await expect(ensureOpenClawdCli(testDir)).resolves.toBeUndefined();
    });
    ...
    it('should verify CLI detection works with real files', async () => {
      ...
      const packageJsonPath = path.join(testDir, 'package.json');
      const hasCliResult = hasOpenClawdCli(packageJsonPath);
  ...
  describe('error recovery', () => {
    it('should continue gracefully when file operations fail', async () => {
      ...
      const result = hasOpenClawdCli(path.join(testDir, 'nonexistent', 'package.json'));
      expect(result).toBe(false);
    });
