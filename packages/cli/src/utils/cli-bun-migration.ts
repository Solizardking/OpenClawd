...
export async function isCliInstalledViaNpm(): Promise<boolean> {
  try {
    const { stdout } = await bunExecSimple('npm', ['list', '-g', '@openclawdsolana/cli', '--depth=0']);
    return stdout.includes('@openclawdsolana/cli');
  } catch (error) {
    // Also check if the current openclawd command points to npm installation
    try {
      const { stdout: whichOutput } = await bunExecSimple('which', ['openclawd']);
...
async function removeNpmInstallation(): Promise<void> {
  logger.info({ src: 'cli', util: 'bun-migration' }, 'Removing npm installation of @openclawdsolana/cli');
  await bunExecInherit('npm', ['uninstall', '-g', '@openclawdsolana/cli']);
}
...
async function installCliWithBun(version: string): Promise<void> {
  ...
  await bunExecInherit('bun', ['add', '-g', `@openclawdsolana/cli@${version}`]);
}
...
async function verifyCliInstallation(expectedVersion: string): Promise<boolean> {
  try {
    const { stdout } = await bunExecSimple('openclawd', ['-v']);
...
export async function migrateCliToBun(targetVersion: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await bunExec('bun', ['remove', '-g', '@openclawdsolana/cli'], { stdio: 'ignore' });
    } catch {
