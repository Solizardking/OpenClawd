...
import {
  autoInstallBun,
  shouldAutoInstall,
} from './auto-install-bun.js';
import { bunExecSimple } from './bun-exec.js';
import { resolveEnvFile } from './resolve-utils.js';
...
export class UserEnvironment {
  ...
  /**
   * Detects the active package manager - always returns bun for OpenClawd CLI
   */
  private async getPackageManagerInfo(): Promise<PackageManagerInfo> {
    logger.debug(
      { src: 'cli', util: 'user-environment' },
      'Using bun as the package manager for OpenClawd CLI'
    );
    ...
    if (typeof Bun !== 'undefined' && Bun.version) {
      ...
      try {
        ...
        if (!version) {
          ...
          logger.error(
            { src: 'cli', util: 'user-environment' },
            'Bun is required for OpenClawd CLI but is not installed or not found in PATH'
          );
          logger.error(
            { src: 'cli', util: 'user-environment' },
          ...
          // Force exit the process - Bun is required for OpenClawd CLI
          logger.error(
            { src: 'cli', util: 'user-environment' },
    ...
    const packageName = '@openclawdsolana/cli';
      ...
      if (existsSync(corePackagePath)) {
        try {
          ...
          if (stats.isDirectory()) {
            // Additional validation: check if this looks like the OpenClawd monorepo
            const packageJsonPath = path.join(currentDir, 'package.json');
            if (existsSync(packageJsonPath)) {
              const packageJsonContent = readFileSync(packageJsonPath, 'utf8');
              const packageJson = JSON.parse(packageJsonContent);
              // Verify this is actually the OpenClawd monorepo
              if (packageJson.name?.includes('openclawd') || packageJson.workspaces) {
                return currentDir;
              }
            }
          }
  ...
  public async getPathInfo(): Promise<PathInfo> {
    ...
    const openclawdDir = path.join(projectRootForPaths, '.openclawd');
    ...
    return {
      openclawdDir,
      envFilePath,
      configPath: path.join(openclawdDir, 'config.json'),
      pluginsDir: path.join(openclawdDir, 'plugins'),
      monorepoRoot,
    ...
    try {
      ...
      if (monorepoRoot) {
        const monoRepoPackagePath = path.join(
          monorepoRoot,
          'packages',
          packageName.replace('@openclawdsolana/', ''),
          'package.json'
        );
  ...
  public async getLocalPackages(): Promise<string[]> {
    ...
    try {
      ...
      const pluginPackages = packagesDirEntries
        ...
        .map((entry) => `@openclawdsolana/${entry.name}`);
