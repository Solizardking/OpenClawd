...
function getCliDirectory(): string | null {
  try {
    ...
    if (cliPath.includes('node_modules/@openclawdsolana/cli')) {
      // Go up to the CLI package root
      const cliDir = path.dirname(
        cliPath.split('node_modules/@openclawdsolana/cli')[0] + 'node_modules/@openclawdsolana/cli'
      );
...
async function attemptInstallation(
  ...
  try {
    ...
    if (skipVerification || process.env.OPENCLAWD_SKIP_PLUGIN_VERIFY) {
      logger.info(
        {
          src: 'cli',
          util: 'install-plugin',
  ...
  if (info.npm?.repo) {
    ...
    if (result.success) {
      ...
      if (
        ...
        !skipVerification &&
        !process.env.OPENCLAWD_SKIP_PLUGIN_VERIFY
      ) {
