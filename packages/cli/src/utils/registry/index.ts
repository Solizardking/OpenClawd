...
const OPENCLAWD_DIR = path.join(process.cwd(), '.openclawd');
const REGISTRY_SETTINGS_FILE = path.join(OPENCLAWD_DIR, 'registrysettings.json');
// Use resolveEnvFile to match how credentials are saved, with fallback to ~/.openclawd/.env
const ENV_FILE = resolveEnvFile() || path.join(OPENCLAWD_DIR, '.env');
const REGISTRY_CACHE_FILE = path.join(OPENCLAWD_DIR, 'registry-cache.json');
...
export async function ensureOpenClawdDir() {
  await fs.mkdir(OPENCLAWD_DIR, { recursive: true });
}

export async function getRegistrySettings(): Promise<RegistrySettings> {
  await ensureOpenClawdDir();
...
export async function getGitHubToken(): Promise<string | undefined> {
  try {
    ...
    const globalEnvPath = path.join(OPENCLAWD_DIR, '.env');
...
export async function setGitHubToken(token: string) {
  ...
  try {
    ...
    try {
      ...
      envContent = '# OpenClawd environment variables\n\n';
    }
...
function normalizePackageName(packageName: string): string {
  ...
  } else if (packageName.startsWith('@openclawdsolana/')) {
    return packageName.replace(/^@openclawd\//, '');
  }
  return packageName;
}
...
const DEFAULT_REGISTRY: Record<string, string> = {
  '@openclawdsolana/plugin-anthropic': 'github:openclawd-plugins/plugin-anthropic',
  '@openclawdsolana/plugin-bootstrap': 'github:openclawd-plugins/plugin-bootstrap',
  '@openclawdsolana/plugin-browser': 'github:openclawd-plugins/plugin-browser',
  '@openclawdsolana/plugin-discord': 'github:openclawd-plugins/plugin-discord',
  '@openclawdsolana/plugin-elevenlabs': 'github:openclawd-plugins/plugin-elevenlabs',
  '@openclawdsolana/plugin-evm': 'github:openclawd-plugins/plugin-evm',
  '@openclawdsolana/plugin-farcaster': 'github:openclawd-plugins/plugin-farcaster',
  '@openclawdsolana/plugin-groq': 'github:openclawd-plugins/plugin-groq',
  '@openclawdsolana/plugin-mcp': 'github:openclawd-plugins/plugin-mcp',
  '@openclawdsolana/plugin-messari-ai-toolkit': 'github:messari/plugin-messari-ai-toolkit',
  '@openclawdsolana/plugin-morpheus': 'github:bowtiedbluefin/plugin-morpheus',
  '@openclawdsolana/plugin-node': 'github:openclawd-plugins/plugin-node',
  '@openclawdsolana/plugin-ollama': 'github:openclawd-plugins/plugin-ollama',
  '@openclawdsolana/plugin-openai': 'github:openclawd-plugins/plugin-openai',
  '@openclawdsolana/plugin-pdf': 'github:openclawd-plugins/plugin-pdf',
  '@openclawdsolana/plugin-redpill': 'github:openclawd-plugins/plugin-redpill',
  '@openclawdsolana/plugin-solana': 'github:openclawd-plugins/plugin-solana',
  '@openclawdsolana/plugin-sql': 'github:openclawd-plugins/plugin-sql',
  '@openclawdsolana/plugin-storage-s3': 'github:openclawd-plugins/plugin-storage-s3',
  '@openclawdsolana/plugin-tee': 'github:openclawd-plugins/plugin-tee',
  '@openclawdsolana/plugin-telegram': 'github:openclawd-plugins/plugin-telegram',
  '@openclawdsolana/plugin-twitter': 'github:openclawd-plugins/plugin-twitter',
  '@openclawdsolana/plugin-venice': 'github:openclawd-plugins/plugin-venice',
  '@openclawdsolana/plugin-video-understanding': 'github:openclawd-plugins/plugin-video-understanding',
};
...
export function normalizePluginName(pluginName: string): string[] {
  ...
  if (pluginName.includes('/')) {
    // Handle formats like "openclawd/plugin-ton" or "openclawd-plugins/plugin-ton"
    const parts = pluginName.split('/');
    baseName = parts[parts.length - 1];
  } else if (pluginName.startsWith('@')) {
    // Handle scoped package format like "@openclawdsolana/plugin-ton"
    const parts = pluginName.split('/');
    if (parts.length > 1) {
      baseName = parts[1];
    }
  ...
  return [
    ...
    `@openclawdsolana/${baseName}`, // Scoped with openclawd
    `@openclawdsolana/plugin-${baseName}`, // Scoped with openclawd and plugin prefix
  ];
}
...
export async function validateDataDir(): Promise<boolean> {
  ...
  if (!status.exists) {
    logger.warn({ src: 'cli', util: 'registry' }, 'OpenClawd data directory not found. Initializing');
