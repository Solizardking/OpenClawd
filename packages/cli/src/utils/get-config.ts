 ...
 */
const SAMPLE_ENV_TEMPLATE = `### OpenClawd Environment Variables ###
...
## MODEL PROVIDER KEYS ##
## OpenClawd is compatible with a wide array of model providers. Many have OpenAI compatible APIs,
...
### DATABASE ###
# By default, OpenClawd will use a local pglite instance
...
/**
 * Retrieves the standard directory paths used by OpenClawd for configuration and database storage.
 *
 * @returns An object containing the OpenClawd configuration directory, the OpenClawd database directory for the current project, and the path to the OpenClawd `.env` file.
 */
export async function getOpenClawdDirectories(targetProjectDir?: string) {
  const userEnv = UserEnvironment.getInstance();
  const paths = await userEnv.getPathInfo();

  const projectRoot = targetProjectDir || paths.monorepoRoot || process.cwd();
  const openclawdDir = targetProjectDir ? path.resolve(targetProjectDir, '.openclawd') : paths.openclawdDir;
  ...
  logger.debug(
    ...
    'OpenClawd directories'
  );

  const defaultOpenClawdDbDir = path.resolve(projectRoot, '.openclawd', '.openclawddb');
  // Pass targetProjectDir to resolvePgliteDir to ensure it uses the correct base directory
  const openclawdDbDir = await resolvePgliteDir(undefined, defaultOpenClawdDbDir, targetProjectDir);

  return { openclawdDir, openclawdDbDir, envFilePath };
}
...
/**
 * Ensures the OpenClawd configuration directory exists and returns standard OpenClawd directory paths.
 *
 * @returns An object containing paths for the OpenClawd configuration directory, the OpenClawd database directory, and the `.env` file.
 */
export async function ensureOpenClawdDir(targetProjectDir?: string) {
  const dirs = await getOpenClawdDirectories(targetProjectDir);
  await ensureDir(dirs.openclawdDir);

  // Also create registry-cache.json and config.json files if they don't exist
  const registryCachePath = path.join(dirs.openclawdDir, 'registry-cache.json');
  const configPath = path.join(dirs.openclawdDir, 'config.json');
 ...
 * @param openclawdDbDir The directory for PGLite database
 * @param envFilePath Path to the .env file
 */
export async function setupPgLite(
  ...
  const dirs = await ensureOpenClawdDir(targetProjectDir);
  const { openclawdDbDir, envFilePath } = dirs;

  // Use provided parameters or defaults from dirs
  const targetDbDir = dbDir || openclawdDbDir;
...
/**
 * Validates an OpenClawd Cloud API key format
 * @param key The API key to validate
 * @returns True if the key appears valid
 */
export function isValidOpenClawdCloudKey(key: string): boolean {
  if (!key || typeof key !== 'string') return false;
  // OpenClawd Cloud keys start with 'openclawd_'
  return key.startsWith('openclawd_') && key.length > 10;
}

/**
 * Stores OpenClawd Cloud API key in the .env file
 * @param key The OpenClawd Cloud API key to store
 * @param envFilePath Path to the .env file
 */
export async function storeOpenClawdCloudKey(key: string, envFilePath: string): Promise<void> {
  if (!key) return;
  ...
  try {
    ...
    // Remove existing OPENCLAWD_API_KEY line if present
    const lines = content.split('\n').filter((line) => !line.startsWith('OPENCLAWD_API_KEY='));
    lines.push(`OPENCLAWD_API_KEY=${key}`);

    await fs.writeFile(envFilePath, lines.join('\n'), 'utf8');
    process.env.OPENCLAWD_API_KEY = key;

    logger.success(
      { src: 'cli', util: 'get-config' },
      'OpenClawd Cloud API key saved to configuration'
    );
  } catch (error) {
    logger.error(
      {
      ...
      'Error saving OpenClawd Cloud API key'
    );
    throw error;
  }
}
...
/**
 * Checks if the user already has a valid OpenClawd Cloud API key
 * @param envFilePath Path to the .env file
 * @returns True if a valid API key exists
 */
export async function hasExistingOpenClawdCloudKey(envFilePath: string): Promise<boolean> {
  // Check environment variable first
  if (process.env.OPENCLAWD_API_KEY && isValidOpenClawdCloudKey(process.env.OPENCLAWD_API_KEY)) {
    return true;
  }
  ...
  if (existsSync(envFilePath)) {
    try {
      const content = await fs.readFile(envFilePath, 'utf8');
      const match = content.match(/^OPENCLAWD_API_KEY=(.+)$/m);
      if (match && match[1] && isValidOpenClawdCloudKey(match[1].trim())) {
        return true;
      }
    } catch {
      // Ignore read errors
...
/**
 * Prompts the user for an OpenClawd Cloud API key or to login, validates it, and stores it
 * @param envFilePath Path to the .env file
 * @returns The configured OpenClawd Cloud API key or null if user cancels
 */
export async function promptAndStoreOpenClawdCloudKey(envFilePath: string): Promise<string | null> {
  // Check if user already has a valid API key
  if (await hasExistingOpenClawdCloudKey(envFilePath)) {
    const existingKey = process.env.OPENCLAWD_API_KEY;
    clack.log.success('Found existing OpenClawd Cloud API key');
    return existingKey || null;
  }

  clack.intro('☁️  OpenClawd Cloud Configuration');

  clack.note(
    `OpenClawd Cloud provides multi-model AI access (GPT-4o, Claude, Gemini & more) with a single API key.\n\n` +
      `Get your API key:\n` +
      `  1. Run: ${colors.cyan('openclawd login')} to authenticate with your browser\n` +
      `  2. Or visit: ${colors.blue('https://www.openclawdcloud.ai/dashboard/api-keys')}\n\n` +
      `Your API key format: ${colors.dim('openclawd_xxxxx')}`,
    'Setup Options'
  );
  ...
  const authChoice = await clack.select({
    ...
    options: [
      ...
      },
      {
        label: 'Skip for now',
        value: 'skip',
        hint: 'You can run openclawd login later',
      },
    ],
    initialValue: 'login',
  });
  ...
  if (authChoice === 'skip') {
    clack.log.warn(
      'Skipping OpenClawd Cloud configuration. Run "openclawd login" later to authenticate.'
    );
    ...
    if (!content.includes('OPENCLAWD_API_KEY=')) {
      if (content && !content.endsWith('\n')) {
        content += '\n';
      }
      content += '\n# OpenClawd Cloud Configuration\n';
      content += '# Get your API key by running: openclawd login\n';
      content += '# Or visit: https://www.openclawdcloud.ai/dashboard/api-keys\n';
      content += 'OPENCLAWD_API_KEY=\n';
      await fs.writeFile(envFilePath, content, 'utf8');
    }
    clack.outro('Run "openclawd login" to complete authentication');
    return null;
  }
  ...
  if (authChoice === 'login') {
    ...
    const cloudUrl = process.env.OPENCLAWD_CLOUD_URL || 'https://www.openclawdcloud.ai';
    ...
    try {
      ...
      return process.env.OPENCLAWD_API_KEY || null;
    } catch (error) {
      clack.log.error('Login failed. You can try again later with "openclawd login"');
      return null;
    }
  }
  ...
  const config: ProviderPromptConfig = {
    name: 'OpenClawd Cloud',
    icon: '☁️',
    noteText: 'Get your API key from: https://www.openclawdcloud.ai/dashboard/api-keys',
    inputs: [
      {
        key: 'key',
        message: 'Enter your OpenClawd Cloud API key:',
        type: 'password',
        placeholder: 'openclawd_xxxxx',
        validate: (value) => {
          if (value.trim() === '') return 'OpenClawd Cloud API key cannot be empty';
          return undefined;
        },
      },
    ],
    storeFunction: async (results, envPath) => {
      const isValid = isValidOpenClawdCloudKey(results.key);
      if (!isValid) {
        clack.log.warn('Invalid API key format detected. Expected format: openclawd_...');
        clack.log.warn('The key has been saved but may not work correctly.');
      }
      await storeOpenClawdCloudKey(results.key, envPath);
    },
    successMessage: 'OpenClawd Cloud integration configured',
  };
...
export async function configureDatabaseSettings(reconfigure = false): Promise<string | null> {
  ...
  const { openclawdDbDir, envFilePath } = await ensureOpenClawdDir();
  ...
  const pgliteDataDir = await resolvePgliteDir(undefined, openclawdDbDir);
  ...
  await setupPgLite(openclawdDbDir, envFilePath);
  return null;
}
