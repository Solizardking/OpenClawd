...
const importStrategies: ImportStrategy[] = [
  ...
  // Try workspace dependencies (for monorepo packages)
  {
    ...
    tryImport: async (repository: string) => {
      if (repository.startsWith('@openclawdsolana/plugin-')) {
        // Try to find the plugin in the workspace
        const pluginName = repository.replace('@openclawdsolana/', '');
...
/**
 * Determines if a package name is from the OpenClawd ecosystem
 */
function isOpenClawdPackageName(repository: string): boolean {
  return repository.startsWith('@openclawdsolana/') || repository.startsWith('@openclawd-plugins/');
}
...
function getStrategiesForPlugin(repository: string): ImportStrategy[] {
  const isOpenClawd = isOpenClawdPackageName(repository);

  if (isOpenClawd) {
    // OpenClawd ecosystem plugins: try all strategies
 ...
 * OpenClawd ecosystem plugins (@openclawdsolana/*) use all strategies,
...
export async function loadPluginModule(
  ...
  logger.debug(
    { src: 'cli', util: 'load-plugin', repository, isOpenClawd, strategyCount: strategies.length },
    'Loading plugin'
  );
