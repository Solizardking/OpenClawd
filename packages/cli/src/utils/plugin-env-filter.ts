 ...
 * This eliminates duplicate file reads that occurred when isOpenClawdPlugin and
...
function parsePackageJson(packageJsonPath: string): ParsedPackageJson | null {
  try {
    ...
    const isPlugin = Boolean(
      ...
      pkg.openclawd?.type === 'plugin' ||
      pkg.name?.startsWith('@openclawdsolana/plugin-') ||
      pkg.keywords?.includes('openclawd-plugin')
    );
...
export function warnAboutMissingDeclarations(
  ...
  const officialPluginsMissing = pluginsWithoutDeclarations.filter((p) =>
    p.startsWith('@openclawdsolana/plugin-')
  );
  if (officialPluginsMissing.length === 0) return false;

  const message = `${officialPluginsMissing.length} OpenClawd plugins missing env var declarations: ${officialPluginsMissing.join(', ')}`;
