 ...
 *
 * This utility checks for a local @openclawdsolana/cli installation and delegates to it
...
async function delegateToLocalCli(localCliPath: string): Promise<void> {
  logger.info(
    ...
    'Using local @openclawdsolana/cli installation'
  );
...
export async function tryDelegateToLocalCli(): Promise<boolean> {
  try {
    ...
    if (process.env._OPENCLAWD_CLI_DELEGATION_DEPTH) {
      const depth = parseInt(process.env._OPENCLAWD_CLI_DELEGATION_DEPTH, 10);
      if (depth > 0) {
        logger.debug(
          { src: 'cli', util: 'local-cli-delegation' },
    ...
    process.env._OPENCLAWD_CLI_DELEGATION_DEPTH = '1';
