...
/**
 * Dependency management utilities for OpenClawd CLI
 */
...
/**
 * Check if @openclawdsolana/cli is present in package.json dependencies or devDependencies
 */
export function hasOpenClawdCli(packageJsonPath: string): boolean {
  try {
    ...
    return '@openclawdsolana/cli' in dependencies || '@openclawdsolana/cli' in devDependencies;
  } catch (error) {
    logger.debug(
...
export function shouldAutoInstallCli(cwd: string = process.cwd()): boolean {
  ...
  if (dirInfo.type === 'openclawd-monorepo' || dirInfo.monorepoRoot) {
  ...
  // Check if @openclawdsolana/cli is already present
  const packageJsonPath = path.join(cwd, 'package.json');
  if (hasOpenClawdCli(packageJsonPath)) {
    logger.debug({ src: 'cli', util: 'dependency-manager' }, '@openclawdsolana/cli already present');
    return false;
  }
...
/**
 * Install @openclawdsolana/cli as a dev dependency using bun
 */
export async function installOpenClawdCli(cwd: string = process.cwd()): Promise<boolean> {
  try {
    logger.info(
      { src: 'cli', util: 'dependency-manager' },
      'Adding @openclawdsolana/cli as dev dependency'
    );

    const result = await runBunWithSpinner(['add', '--dev', '@openclawdsolana/cli'], cwd, {
      spinnerText: 'Installing @openclawdsolana/cli with bun...',
      successText: '✓ @openclawdsolana/cli installed successfully',
      errorText: 'Failed to install @openclawdsolana/cli',
    ...
    if (result.success) {
      logger.info(
        { src: 'cli', util: 'dependency-manager' },
        '@openclawdsolana/cli added as dev dependency'
      );
      return true;
    } else {
      logger.warn(
        { src: 'cli', util: 'dependency-manager' },
        'Failed to install @openclawdsolana/cli (optional)'
      );
      logger.debug(
...
/**
 * Auto-install @openclawdsolana/cli if conditions are met
 * This is the main function that should be called from start/dev commands
 * Uses bun as the package manager (OpenClawd standard)
 */
export async function ensureOpenClawdCli(cwd: string = process.cwd()): Promise<void> {
  // Quick check if we should proceed
  if (!shouldAutoInstallCli(cwd)) {
    return;
  }

  logger.debug({ src: 'cli', util: 'dependency-manager' }, 'Auto-installing @openclawdsolana/cli');

  // Attempt to install using bun
  const success = await installOpenClawdCli(cwd);
...
/**
 * Get the version of @openclawdsolana/cli that would be installed
 * This is useful for showing the user what version will be added
 */
export async function getLatestOpenClawdCliVersion(): Promise<string | null> {
  try {
    const result: BunExecResult = await bunExec('bun', ['info', '@openclawdsolana/cli', '--json'], {
      stdio: 'pipe',
    });
    ...
    logger.debug(
      ...
      'Error getting @openclawdsolana/cli version'
    );
    return null;
  }
}
...
/**
 * Check if the current directory already has OpenClawd dependencies
 * This helps determine if auto-installing the CLI makes sense
 */
export function hasOpenClawdDependencies(cwd: string = process.cwd()): boolean {
  const dirInfo = detectDirectoryType(cwd);
  return dirInfo.hasOpenClawdDependencies && dirInfo.openclawdPackageCount > 0;
}

/**
 * Create a package.json if it doesn't exist (for standalone usage)
 * This is a fallback for cases where someone wants to use openclawd in a new directory
...
export async function ensurePackageJson(cwd: string = process.cwd()): Promise<boolean> {
  ...
  try {
    ...
    const packageName = sanitizedName || 'openclawd-project';
