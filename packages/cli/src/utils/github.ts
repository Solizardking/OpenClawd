      ...
      if (branchesResponse.status === 200) {
        ...
        if (branches && branches.length > 0) {
          ...
          const blobResponse = await fetch(`${GITHUB_API_URL}/repos/${owner}/${repo}/git/blobs`, {
            ...
            body: JSON.stringify({
              content: '# Repository initialized by OpenClawd CLI',
              encoding: 'utf-8',
            }),
          });
...
2. Give your token a descriptive name (e.g., "OpenClawd CLI")
...
/**
 * Saves the provided GitHub username and token to the `.env` file in the user's `.openclawd` directory.
 *
...
export async function saveGitHubCredentials(username: string, token: string): Promise<void> {
  const envInfo = await UserEnvironment.getInstanceInfo();
  const { openclawdDir, envFilePath } = envInfo.paths;

  logger.debug({ src: 'cli', util: 'github' }, 'Saving credentials');

  // Ensure .openclawd directory exists
  if (!existsSync(openclawdDir)) {
    await fs.mkdir(openclawdDir, { recursive: true });
  }
...
export async function pushToGitHub(
  ...
  try {
    ...
    try {
      ...
      await bunExec('git', ['config', 'user.email', 'plugindev@openclawd.com'], { cwd });
      await bunExec('git', ['config', 'user.name', 'OpenClawd Plugin Dev'], { cwd });
      logger.info({ src: 'cli', util: 'github' }, 'Set git user info for commit');
    }

    // Commit if there are changes
    try {
      await bunExec('git', ['commit', '-m', 'Initial commit from OpenClawd CLI'], { cwd });
