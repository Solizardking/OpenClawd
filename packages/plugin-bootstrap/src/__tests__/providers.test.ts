...
import { IAgentRuntime, Memory, State, UUID, ChannelType, Media } from '@openclawdsolana/core';
...
describe('Choice Provider', () => {
  ...
  it('should handle errors from getTasks gracefully', async () => {
    ...
    // Cannot check logger.error calls without mocking @openclawdsolana/core
  });
});
...
describe('Settings Provider', () => {
  ...
  afterEach(async () => {
    mock.restore();
    // Cannot mock @openclawdsolana/core functions in bun:test
  });
  ...
  it('should retrieve settings in onboarding mode', async () => {
    ...
    // Note: We cannot mock findWorldsForOwner and getWorldSettings from @openclawdsolana/core
  ...
  it('should handle errors gracefully when getWorldSettings fails', async () => {
    // Note: We cannot directly mock getWorldSettings from @openclawdsolana/core in bun:test
