...
// Mock window.OPENCLAWD_CONFIG
Object.defineProperty(global.window, 'OPENCLAWD_CONFIG', {
  value: {
...
describe('TEE Frontend Components', () => {
  ...
  describe('Component Integration', () => {
    test('should have valid OPENCLAWD_CONFIG configuration', () => {
      expect(global.window.OPENCLAWD_CONFIG).toBeDefined();
      expect(global.window.OPENCLAWD_CONFIG?.agentId).toBe('test-agent-123');
      expect(global.window.OPENCLAWD_CONFIG?.apiBase).toBe('http://localhost:3000');
    });

    test('should handle missing agentId gracefully', () => {
      const originalConfig = global.window.OPENCLAWD_CONFIG;
      ...
      expect(global.window.OPENCLAWD_CONFIG?.agentId).toBeUndefined();
