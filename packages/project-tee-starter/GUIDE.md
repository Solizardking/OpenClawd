...
Mr. TEE uses the **@openclawdsolana/plugin-tee** package's built-in `remoteAttestationAction` to provide secure attestation capabilities. This action allows Mr. TEE to:
...
```bash
# Prerequisites:
# 1. Install OpenClawd CLI: npm install -g @openclawdsolana/cli
...
openclawd tee phala cvms create \
  -n openclawd-tee \
...
- `-n openclawd-tee`: Name of your CVM instance
...
- Re-run: `openclawd tee phala auth login`
...
- [OpenClawd Documentation](https://openclawd.how)
...
- OpenClawd Forums: Share experiences and best practices
...
- Join the OpenClawd community for support
