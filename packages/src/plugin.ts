...
function isAutoInstallAllowed(): boolean {
  if (process.env.OPENCLAWD_NO_AUTO_INSTALL === 'true') return false;
  if (process.env.OPENCLAWD_NO_PLUGIN_AUTO_INSTALL === 'true') return false;
  if (process.env.CI === 'true') return false;
  if (process.env.OPENCLAWD_TEST_MODE === 'true') return false;
 ...
 * Examples:
 *  - '@openclawdsolana/plugin-discord' -> 'discord'
 *  - '@openclawdsolana/plugin-sql' -> 'sql'
 *  - 'bootstrap' -> 'bootstrap'
 *  - 'plugin-custom' -> 'plugin-custom'
 */
export function normalizePluginName(pluginName: string): string {
  // Match patterns like @openclawdsolana/plugin-{name} or @{scope}/plugin-{name}
 ...
 *
 * Supports both scoped package names (@openclawdsolana/plugin-discord) and short names (discord)
 */
export function resolvePluginDependencies(
  ...
  // Allows finding plugins by short name ('discord') or scoped name ('@openclawdsolana/plugin-discord')
  ...
  for (const [key, plugin] of availablePlugins.entries()) {
    ...
    if (!plugin.name.startsWith('@')) {
      lookupMap.set(`@openclawdsolana/plugin-${plugin.name}`, plugin);
    }
 ...
 * are specified using different naming conventions (e.g., '@openclawdsolana/plugin-discord' vs 'discord').
 *
