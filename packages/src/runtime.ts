...
export class AgentRuntime implements IAgentRuntime {
  ...
  /**
   * Reference to the OpenClawd instance that created this runtime
   * Set by OpenClawd when runtime is registered
   * @optional
   */
  OpenClawd?: IOpenClawd;
  ...
  async initialize(options?: { skipMigrations?: boolean }): Promise<void> {
    ...
    if (!this.adapter) {
      ...
      throw new Error(
        'Database adapter not initialized. The SQL plugin (@openclawdsolana/plugin-sql) is required for agent initialization. Please ensure it is included in your character configuration.'
      );
    }
  ...
  async runPluginMigrations(): Promise<void> {
    ...
    const forceDestructive = process.env.OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS === 'true';
  ...
  hasOpenClawd(): this is IAgentRuntime & { OpenClawd: IOpenClawd } {
    return this.OpenClawd !== undefined;
  }
}
