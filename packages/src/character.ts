 ...
 * 1. Core plugins (@openclawdsolana/plugin-sql)
...
export function buildCharacterPlugins(
  ...
  const plugins = [
    ...
    ...(env.ANTHROPIC_API_KEY?.trim() ? ['@openclawdsolana/plugin-anthropic'] : []),
    ...(env.OPENROUTER_API_KEY?.trim() ? ['@openclawdsolana/plugin-openrouter'] : []),

    // Embedding-capable plugins (before platform plugins per documented order)
    ...(env.OPENAI_API_KEY?.trim() ? ['@openclawdsolana/plugin-openai'] : []),
    ...(env.GOOGLE_GENERATIVE_AI_API_KEY?.trim() ? ['@openclawdsolana/plugin-google-genai'] : []),

    // Platform plugins
    ...(env.DISCORD_API_TOKEN?.trim() ? ['@openclawdsolana/plugin-discord'] : []),
    ...
    env.TWITTER_ACCESS_TOKEN_SECRET?.trim()
      ...
      : []),
    ...(env.TELEGRAM_BOT_TOKEN?.trim() ? ['@openclawdsolana/plugin-telegram'] : []),

    // Bootstrap plugin
    ...(() => {
      const ignore = env.IGNORE_BOOTSTRAP?.trim().toLowerCase();
      const shouldIgnore = ignore === 'true' || ignore === '1' || ignore === 'yes';
      return shouldIgnore ? [] : ['@openclawdsolana/plugin-bootstrap'];
    })(),
