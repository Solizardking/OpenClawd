 ...
 */
export class OpenClawd extends EventTarget implements IOpenClawd {
   ...
   * const ids = await OpenClawd.addAgents([{ character, plugins }]);
   * await OpenClawd.startAgents(ids);
   *
   * @example
   * // Serverless (ephemeral, with pre-cached DB adapter)
   * const runtimes = await OpenClawd.addAgents([{
   *   character,
   *   plugins,
  ...
  async addAgents(
    ...
    const promises = agents.map(async (agent) => {
      ...
      if (agent.databaseAdapter) {
        resolvedPlugins = resolvedPlugins.filter((p) => p.name !== '@openclawdsolana/plugin-sql');
      }
