...
describe('Plugin Functions', () => {
  ...
  describe('loadPlugin', () => {
    ...
    test('should handle plugin loading errors gracefully', async () => {
      ...
      const result = await loadPlugin('@openclawdsolana/non-existent-plugin');

      expect(result).toBeNull();
    });

    test('should load bootstrap plugin successfully', async () => {
      const result = await loadPlugin('@openclawdsolana/plugin-bootstrap');
  ...
  describe('resolvePlugins', () => {
    ...
    test('should handle scoped plugin names with short name dependencies', async () => {
      ...
      const pluginDiscord: Plugin = {
        name: '@openclawdsolana/plugin-discord',
      ...
      const indexDiscord = resolved.findIndex((p) => p.name === '@openclawdsolana/plugin-discord');
      ...
      expect(resolved.some((p) => p.name === '@openclawdsolana/plugin-discord')).toBe(true);
    ...
    test('should not queue dependency twice when plugin has scoped name', async () => {
      ...
      const discordCount = resolved.filter((p) => p.name === '@openclawdsolana/plugin-discord').length;
      expect(discordCount).toBe(1);
    });
  });

  describe('normalizePluginName', () => {
    test('should extract short name from @openclawd scoped packages', () => {
      expect(normalizePluginName('@openclawdsolana/plugin-discord')).toBe('discord');
      expect(normalizePluginName('@openclawdsolana/plugin-sql')).toBe('sql');
      expect(normalizePluginName('@openclawdsolana/plugin-bootstrap')).toBe('bootstrap');
    });
    ...
    test('should return original name for scoped packages without plugin- prefix', () => {
      expect(normalizePluginName('@openclawdsolana/core')).toBe('@openclawdsolana/core');
  ...
  describe('resolvePluginDependencies with scoped names', () => {
    test('should resolve dependency using scoped package name', () => {
      ...
      const pluginB: Plugin = {
        ...
        dependencies: ['@openclawdsolana/plugin-discord'], // Scoped name
        actions: [],
        services: [],
      };
    ...
    test('should handle mixed scoped and short names', () => {
      ...
      const pluginB: Plugin = {
        name: 'discord',
        description: 'Discord plugin',
        dependencies: ['@openclawdsolana/plugin-sql'], // Scoped name
        actions: [],
        services: [],
      };
    ...
    test('should warn on missing scoped dependency', () => {
      const pluginA: Plugin = {
        name: 'music-player',
        description: 'Music player plugin',
        dependencies: ['@openclawdsolana/plugin-nonexistent'],
        actions: [],
        services: [],
      };
    ...
    test('should not create double-scoped names in lookup map', () => {
      // Plugin with scoped name - this should not create '@openclawdsolana/plugin-@openclawdsolana/plugin-discord'
      const pluginDiscord: Plugin = {
        name: '@openclawdsolana/plugin-discord',
        description: 'Discord plugin',
        actions: [],
      ...
      const pluginMap = new Map<string, Plugin>();
      pluginMap.set('@openclawdsolana/plugin-discord', pluginDiscord);
    ...
    test('should handle complex dependency chains with scoped names', () => {
      ...
      const pluginB: Plugin = {
        name: 'bootstrap',
        description: 'Bootstrap plugin',
        dependencies: ['@openclawdsolana/plugin-sql'],
        actions: [],
        services: [],
      };
      ...
      const pluginD: Plugin = {
        name: 'music-player',
        description: 'Music player plugin',
        dependencies: ['@openclawdsolana/plugin-discord', '@openclawdsolana/plugin-bootstrap'],
        actions: [],
        services: [],
      };
  ...
  describe('resolvePlugins with scoped dependencies', () => {
    ...
    test('should handle test dependencies with scoped names', async () => {
      ...
      const pluginB: Plugin = {
        name: 'test-plugin',
        description: 'Test plugin',
        testDependencies: ['@openclawdsolana/plugin-sql'],
        actions: [],
        services: [],
      };
  ...
  describe('tryInstallPlugin (auto-install)', () => {
    ...
    beforeEach(() => {
      ...
      delete process.env.OPENCLAWD_NO_PLUGIN_AUTO_INSTALL;
    });
    ...
    test('returns false when auto-install disallowed by OPENCLAWD_NO_PLUGIN_AUTO_INSTALL', async () => {
      process.env.OPENCLAWD_NO_PLUGIN_AUTO_INSTALL = 'true';
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-no-plugin-auto-install');
      expect(result).toBe(false);
      expect(called).toBe(0);
    });

    test('returns false when auto-install disallowed by OPENCLAWD_NO_AUTO_INSTALL', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-no-auto-install');
      expect(result).toBe(false);
      expect(called).toBe(0);
    });
    ...
    test('returns false when in CI environment', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-ci-env');
      expect(result).toBe(false);
      expect(called).toBe(0);
    });
    ...
    test('returns false when in test mode', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-test-mode');
      expect(result).toBe(false);
      expect(called).toBe(0);
    });
    ...
    test('succeeds when bun present and bun add exits 0', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-success');
      expect(result).toBe(true);
      expect(calls.length).toBe(2);
      expect(calls[0]).toEqual(['bun', '--version']);
      expect(calls[1]).toEqual(['bun', 'add', '@openclawdsolana/test-success']);
    });
    ...
    test('fails when bun --version exits non-zero', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-bun-version-fail');
      expect(result).toBe(false);
    ...
    test('fails when bun add exits non-zero', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/test-bun-add-fail');
      expect(result).toBe(false);
      expect(calls.length).toBe(2);
    });
    ...
    test('awaits process completion before returning', async () => {
      ...
      const result = await tryInstallPlugin('@openclawdsolana/plugin-unique-test');
      expect(result).toBe(true);
