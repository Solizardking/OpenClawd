...
import { OpenClawd } from '../openclawd';
...
describe('OpenClawd', () => {
  let OpenClawd: OpenClawd;

  beforeEach(() => {
    OpenClawd = new OpenClawd();
  });
  ...
  describe('Agent Management', () => {
    ...
    it('should add multiple agents', async () => {
      const agentIds = await OpenClawd.addAgents([
    ...
    it('should start agents', async () => {
      ...
      await OpenClawd.startAgents(agentIds);

      const agent = OpenClawd.getAgent(agentIds[0]);
    ...
    it('should stop agents', async () => {
      ...
      await OpenClawd.startAgents(agentIds);
      await OpenClawd.stopAgents(agentIds);
    ...
    it('should delete agents', async () => {
      ...
      await OpenClawd.deleteAgents(agentIds);
    ...
    it('should get all agents', async () => {
      ...
      const agents = OpenClawd.getAgents();
      expect(agents).toHaveLength(2);
    });
  });

  describe('Runtime OpenClawd Reference', () => {
    const testCharacter: Character = {
      name: 'TestAgent',
      bio: 'A test agent',
      system: 'You are a test agent',
    ...
    it('should assign OpenClawd reference to runtime when agent is added', async () => {
      const agentIds = await OpenClawd.addAgents([
        { character: testCharacter, plugins: [mockSqlPlugin] },
      ]);

      const runtime = OpenClawd.getAgent(agentIds[0]);
      expect(runtime).toBeTruthy();
      expect(runtime?.OpenClawd).toBe(OpenClawd);
    });

    it('should assign OpenClawd reference to runtime when agent is registered', async () => {
      const agentIds = await OpenClawd.addAgents([
        { character: testCharacter, plugins: [mockSqlPlugin] },
      ]);
      const runtime = OpenClawd.getAgent(agentIds[0]);
      ...
      await OpenClawd.deleteAgents(agentIds);
      OpenClawd.registerAgent(runtime!);

      expect(runtime?.OpenClawd).toBe(OpenClawd);
    });

    it('hasOpenClawd() should return true when OpenClawd is assigned', async () => {
      const agentIds = await OpenClawd.addAgents([
        { character: testCharacter, plugins: [mockSqlPlugin] },
      ]);

      const runtime = OpenClawd.getAgent(agentIds[0]);
      expect(runtime?.hasOpenClawd()).toBe(true);
    });

    it('hasOpenClawd() should narrow TypeScript type correctly', async () => {
      const agentIds = await OpenClawd.addAgents([
        { character: testCharacter, plugins: [mockSqlPlugin] },
      ]);

      const runtime = OpenClawd.getAgent(agentIds[0]);
      if (runtime?.hasOpenClawd()) {
        // TypeScript should know OpenClawd is defined here
        expect(runtime.OpenClawd).toBeDefined();
        expect(runtime.OpenClawd.handleMessage).toBeDefined();
        expect(runtime.OpenClawd.getAgent).toBeDefined();
      } else {
        throw new Error('hasOpenClawd() should return true');
      }
    });

    it('should clear OpenClawd reference on runtime.stop()', async () => {
      const agentIds = await OpenClawd.addAgents([
        { character: testCharacter, plugins: [mockSqlPlugin] },
      ]);
      ...
      // OpenClawd reference should be cleared to prevent memory leak
      expect(runtime?.OpenClawd).toBeUndefined();
    });
    ...
    it('should prevent memory leaks with bidirectional reference', async () => {
      ...
      expect(runtime?.OpenClawd).toBe(OpenClawd);
      expect(OpenClawd.getAgent(agentIds[0])).toBe(runtime);
  ...
  describe('Serverless Mode', () => {
    ...
    it('should not store runtime in registry when ephemeral is true', async () => {
      const result = await OpenClawd.addAgents(
        [{ character: testCharacter, plugins: [mockSqlPlugin] }],
        {
          ephemeral: true,
          returnRuntimes: true,
      ...
      expect(OpenClawd.getAgent(runtime.agentId)).toBeUndefined();
    });
    ...
    it('should emit event with ephemeral flag', async () => {
      ...
      OpenClawd.addEventListener('agents:added', (e: Event) => {
        addedHandler((e as CustomEvent).detail);
      });

      await OpenClawd.addAgents([{ character: testCharacter, plugins: [mockSqlPlugin] }], {
        ephemeral: true,
      });
  ...
  describe('handleMessage with runtime', () => {
    ...
    it('should accept runtime directly instead of UUID', async () => {
      ...
      const result = await OpenClawd.handleMessage(runtime, {
    ...
    it('should throw error for unknown UUID', async () => {
      ...
      await expect(
        OpenClawd.handleMessage(unknownId, {
          entityId: uuidv4() as UUID,
          roomId: uuidv4() as UUID,
          content: { text: 'Hello' },
        })
  ...
  describe('Event System', () => {
    ...
    it('should emit events when agents are stopped', async () => {
      ...
      OpenClawd.addEventListener('agents:stopped', (e: Event) => {
    ...
    it('should emit events when agents are deleted', async () => {
      ...
      OpenClawd.addEventListener('agents:deleted', (e: Event) => {
