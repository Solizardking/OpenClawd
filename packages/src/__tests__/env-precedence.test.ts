...
describe('Environment File Precedence', () => {
  ...
  describe('Real-world Scenario: Monorepo Structure', () => {
    it('should handle typical monorepo .env hierarchy', () => {
      ...
      const monorepoRoot = path.join(tempDir, 'openclawd');
      const packagesDir = path.join(monorepoRoot, 'packages');
      ...
      fs.writeFileSync(
        ...
        'LOG_LEVEL=info\nPOSTGRES_URL=postgresql://localhost/openclawd\nSHARED_VAR=from-root'
      );
      ...
      expect(process.env.POSTGRES_URL).toBe('postgresql://localhost/openclawd');
    });
  });
});
