import type { Character, IAgentRuntime, IOpenClawd, UUID, Memory, Plugin, RuntimeSettings, Content, HandleMessageOptions, HandleMessageResult, IDatabaseAdapter, HealthStatus, ReadonlyRuntime } from './types';
/**
 * OpenClawd - Multi-agent orchestration framework
 */
export declare class OpenClawd extends EventTarget implements IOpenClawd {
    private runtimes;
     ...
     * const result = await OpenClawd.handleMessage(agentId, {
     *   entityId: user.id,
     *   roomId: room.id,
     ...
     * const [runtime] = await OpenClawd.addAgents([config], { ephemeral: true, autoStart: true, returnRuntimes: true });
     ...
     * await OpenClawd.handleMessage(agentId, {
     *   entityId: user.id,
     *   roomId: room.id,
     ...
     *
     * @example
     * const results = await OpenClawd.handleMessages([
     *   {
     *     agentId: agent1Id,
     *     message: {
     *       entityId: user.id,
...
//# sourceMappingURL=openclawd.d.ts.map