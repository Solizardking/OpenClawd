...
>(
    ...
    if (typeof market.outcomePrices === "string" && market.outcomePrices.length > 0) {
        try {
            ...
            console.error("[OpenClawd Polymarket] Failed to parse outcomePrices for market:", e);
    ...
    if (typeof market.outcomes === "string" && market.outcomes.length > 0) {
        try {
            ...
            console.error("[OpenClawd Polymarket] Failed to parse outcomes for market:", e);
            outcomesParsed = [];
        }
    }
    ...
    if (typeof market.clobTokenIds === "string" && market.clobTokenIds.length > 0) {
        try {
            ...
            console.error("[OpenClawd Polymarket] Failed to parse clobTokenIds for market:", e);
            clobTokenIdsParsed = [];
        }
    }
