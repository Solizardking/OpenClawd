...
The server package provides the REST API and WebSocket server infrastructure for OpenClawd agents. It's the core runtime server that powers the OpenClawd CLI and can be embedded in other applications.

## Overview

`@openclawdsolana/server` exports a complete agent server implementation including:
...
This package is used internally by the OpenClawd CLI (`@openclawdsolana/cli`) but can also be imported directly to create custom server implementations.

## Installation

```bash
bun add @openclawdsolana/server
```
...
```typescript
import { AgentServer, ServerOptions, ServerMiddleware } from '@openclawdsolana/server';
...
server.registerMiddleware((req, res, next) => {
  res.setHeader('X-Server', 'OpenClawd');
  next();
});
...
// Create OpenClawd server
const openclawdServer = new AgentServer();
await openclawdServer.initialize();

// Mount OpenClawd APIs on your Express app
// The server provides its own Express app instance
app.use('/openclawd', openclawdServer.app);
...
```typescript
import { AgentServer } from '@openclawdsolana/server';
import { AgentRuntime, Character } from '@openclawdsolana/core';
...
> **Note:** Database operations are handled by `@openclawdsolana/core`
...
The server package includes the OpenClawd web client UI. During the build process:

1. The client package (`@openclawdsolana/client`) is built separately
...
3. Via `@openclawdsolana/client` package resolution
