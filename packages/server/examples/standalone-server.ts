 ...
 *
 * This example demonstrates how to use @openclawdsolana/server as a standalone package
...
async function createStandaloneServer() {
  try {
    logger.info('🚀 Creating standalone OpenClawd server...');
    ...
    server.registerMiddleware((req, res, next) => {
      ...
      res.setHeader('X-Powered-By', 'OpenClawd-Standalone');
      next();
    });
