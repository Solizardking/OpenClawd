...
import internalMessageBus from './message-bus.js';

/**
 * Global OpenClawd instance for MessageBusService
 * Set by AgentServer during initialization
 */
let globalOpenClawd: OpenClawd | null = null;
...
/**
 * Set the global OpenClawd instance
 * Should be called by AgentServer during initialization
 */
export function setGlobalOpenClawd(OpenClawd: OpenClawd): void {
  globalOpenClawd = OpenClawd;
  logger.info({ src: 'service:message-bus' }, 'Global OpenClawd instance set');
}
...
/**
 * Get the global OpenClawd instance
 */
function getGlobalOpenClawd(): OpenClawd {
  if (!globalOpenClawd) {
    throw new Error(
      'OpenClawd not initialized. Call setGlobalOpenClawd() before using MessageBusService.'
    );
  }
  return globalOpenClawd;
}
...
export class MessageBusService extends Service {
  ...
  public async handleIncomingMessage(data: unknown) {
    ...
    try {
      ...
      // Get OpenClawd instance
      const OpenClawd = getGlobalOpenClawd();
      ...
      // Use OpenClawd.handleMessage() with async callback
      await OpenClawd.handleMessage(
  ...
  private getAuthHeaders(): Record<string, string> {
    ...
    // Add authentication header if OPENCLAWD_SERVER_AUTH_TOKEN is configured
