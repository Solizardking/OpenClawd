import type { OpenClawd, LogEntry, Media } from '@openclawdsolana/core';
import {
  logger,
  customLevels,
  SOCKET_MESSAGE_TYPE,
...
export class SocketIORouter {
  ...
  constructor(OpenClawd: OpenClawd, serverInstance: AgentServer) {
    this.OpenClawd = OpenClawd;
  ...
  setupListeners(io: SocketIOServer) {
    logger.info(
      { src: 'ws', agentCount: this.OpenClawd.getAgents().length },
  ...
  private async handleChannelJoining(socket: Socket, payload: ChannelJoiningPayload) {
    ...
    if (entityId && (messageServerId || this.serverInstance.messageServerId)) {
      ...
      const runtime = this.OpenClawd.getAgents()[0];
      if (runtime) {
    ...
    this.logStreamConnections.forEach((filters, socketId) => {
      ...
      if (socket) {
        ...
        if (filters.level && filters.level !== 'all') {
          // Use logger levels directly from @openclawdsolana/core
          const numericLevel =
