 ...
 */
import { getOpenClawdPaths } from '@openclawdsolana/core';
import path from 'node:path';
import fs from 'node:fs';
import os from 'node:os';

/**
 * Clean up global singletons from @openclawdsolana/plugin-sql
...
export function captureEnvironment(): EnvironmentSnapshot {
  return {
    ...
    OPENCLAWD_DATABASE_DIR: process.env.OPENCLAWD_DATABASE_DIR,
    IGNORE_BOOTSTRAP: process.env.IGNORE_BOOTSTRAP,
  };
}

/**
 * Clean test-related environment variables and OpenClawdPaths cache
 */
export function cleanTestEnvironment(): void {
  // Clear OpenClawdPaths singleton cache
  getOpenClawdPaths().clearCache();
...
export function restoreEnvironment(snapshot: EnvironmentSnapshot): void {
  ...
  if (snapshot.OPENCLAWD_DATABASE_DIR !== undefined) {
    process.env.OPENCLAWD_DATABASE_DIR = snapshot.OPENCLAWD_DATABASE_DIR;
  } else {
    delete process.env.OPENCLAWD_DATABASE_DIR;
  }
...
export function setupTestEnvironment(options?: { isolateDatabase?: boolean }): EnvironmentSnapshot {
  ...
  if (options?.isolateDatabase) {
    ...
    const testDbPath = fs.mkdtempSync(path.join(os.tmpdir(), 'openclawd-server-test-'));
