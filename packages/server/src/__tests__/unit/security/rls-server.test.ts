  ...
  describe('Server ID Assignment', () => {
    it('should use server_id as serverId when data isolation is enabled', () => {
      const mockConfig = {
        ...
        OPENCLAWD_SERVER_ID: 'test-tenant-123',
      };

      const dataIsolationEnabled = mockConfig.ENABLE_DATA_ISOLATION === 'true';
      const serverId = stringToUuid(mockConfig.OPENCLAWD_SERVER_ID);
...
describe('Environment Variable Configuration', () => {
  ...
  describe('OPENCLAWD_SERVER_ID', () => {
    it('should generate consistent server_id from OPENCLAWD_SERVER_ID string', () => {
      const openclawdServerId = 'my-tenant-123';
      const serverId1 = stringToUuid(openclawdServerId);
      const serverId2 = stringToUuid(openclawdServerId);

      expect(serverId1).toBe(serverId2);
    });

    it('should generate different server_ids for different OPENCLAWD_SERVER_ID values', () => {
