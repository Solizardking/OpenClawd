...
import type { IAgentRuntime, UUID, OpenClawd } from '@openclawdsolana/core';
...
} as Partial<OpenClawd> as OpenClawd;
...
describe('Sessions API', () => {
  ...
  beforeEach(() => {
    ...
    jest.clearAllMocks();
    mockAgents.clear();
    (mockOpenClawd.getAgent as jest.Mock).mockImplementation((id: UUID) => mockAgents.get(id));
    (mockOpenClawd.getAgents as jest.Mock).mockImplementation(() => Array.from(mockAgents.values()));
    ...
    router = createSessionsRouter(mockOpenClawd, mockServerInstance);
  ...
  describe('Transport Parameter', () => {
    ...
    it('should accept http transport', async () => {
      ...
      // Mock OpenClawd.handleMessage for http transport
      const mockHandleMessage = jest.fn().mockResolvedValue({
        processing: {
          responseContent: { text: 'Agent response' },
        },
      });
      (mockOpenClawd as any).handleMessage = mockHandleMessage;
    ...
    it('should accept sse transport and set SSE headers', async () => {
      ...
      // Mock OpenClawd.handleMessage for sse transport
