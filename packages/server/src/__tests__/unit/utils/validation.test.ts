...
import type { IAgentRuntime, UUID } from '@openclawdsolana/core';
...
describe('Validation Functions', () => {
  ...
  describe('getRuntime', () => {
    it('should return runtime when agent exists', () => {
      ...
      const result = getRuntime(mockOpenClawd, agentId);

      expect(result).toBe(mockRuntime);
      expect(mockOpenClawd.getAgent).toHaveBeenCalledWith(agentId);
    });
    ...
    it('should throw error when agent does not exist', () => {
      ...
      expect(() => getRuntime(mockOpenClawd, agentId)).toThrow(`Agent not found: ${agentId}`);
    });
