...
describe('Client Path Resolution Logic', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `openclawd-test-${Date.now()}`);
  ...
  describe('Path Resolution Priority', () => {
    ...
    it('should check bun global installation path', () => {
      ...
      const bunGlobalPath = path.join(
        tempDir,
        '.bun/install/global/node_modules/@openclawdsolana/cli/dist'
      );
  ...
  describe('Fallback Paths', () => {
    it('should check common global npm paths', () => {
      const commonPaths = [
        '/usr/local/lib/node_modules/@openclawdsolana/cli/dist',
        '/usr/lib/node_modules/@openclawdsolana/cli/dist',
        path.join(os.homedir(), '.npm-global/lib/node_modules/@openclawdsolana/cli/dist'),
      ];

      // Verify these are valid path formats
      commonPaths.forEach((p) => {
        expect(path.isAbsolute(p)).toBe(true);
        expect(p).toContain('@openclawdsolana/cli/dist');
      });
    });
  ...
  describe('Development vs Production', () => {
    ...
    it('should handle require.resolve for installed packages', () => {
      ...
      const mockRequireResolve = (pkg: string) => {
        if (pkg === '@openclawdsolana/cli/package.json') {
          return path.join(tempDir, 'node_modules/@openclawdsolana/cli/package.json');
        }
        throw new Error('Module not found');
      };

      try {
        const packageJsonPath = mockRequireResolve('@openclawdsolana/cli/package.json');
        const cliPath = path.resolve(path.dirname(packageJsonPath), 'dist');

        expect(cliPath).toContain('@openclawdsolana/cli/dist');
      } catch {
