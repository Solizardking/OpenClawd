...
describe('Basic Server Functionality', () => {
  ...
  describe('UI Enable/Disable Logic', () => {
    ...
    const testUIEnabled = (nodeEnv?: string, openclawdUIEnable?: string): boolean => {
      const originalNodeEnv = process.env.NODE_ENV;
      const originalUIEnable = process.env.OPENCLAWD_UI_ENABLE;
      ...
      if (openclawdUIEnable !== undefined) {
        process.env.OPENCLAWD_UI_ENABLE = openclawdUIEnable;
      } else {
        delete process.env.OPENCLAWD_UI_ENABLE;
      }
      ...
      if (originalUIEnable !== undefined) {
        process.env.OPENCLAWD_UI_ENABLE = originalUIEnable;
      } else {
        delete process.env.OPENCLAWD_UI_ENABLE;
      }
    ...
    it('should allow explicit override with OPENCLAWD_UI_ENABLE=true', () => {
      expect(testUIEnabled('production', 'true')).toBe(true);
      expect(testUIEnabled('development', 'true')).toBe(true);
      expect(testUIEnabled('production', 'TRUE')).toBe(true);
    });

    it('should allow explicit override with OPENCLAWD_UI_ENABLE=false', () => {
