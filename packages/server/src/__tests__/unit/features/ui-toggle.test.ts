/**
 * Tests for OPENCLAWD_UI_ENABLE feature
 */
...
describe('UI Disable Feature Integration', () => {
  ...
  describe('Environment Variable Behavior', () => {
    it('should read OPENCLAWD_UI_ENABLE from environment', () => {
    ...
    it('should handle combined NODE_ENV and OPENCLAWD_UI_ENABLE scenarios', () => {
      const testScenarios = [
        // NODE_ENV, OPENCLAWD_UI_ENABLE, Expected UI State, Description
      ...
      testScenarios.forEach(([nodeEnv, openclawdUIEnable, expected, description]) => {
        const isProduction = nodeEnv === 'production';
        const uiEnabled =
          openclawdUIEnable !== undefined
            ? (openclawdUIEnable as string).toLowerCase() === 'true'
            : !isProduction;
  ...
  describe('Documentation Consistency', () => {
    it('should match .env.example documentation patterns', () => {
      ...
      expect(envExamplePattern.variable).toBe('OPENCLAWD_UI_ENABLE=');
    });
  });
});
