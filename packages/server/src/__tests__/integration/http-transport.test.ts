 ...
 *
 * Note: OpenClawd.handleMessage is mocked to avoid LLM costs while testing
 * the full request/response flow.
 */

import { describe, it, expect, beforeAll, afterAll } from 'bun:test';
import type { UUID, OpenClawd, HandleMessageResult } from '@openclawdsolana/core';
...
describe('HTTP Transport Integration', () => {
  ...
  let agentId: UUID;
  let sessionId: UUID;
  let originalHandleMessage: OpenClawd['handleMessage'];
  ...
  beforeAll(async () => {
    ...
    // Mock OpenClawd.handleMessage to avoid LLM calls
    // This is the ONLY mock - everything else is real
    const OpenClawd = serverFixture.getServer().OpenClawd!;
    originalHandleMessage = OpenClawd.handleMessage.bind(OpenClawd);
    OpenClawd.handleMessage = async (
      _agentId: UUID,
      message: any,
      _options?: unknown
  ...
  afterAll(async () => {
    ...
    if (originalHandleMessage && serverFixture.getServer().OpenClawd) {
      serverFixture.getServer().OpenClawd!.handleMessage = originalHandleMessage;
    }
