import type { Agent, Character, OpenClawd } from '@openclawdsolana/core';
import { validateUuid, logger, getSalt, encryptObjectValues } from '@openclawdsolana/core';
...
export function createAgentCrudRouter(
  ...
  router.get('/', async (_, res) => {
    try {
      ...
      const runtimes = OpenClawd.getAgents().map((a) => a.agentId);
  ...
  router.patch('/:agentId', async (req, res) => {
    ...
    try {
      ...
      const activeRuntime = OpenClawd.getAgent(agentId);
      ...
      if (activeRuntime && updatedAgent) {
        if (needsRestart) {
          ...
          await OpenClawd.updateAgent(agentId, characterData as Character);
        }
      }
