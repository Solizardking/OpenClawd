import type { OpenClawd, IAgentRuntime, UUID } from '@openclawdsolana/core';
import { validateUuid, logger, createUniqueUuid } from '@openclawdsolana/core';
 ...
 */
export function createAgentWorldsRouter(OpenClawd: OpenClawd): express.Router {
  const router = express.Router();

  // Get all worlds
  router.get('/worlds', async (_req, res) => {
    try {
      const runtime = OpenClawd.getAgents()[0];
