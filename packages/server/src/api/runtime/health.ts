 ...
 */
export function createHealthRouter(OpenClawd: OpenClawd, serverInstance: AgentServer): express.Router {
  ...
  router.get('/status', (_req, res) => {
    ...
    res.send(
      JSON.stringify({
        status: 'ok',
        agentCount: OpenClawd.getAgents().length,
  ...
  router.get('/health', (_req, res) => {
    ...
    const healthcheck = {
      ...
      timestamp: new Date().toISOString(),
      dependencies: {
        agents: OpenClawd.getAgents().length > 0 ? 'healthy' : 'no_agents',
      },
    };
