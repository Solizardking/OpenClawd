import { logger, recentLogs } from '@openclawdsolana/core';
import express from 'express';

// Custom levels from @openclawdsolana/core logger
const LOG_LEVELS = {
  fatal: 60,
  error: 50,
  warn: 40,
...
export function createLoggingRouter(): express.Router {
  ...
  const logsHandler = async (req: express.Request, res: express.Response) => {
    ...
    try {
      // Get logs from the OpenClawd logger's recentLogs function
