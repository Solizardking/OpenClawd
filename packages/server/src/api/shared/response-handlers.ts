...
import type { OpenClawd, Memory } from '@openclawdsolana/core';
import type { UUID, Content } from '@openclawdsolana/core';
import { logger } from '@openclawdsolana/core';
import { type TransportType } from './constants';

/**
 * Message memory type for OpenClawd.handleMessage
...
async function handleSSETransport(
  ...
  try {
    await OpenClawd.handleMessage(agentId, messageMemory, {
...
async function handleHttpTransport(
  ...
  try {
    const result = await OpenClawd.handleMessage(agentId, messageMemory);
    res.status(201).json({
      success: true,
      userMessage,
...
export async function handleTransport(options: HandleTransportOptions): Promise<void> {
  ...
  switch (transport) {
    case 'sse':
      await handleSSETransport(res, OpenClawd, agentId, messageMemory, userMessage);
      break;
