import type { IAgentRuntime, UUID, OpenClawd, RouteRequest } from '@openclawdsolana/core';
import { logger, validateUuid, addLogListener } from '@openclawdsolana/core';
 ...
 * @param OpenClawd OpenClawd instance
...
export function setupSocketIO(
  ...
  const centralSocketRouter = new SocketIORouter(OpenClawd, serverInstance);
...
export function createPluginRouteHandler(OpenClawd: OpenClawd): express.RequestHandler {
  return (req, res, next) => {
    logger.debug(
    ...
    if (OpenClawd.getAgents().length === 0) {
    ...
    if (agentIdFromQuery && validateUuid(agentIdFromQuery)) {
      const runtime = OpenClawd.getAgent(agentIdFromQuery);
      if (runtime) {
        logger.debug(
      ...
      // check in all agents...
      for (const runtime of OpenClawd.getAgents()) {
 ...
 * @param {OpenClawd} OpenClawd - OpenClawd instance containing all agents and their runtimes.
...
export function createApiRouter(
  ...
  router.use(createPluginRouteHandler(OpenClawd));
