...
import sqlPlugin, {
  ...
  assignAgentToServer,
  uninstallRLS,
} from '@openclawdsolana/plugin-sql';
import { stringToUuid, type Plugin } from '@openclawdsolana/core';
 ...
 * Signatures match BaseDrizzleAdapter in @openclawdsolana/plugin-sql
 */
...
export class AgentServer {
  ...
  public OpenClawd?: OpenClawd; // Core OpenClawd instance (public for direct access)
  ...
  public async startAgents(
    ...
    // Delegate to OpenClawd for config/plugin resolution and agent creation
    const agentIds = await this.OpenClawd.addAgents(agentConfigs, options);

    // Start all agents
    await this.OpenClawd.startAgents(agentIds);

    // Register agents with server and persist to database
    const runtimes: IAgentRuntime[] = [];
    for (const id of agentIds) {
      const runtime = this.OpenClawd.getAgent(id);
      if (runtime) {
        if (this.database) {
          try {
  ...
  public async stopAgents(agentIds: UUID[]): Promise<void> {
    if (!this.OpenClawd) {
      throw new Error('OpenClawd not initialized');
    }

    // Delegate to OpenClawd for batch stop
    await this.OpenClawd.stopAgents(agentIds);
  }

  /**
   * Get all agents from the OpenClawd instance
  ...
  public getAllAgents(): IAgentRuntime[] {
    ...
    return this.OpenClawd.getAgents();
  }

  /**
   * Get an agent by ID from the OpenClawd instance
  ...
  public getAgent(agentId: UUID): IAgentRuntime | undefined {
    ...
    return this.OpenClawd.getAgent(agentId);
  }
  ...
  private async initialize(config?: ServerConfig): Promise<void> {
    ...
    try {
      ...
      const openclawdServerIdString = process.env.OPENCLAWD_SERVER_ID;
      ...
      if (dataIsolationEnabled) {
        ...
        if (!openclawdServerIdString) {
          logger.error(
            { src: 'db' },
            'ENABLE_DATA_ISOLATION requires OPENCLAWD_SERVER_ID environment variable'
          );
          throw new Error(
            'OPENCLAWD_SERVER_ID environment variable is required when data isolation is enabled'
          );
        }

        // Convert OPENCLAWD_SERVER_ID string to deterministic UUID
        const server_id = stringToUuid(openclawdServerIdString);

        logger.info(
          { src: 'db', serverId: server_id.slice(0, 8), serverIdString: openclawdServerIdString },
      ...
      logger.info({ src: 'http' }, 'Initializing OpenClawd');
      this.OpenClawd = new OpenClawd();
      this.OpenClawd.enableEditableMode();
      setGlobalOpenClawd(this.OpenClawd);
      setGlobalAgentServer(this);
      logger.success({ src: 'http' }, 'OpenClawd initialized');
  ...
  private async ensureDefaultServer(): Promise<void> {
    try {
      ...
      // - rlsServerId: Used for PostgreSQL RLS isolation (from OPENCLAWD_SERVER_ID env var)
      // - messageServerId: Used for message_servers.id (random UUID, exposed in API)
      // This prevents leaking sensitive OPENCLAWD_SERVER_ID values in public API paths
      ...
      if (!defaultServer) {
        ...
        try {
          ...
          await db.execute(sql`
            ...
            VALUES (${this.messageServerId}, ${serverName}, ${'openclawd_default'}, NOW(), NOW())
  ...
  private async initializeServer(config?: ServerConfig) {
    try {
      ...
      this.app.get(
        ...
        healthCheckRateLimiter,
        (_req: express.Request, res: express.Response) => {
          const agents = this.OpenClawd?.getAgents() || [];
      ...
      const serverAuthToken = process.env.OPENCLAWD_SERVER_AUTH_TOKEN;
      logger.info(
        { src: 'http' },
      ...
      // Active if OPENCLAWD_SERVER_AUTH_TOKEN is configured
      ...
      if (serverAuthToken) {
        ...
        logger.warn(
          { src: 'http' },
          'Authentication middleware configured - API Key: DISABLED (set OPENCLAWD_SERVER_AUTH_TOKEN to enable)'
        );
      }
      ...
      if (this.isWebUIEnabled) {
        ...
        const possiblePaths = [
          ...
          (() => {
            try {
              return path.resolve(
                path.dirname(require.resolve('@openclawdsolana/client/package.json')),
                'dist'
              );
            } catch {
              return null;
          ...
          (() => {
            try {
              ...
              const bunGlobalPath = path.join(
                os.homedir(),
                '.bun/install/global/node_modules/@openclawdsolana/server/dist/client'
              );
              ...
              try {
                ...
                if (proc.exitCode === 0 && proc.stdout) {
                  ...
                  const globalServerPath = path.join(npmRoot, '@openclawdsolana/server/dist/client');
          ...
          ...[
            '/usr/local/lib/node_modules/@openclawdsolana/server/dist/client',
            '/usr/lib/node_modules/@openclawdsolana/server/dist/client',
            path.join(os.homedir(), '.npm-global/lib/node_modules/@openclawdsolana/server/dist/client'),
                ...
                if (existsSync(nvmPath)) {
                  ...
                  for (const version of versions) {
                    const cliPath = path.join(
                      nvmPath,
                      version,
                      'lib/node_modules/@openclawdsolana/server/dist/client'
                    );
      ...
      const pluginRouteHandler = createPluginRouteHandler(this.OpenClawd!);
      ...
      const apiRouter = createApiRouter(this.OpenClawd!, this);
      this.app.use(
        '/api',
        apiRouter,
      ...
      this.socketIO = setupSocketIO(this.server, this.OpenClawd!, this);
   ...
   * Note: Agents should ideally be created through OpenClawd.addAgent() for proper orchestration.
  ...
  public async unregisterAgent(agentId: UUID) {
    ...
    try {
      const agent = this.OpenClawd?.getAgent(agentId);
      ...
      if (this.OpenClawd) {
        await this.OpenClawd.deleteAgents([agentId]);
      }
...
// Export OpenClawd from core (re-export for convenience)
export { OpenClawd } from '@openclawdsolana/core';
