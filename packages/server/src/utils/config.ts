...
import { type UUID, parseBooleanFromText, getDatabaseDir } from '@openclawdsolana/core';
...
export function resolvePgliteDir(dir?: string, fallbackDir?: string): string {
  ...
  if (fallbackDir && !process.env.PGLITE_DATA_DIR && !process.env.OPENCLAWD_DATABASE_DIR) {
...
export function isWebUIEnabled(): boolean {
  ...
  const uiEnabledEnv = process.env.OPENCLAWD_UI_ENABLE;
