/**
 * Test that the .openclawd directory is automatically created when using PGLite with a file path
 */
...
describe('Directory Creation', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = mkdtempSync(path.join(tmpdir(), 'openclawd-test-'));
  });
  ...
  it('should automatically create directory for PGLite when it does not exist', () => {
    const dataDir = path.join(tempDir, '.openclawd', '.openclawddb');
