...
import { type IDatabaseAdapter, stringToUuid, type UUID } from '@openclawdsolana/core';
...
describe.skipIf(!process.env.POSTGRES_URL)('PostgreSQL RLS Entity Integration', () => {
  let setupClient: Client; // Setup client for migrations (openclawd_test user)
  ...
  const POSTGRES_URL =
    process.env.POSTGRES_URL || 'postgresql://openclawd_test:test123@localhost:5432/openclawd_test';
  // Use OPENCLAWD_SERVER_ID if set (CI mode with ENABLE_DATA_ISOLATION=true)
  // Otherwise generate a random UUID for local testing
  const serverId = process.env.OPENCLAWD_SERVER_ID
    ? stringToUuid(process.env.OPENCLAWD_SERVER_ID)
    : uuidv4();
  ...
  beforeAll(async () => {
    ...
    // Grant permissions on newly created tables to openclawd_test
    // (in case the test is run by a different user who owns the tables)
    try {
      await setupClient.query(`GRANT ALL ON ALL TABLES IN SCHEMA public TO openclawd_test`);
      await setupClient.query(`GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO openclawd_test`);
    } catch (_err) {
      // Ignore if already granted or permission denied (we're already openclawd_test)
