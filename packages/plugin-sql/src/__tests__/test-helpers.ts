import type { Plugin, UUID } from '@openclawdsolana/core';
import { AgentRuntime } from '@openclawdsolana/core';
...
export async function createTestDatabase(
  ...
  if (process.env.POSTGRES_URL) {
    ...
    // PGlite testing
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'openclawd-test-'));
...
export async function createIsolatedTestDatabase(
  ...
  if (process.env.POSTGRES_URL) {
    ...
    const tempDir = path.join(os.tmpdir(), `openclawd-test-${testId}-${Date.now()}`);
...
export async function createIsolatedTestDatabaseForMigration(testName: string): Promise<{
  ...
  if (process.env.POSTGRES_URL) {
    ...
    // Ensure grants for openclawd_test user (PostgreSQL 15+ requires explicit grants on public schema)
    // These must be set at the start of each test to ensure openclawd_test can create tables
    await db.execute(sql.raw(`GRANT ALL ON SCHEMA public TO openclawd_test`));
    await db.execute(sql.raw(`GRANT USAGE ON SCHEMA public TO openclawd_test`));
    await db.execute(sql.raw(`GRANT CREATE ON SCHEMA public TO openclawd_test`));
    ...
    const cleanup = async () => {
      try {
        ...
        // Restore grants for openclawd_test user (PostgreSQL 15+ requires explicit grants on public schema)
    ...
    // PGLite - use unique directory per test
    const tempDir = path.join(os.tmpdir(), `openclawd-migration-test-${testId}-${Date.now()}`);
 ...
 * with destructive migrations. This helper manages the OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS
...
export async function createIsolatedTestDatabaseForSchemaEvolutionTests(testName: string): Promise<{
  ...
  const originalDestructiveSetting = process.env.OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS;
  ...
  const enhancedCleanup = async () => {
    ...
    if (originalDestructiveSetting !== undefined) {
      process.env.OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS = originalDestructiveSetting;
    } else {
      delete process.env.OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS;
    }
