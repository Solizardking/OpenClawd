...
describe('Runtime Migrator - Transaction Support & Concurrency Tests', () => {
  ...
  describe('Transaction Atomicity', () => {
    it('should commit all changes when migration succeeds', async () => {
      ...
      await migrator.migrate('@openclawdsolana/transaction-test-success', validSchema);
      ...
      const migrationRecorded = await db.execute(
        sql.raw(`SELECT COUNT(*) as count
                 ...
                 WHERE plugin_name = '@openclawdsolana/transaction-test-success'`)
      );
    ...
    it('should rollback all changes when migration fails', async () => {
      ...
      let errorMessage = '';
      try {
        await migrator.migrate('@openclawdsolana/transaction-test-fail', failingSchema);
      } catch (error) {
      ...
      const failedMigrationRecorded = await db.execute(
        sql.raw(`SELECT COUNT(*) as count
                 FROM migrations._migrations
                 WHERE plugin_name = '@openclawdsolana/transaction-test-fail'`)
      );
    ...
    it('should maintain consistent state across migration failures', async () => {
      ...
      try {
        ...
        await migrator.migrate('@openclawdsolana/invalid-migration-test', invalidSchema);
      } catch (error) {
        errorOccurred = true;
      }
  ...
  describe('PostgreSQL Advisory Locks for Concurrent Migrations', () => {
    ...
    testOrSkip(
      ...
      async () => {
        ...
        const [result1, result2] = await Promise.allSettled([
          migrator.migrate('@openclawdsolana/concurrent-test-same-plugin', schema),
          migrator.migrate('@openclawdsolana/concurrent-test-same-plugin', schema),
        ]);
        ...
        const migrationCount = await db.execute(
          sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name = '@openclawdsolana/concurrent-test-same-plugin'`)
        );
    ...
    it('should allow concurrent migrations for different plugins', async () => {
      ...
      const [result1, result2] = await Promise.allSettled([
        migrator.migrate('@openclawdsolana/concurrent-test-1', schema1),
        migrator.migrate('@openclawdsolana/concurrent-test-2', schema2),
      ]);
      ...
      const migration1Count = await db.execute(
        sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name = '@openclawdsolana/concurrent-test-1'`)
      );

      const migration2Count = await db.execute(
        sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name = '@openclawdsolana/concurrent-test-2'`)
      );
    ...
    testOrSkip('should use proper locking to prevent race conditions', async () => {
      ...
      const results = await Promise.allSettled([
        migrator.migrate('@openclawdsolana/concurrent-test-locking', testSchema) as Promise<any>,
        migrator2.migrate('@openclawdsolana/concurrent-test-locking', testSchema) as Promise<any>,
        migrator3.migrate('@openclawdsolana/concurrent-test-locking', testSchema) as Promise<any>,
      ]);
      ...
      const migrationCount = await db.execute(
        sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name = '@openclawdsolana/concurrent-test-locking'`)
      );
    ...
    testOrSkip('should release advisory locks after migration completion', async () => {
      ...
      await migrator.migrate('@openclawdsolana/concurrent-test-cleanup', testSchema);
      ...
      await migrator.migrate('@openclawdsolana/concurrent-test-cleanup-2', anotherSchema);
    ...
    it('should handle high-concurrency scenarios with advisory locks', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        migrationPromises.push(migrator.migrate(`@openclawdsolana/concurrent-test-high-${i}`, schema));
      }
      ...
      const totalMigrations = await db.execute(
        sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name LIKE '@openclawdsolana/concurrent-test-high-%'`)
      );
    ...
    it('should handle errors in one migration without affecting others', async () => {
      ...
      const [validResult, invalidResult] = await Promise.allSettled([
        migrator.migrate('@openclawdsolana/concurrent-test-valid', validSchema),
        migrator.migrate('@openclawdsolana/concurrent-test-invalid', invalidSchema),
      ]);
      ...
      const validMigrationExists = await db.execute(
        sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name = '@openclawdsolana/concurrent-test-valid'`)
      );
      ...
      const invalidMigrationExists = await db.execute(
        sql.raw(`SELECT COUNT(*) as count FROM migrations._migrations 
                 WHERE plugin_name = '@openclawdsolana/concurrent-test-invalid'`)
      );
  ...
  describe('Advisory Lock Security', () => {
    ...
    it('should generate consistent lock IDs for the same plugin', async () => {
      const pluginName = '@openclawdsolana/advisory-lock-test';
    ...
    it('should generate different lock IDs for different plugins', async () => {
      const plugin1 = '@openclawdsolana/lock-plugin-1';
      const plugin2 = '@openclawdsolana/lock-plugin-2';
    ...
    it('should use CAST for type safety in advisory lock queries', async () => {
      ...
      const lockId = (migrator as any).getAdvisoryLockId('@openclawdsolana/lock-security-test');
    ...
    it('should reject migration if invalid lock ID is generated', async () => {
      ...
      try {
        ...
        await expect(migrator.migrate('@openclawdsolana/invalid-lock-test', testSchema)).rejects.toThrow(
    ...
    it('should handle concurrent migrations safely with advisory locks', async () => {
      ...
      const results = await Promise.allSettled([
        migrator.migrate('@openclawdsolana/advisory-concurrent-test-1', schema1),
        migrator.migrate('@openclawdsolana/advisory-concurrent-test-2', schema2),
      ]);
  ...
  describe('Race Condition Prevention', () => {
    ...
    testOrSkip('should handle race condition when lastMigration is initially null', async () => {
      ...
      const pluginName = '@openclawdsolana/test-race-condition-null-initial';
