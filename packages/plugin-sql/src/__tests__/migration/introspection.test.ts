...
describe('Database Introspection Tests', () => {
  ...
  describe('Basic Introspection', () => {
    ...
    it('should detect existing tables correctly', async () => {
      // Initially no tables
      let hasExisting = await introspector.hasExistingTables('@openclawdsolana/plugin-sql');
      ...
      hasExisting = await introspector.hasExistingTables('@openclawdsolana/plugin-sql');
  ...
  describe('Migration with Pre-existing Tables', () => {
    it('should handle migration when tables already exist without snapshots', async () => {
      ...
      await migrator.migrate('@openclawdsolana/plugin-sql', schema, { verbose: false, force: true });
    ...
    it('should handle plugin schemas correctly during introspection', async () => {
      ...
      await db.execute(sql`CREATE SCHEMA openclawd_test_plugin`);

      await db.execute(sql`
        CREATE TABLE openclawd_test_plugin.config (
          id SERIAL PRIMARY KEY,
          key TEXT UNIQUE NOT NULL,
          value TEXT
        )
      ...
      const hasExisting = await introspector.hasExistingTables('@openclawdsolana/test-plugin');
      expect(hasExisting).toBe(true);

      // Define schema in code
      const testSchema = pgSchema('openclawd_test_plugin');
      ...
      // Run migration
      await migrator.migrate('@openclawdsolana/test-plugin', schema, { verbose: false });

      // Verify the new column was added
      const result = await db.execute(sql`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_schema = 'openclawd_test_plugin' 
  ...
  describe('Edge Cases', () => {
    ...
    it('should not introspect when snapshot already exists', async () => {
      ...
      await migrator.migrate('@openclawdsolana/plugin-sql', updatedSchema, { verbose: false });
