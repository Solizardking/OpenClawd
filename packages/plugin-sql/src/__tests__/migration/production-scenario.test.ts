...
describe('Production Migration Scenarios', () => {
  ...
  describe('Scenario 1: Existing OpenClawd Core Tables', () => {
    it('should block destructive migrations when camelCase columns need renaming to snake_case', async () => {
      ...
      try {
        await migrator.migrate('@openclawdsolana/plugin-sql', coreSchema, { verbose: false, force: false });
    ...
    it('should work with tables that already have snake_case columns (post-migration)', async () => {
      ...
      await migrator.migrate('@openclawdsolana/plugin-sql', coreSchema, { verbose: false });
    ...
    it('should handle version mismatch between DB and code schema', async () => {
      ...
      // Run migration
      await migrator.migrate('@openclawdsolana/plugin-sql', schema, { verbose: false });
  ...
  describe('Scenario 2: Multiple Plugin Tables in Production', () => {
    it('should handle multiple plugins with existing tables', async () => {
      ...
      await db.execute(sql`CREATE SCHEMA IF NOT EXISTS openclawd_analytics`);

      await db.execute(sql`
        CREATE TABLE openclawd_analytics.events (
      ...
      await db.execute(sql`
        CREATE TABLE openclawd_analytics.metrics (
          id SERIAL PRIMARY KEY,
      ...
      await db.execute(sql`
        INSERT INTO openclawd_analytics.events (event_type, agent_id, payload) 
        VALUES 
      ...
      await db.execute(sql`
        INSERT INTO openclawd_analytics.metrics (metric_name, value, tags) 
        VALUES 
      ...
      const hasExisting = await introspector.hasExistingTables('@openclawdsolana/analytics');
      expect(hasExisting).toBe(true);

      // Introspect to verify structure
      const snapshot = await introspector.introspectSchema('openclawd_analytics');
      expect(Object.keys(snapshot.tables)).toHaveLength(2);
      expect(snapshot.tables['openclawd_analytics.events']).toBeDefined();
      expect(snapshot.tables['openclawd_analytics.metrics']).toBeDefined();

      // Verify data is there
      const events = await db.execute(sql`
        SELECT COUNT(*) as count FROM openclawd_analytics.events
      `);
      expect(Number(events.rows[0].count)).toBe(2);

      const metrics = await db.execute(sql`
        SELECT COUNT(*) as count FROM openclawd_analytics.metrics
      `);
    ...
    it('should handle schema conflicts gracefully', async () => {
      ...
      try {
        await migrator.migrate('@openclawdsolana/plugin-sql', schema, {
          verbose: false,
