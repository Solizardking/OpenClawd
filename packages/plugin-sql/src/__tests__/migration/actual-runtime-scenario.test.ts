...
describe('Actual Runtime Scenario - Plugin Loading Simulation', () => {
  ...
  it('should handle plugin migrations as they would be loaded at runtime', async () => {
    ...
    await sqlPluginMigrator.migrate('@openclawdsolana/plugin-sql', coreSchema, {
      verbose: false,
    });
    ...
    await sqlPluginMigrator2.migrate('@openclawdsolana/plugin-sql', coreSchema, {
      verbose: false,
    });
  ...
  it('should test shared migrator scenario', async () => {
    ...
    await sharedMigrator.migrate('@openclawdsolana/plugin-sql', coreSchema, {
      verbose: false,
    });
