 ...
 *
 * This test uses the ACTUAL OpenClawd production schemas to verify
...
describe('Schema Evolution Test: Drop Column with Production Schema', () => {
  ...
  it('should handle dropping username column from agents table with production data', async () => {
    ...
    expect(blockedError?.message).toContain('OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS');
    ...
    console.log('\n⚠️  Testing with OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS=true...');
    await migrator.migrate('@openclawdsolana/production-schema-v1', schemaV2);
