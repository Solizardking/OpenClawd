 ...
 *
 * This test uses the ACTUAL OpenClawd production schemas with their
...
describe('Schema Evolution Test: Drop Table with Production Relationships', () => {
  ...
  it('should handle dropping memories table with cascade effects on production schema', async () => {
    ...
    await migrator.migrate('@openclawdsolana/production-schema-v1', schemaV1);
    ...
    const dataLossCheck = await migrator.checkMigration('@openclawdsolana/production-schema-v1', schemaV2);
    ...
    try {
      await migrator.migrate('@openclawdsolana/production-schema-v1', schemaV2);
    } catch (error) {
    ...
    expect(productionError?.message).toContain('OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS');
    ...
    console.log('\n⚠️  Attempting migration with OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS=true...');
  ...
  it('should detect cascade effects when dropping multiple related tables', async () => {
    ...
    await migrator.migrate('@openclawdsolana/production-cascade-test', schemaV1);
    ...
    const check = await migrator.checkMigration('@openclawdsolana/production-cascade-test', schemaV2);
