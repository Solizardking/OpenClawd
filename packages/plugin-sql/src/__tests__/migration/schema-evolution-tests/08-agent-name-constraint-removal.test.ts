...
import type { MessageExample } from '@openclawdsolana/core';
...
describe('Schema Evolution: Agent Name Constraint Removal', () => {
  ...
  it('should handle real-world migration scenario: existing database to new schema', async () => {
    ...
    const productionAgents = [
      { id: uuidv4(), name: 'OpenClawd', username: 'openclawd_prod', bio: '["Production OpenClawd"]' },
    ...
    try {
      await db.execute(sql`
        ...
        VALUES (${uuidv4()}, 'OpenClawd', 'openclawd_duplicate')
      `);
    } catch (error) {
    ...
    const openclawdClone1 = uuidv4();
    const openclawdClone2 = uuidv4();
    const openclawdClone3 = uuidv4();

    await db.execute(sql`
      INSERT INTO agents (id, name, username, bio)
      VALUES (${openclawdClone1}, 'OpenClawd', 'openclawd_clone1', '["First OpenClawd clone"]'::jsonb)
    `);

    await db.execute(sql`
      INSERT INTO agents (id, name, username, bio)
      VALUES (${openclawdClone2}, 'OpenClawd', 'openclawd_clone2', '["Second OpenClawd clone"]'::jsonb)
    `);

    await db.execute(sql`
      INSERT INTO agents (id, name, username, bio)
      VALUES (${openclawdClone3}, 'OpenClawd', 'openclawd_clone3', '["Third OpenClawd clone"]'::jsonb)
    `);

    // Step 6: Verify all OpenClawd instances exist
    const openclawds = await db.execute<{
      id: string;
      name: string;
      username: string;
      bio: any;
    }>(sql`SELECT id, name, username, bio FROM agents WHERE name = 'OpenClawd' ORDER BY created_at`);

    expect(openclawds.rows.length).toBe(4); // 1 original + 3 clones
    const openclawdIds = openclawds.rows.map((e) => e.id);
    expect(new Set(openclawdIds).size).toBe(4); // All unique IDs

    // Verify each is distinct by username
    const openclawdUsernames = openclawds.rows.map((e) => e.username);
    expect(openclawdUsernames).toContain('openclawd_prod');
    expect(openclawdUsernames).toContain('openclawd_clone1');
    expect(openclawdUsernames).toContain('openclawd_clone2');
    expect(openclawdUsernames).toContain('openclawd_clone3');

    // Step 7: Verify CRUD operations work correctly on duplicate-name agents
    // Update one OpenClawd
    await db.execute(sql`
      UPDATE agents 
      SET bio = '["Updated OpenClawd clone 1"]'::jsonb
      WHERE id = ${openclawdClone1}
    `);

    const updatedOpenClawd = await db.execute<{ bio: any }>(sql`
      SELECT bio FROM agents WHERE id = ${openclawdClone1}
    `);

    expect(updatedOpenClawd.rows[0].bio).toContain('Updated OpenClawd clone 1');

    // Delete one OpenClawd
    await db.execute(sql`DELETE FROM agents WHERE id = ${openclawdClone2}`);

    const remainingOpenClawds = await db.execute<{ count: number }>(sql`
      SELECT COUNT(*) as count FROM agents WHERE name = 'OpenClawd'
    `);
    expect(Number(remainingOpenClawds.rows[0].count)).toBe(3);

    // Final verification: Total agent count
    const finalAgentCount = await db.execute<{ count: number }>(sql`
      SELECT COUNT(*) as count FROM agents
    `);
    expect(Number(finalAgentCount.rows[0].count)).toBe(6); // 4 original + 3 OpenClawds - 1 deleted = 6
  ...
  it('should document the migration path for users', async () => {
    ...
    console.log('================================================================');
    console.log('If you have an existing OpenClawd database with the old schema,');
