...
describe('Schema Evolution Test: Type Changes with Data', () => {
  ...
  it('should handle JSONB to text conversion with complex data', async () => {
    ...
    await migrator.migrate('@openclawdsolana/schema-evolution-test-types-v1', schemaV1);
    ...
    const check = await migrator.checkMigration(
      '@openclawdsolana/schema-evolution-test-types-v1',
      schemaV2
    );
    ...
    // Migration allowed because OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS is set in beforeEach
    await migrator.migrate('@openclawdsolana/schema-evolution-test-types-v1', schemaV2);
  ...
  it('should handle text to integer conversion with invalid data', async () => {
    ...
    await migrator.migrate('@openclawdsolana/schema-evolution-test-text-to-int-v1', schemaV1);
    ...
    const check = await migrator.checkMigration(
      '@openclawdsolana/schema-evolution-test-text-to-int-v1',
      schemaV2
    );
    ...
    try {
      await migrator.migrate('@openclawdsolana/schema-evolution-test-text-to-int-v1', schemaV2);
    } catch (error) {
  ...
  it('should handle boolean to text and back conversions', async () => {
    ...
    await migrator.migrate('@openclawdsolana/schema-evolution-test-bool-v1', schemaV1);
    ...
    await migrator.migrate('@openclawdsolana/schema-evolution-test-bool-v1', schemaV2);
