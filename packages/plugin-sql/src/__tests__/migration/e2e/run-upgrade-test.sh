...
OPENCLAWD_REPO_DIR="$(cd "$SCRIPT_DIR/../../../../../.." && pwd)"
...
cat > "$TEST_PROJECT_DIR/package.json" << EOF
{
  ...
  "type": "module",
  "scripts": {
    "start": "openclawd start --character=./character.json",
    "seed": "bun run seed-data.ts"
  },
  "dependencies": {
    "@openclawdsolana/cli": "${SOURCE_VERSION}",
    "@openclawdsolana/plugin-bootstrap": "${SOURCE_VERSION}",
    "@openclawdsolana/plugin-sql": "${SOURCE_VERSION}"
  }
}
EOF
...
cat > "$TEST_PROJECT_DIR/character.json" << 'EOF'
{
  ...
  "plugins": ["@openclawdsolana/plugin-bootstrap", "@openclawdsolana/plugin-sql"]
}
EOF
...
echo -e "${GREEN}  Using workspace-linked packages from: $OPENCLAWD_REPO_DIR/packages/*${NC}"
...
cat > "$TEST_PROJECT_DIR/package.json" << EOF
{
  ...
  "workspaces": [
    "$OPENCLAWD_REPO_DIR/packages/*"
  ],
  "dependencies": {
    "@openclawdsolana/cli": "workspace:*",
    "@openclawdsolana/plugin-bootstrap": "workspace:*",
...
echo -e "${BLUE}  Running: bun run start (using workspace-linked @openclawdsolana/plugin-sql)${NC}"
