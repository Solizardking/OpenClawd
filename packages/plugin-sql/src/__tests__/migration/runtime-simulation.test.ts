...
describe('Runtime Simulation - Full Migration Flow', () => {
  ...
  it('should perform complete migration flow as in runtime', async () => {
    ...
    console.log('\n' + '='.repeat(80));
    console.log('STEP 2: Migrate Core Schema (@openclawdsolana/plugin-sql)');
    console.log('='.repeat(80));

    // Migrate core schema (this is what plugin-sql does on initialization)
    await migrator.migrate('@openclawdsolana/plugin-sql', coreSchema, {
      verbose: true,
    });
    ...
    const coreMigrationCheck = await db.execute(sql`
      ...
      WHERE plugin_name = '@openclawdsolana/plugin-sql'
    ...
    const coreStatus = await migrator.getStatus('@openclawdsolana/plugin-sql');
    const polymarketStatus = await migrator.getStatus('polymarket');

    console.log('\n📈 Migration Status:');
    console.log('Core (@openclawdsolana/plugin-sql):');
    ...
    await migrator2.migrate('@openclawdsolana/plugin-sql', coreSchema, {
      verbose: false,
    });
  ...
  it('should handle plugin registration order correctly', async () => {
    ...
    expect(firstMigration.plugin_name).toBe('@openclawdsolana/plugin-sql');
