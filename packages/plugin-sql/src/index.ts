...
import { type IAgentRuntime, type Plugin, logger, stringToUuid } from '@openclawdsolana/core';
 ...
 * @param {string} [config.dataDir] - The directory where data is stored. Defaults to "./.openclawd/.openclawddb".
  ...
  if (config.postgresUrl) {
    if (!globalSingletons.postgresConnectionManager) {
      ...
      if (dataIsolationEnabled) {
        const rlsServerIdString = process.env.OPENCLAWD_SERVER_ID;
        if (!rlsServerIdString) {
          throw new Error(
            '[Data Isolation] ENABLE_DATA_ISOLATION=true requires OPENCLAWD_SERVER_ID environment variable'
          );
        }
