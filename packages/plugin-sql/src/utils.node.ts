...
export function resolvePgliteDir(dir?: string, fallbackDir?: string): string {
  ...
  const base =
    ...
    fallbackDir ??
    (monoPath ? path.join(monoPath, '.openclawd', '.openclawddb') : undefined) ??
    path.join(process.cwd(), '.openclawd', '.openclawddb');

  const resolved = expandTildePath(base);
  const legacyPath = path.join(process.cwd(), '.openclawddb');
  if (resolved === legacyPath) {
    const newPath = path.join(process.cwd(), '.openclawd', '.openclawddb');
