...
export class RuntimeMigrator {
   ...
   * @openclawdsolana/plugin-sql uses 'public' schema (core application)
  ...
  private getExpectedSchemaName(pluginName: string): string {
    ...
    if (pluginName === '@openclawdsolana/plugin-sql') {
      return 'public';
    }
  ...
  private validateSchemaUsage(pluginName: string, snapshot: SchemaSnapshot): void {
    ...
    const isCorePLugin = pluginName === '@openclawdsolana/plugin-sql';
    ...
    for (const table of Object.values(snapshot.tables)) {
      ...
      if (isCorePLugin && actualSchema !== 'public') {
        logger.warn(
          {
            src: 'plugin:sql',
            pluginName: '@openclawdsolana/plugin-sql',
      ...
      if (dataLossCheck.hasDataLoss) {
        ...
        if (!allowDestructive) {
          ...
          logger.error(
            ...
            'Destructive migration blocked - set OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS=true or use force option'
          );

          const errorMessage = isProduction
            ? `Destructive migration blocked in production for ${pluginName}. Set OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS=true or use drizzle-kit.`
            : `Destructive migration blocked for ${pluginName}. Set OPENCLAWD_ALLOW_DESTRUCTIVE_MIGRATIONS=true to proceed.`;
