...
function getWalletClientImport(type: PluginOptions["type"]): string {
    switch (type) {
        case "evm":
            return 'import { EVMWalletClient } from "@openclawdsolana/wallet-evm";';
        case "solana":
            return 'import { SolanaWalletClient } from "@openclawdsolana/wallet-solana";';
        case "any":
            return 'import { WalletClientBase } from "@openclawdsolana/core";';
    }
}
...
function getDependencies(type: PluginOptions["type"]): Record<string, string> {
    ...
    switch (type) {
        case "evm":
            deps["@openclawdsolana/wallet-evm"] = "workspace:*";
            break;
        case "solana":
            deps["@openclawdsolana/wallet-solana"] = "workspace:*";
            break;
    }
...
function createPackageJson(options: PluginOptions): string {
    ...
    const packageJson = {
        name: `@openclawdsolana/plugin-${name}`,
        version: "0.1.0",
        ...
        repository: {
            type: "git",
            url: "git+https://github.com/openclawd/openclawd.git",
        },
        license: "MIT",
        bugs: {
            url: "https://github.com/openclawd/openclawd/issues",
        },
...
function createParametersContent(): string {
    return `import { createToolParameters } from "@openclawdsolana/core";
import { z } from "zod";
...
function createServiceContent(options: PluginOptions): string {
    ...
    return `import { Tool } from "@openclawdsolana/core";
${walletClientImport}
...
function createPluginContent(options: PluginOptions): string {
    ...
    return `import { PluginBase } from "@openclawdsolana/core";
...
# ${className} OpenClawd Plugin

Brief description of the ${className} plugin and what it does

## Installation
\`\`\`bash
npm install @openclawdsolana/plugin-${name}
yarn add @openclawdsolana/plugin-${name}
pnpm add @openclawdsolana/plugin-${name}
\`\`\`

## Usage
\`\`\`typescript
import { ${functionName} } from '@openclawdsolana/plugin-${name}';
...
program
    .name("createPlugin")
    .description("Create a new OpenClawd plugin")
