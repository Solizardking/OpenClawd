...
import { OpenClawdClient } from '../client';
...
describe('OpenClawdClient', () => {
  ...
  it('should create client with all services', () => {
    const client = new OpenClawdClient(config);
  ...
  it('should create client using static create method', () => {
    const client = OpenClawdClient.create(config);

    expect(client).toBeInstanceOf(OpenClawdClient);
    expect(client.agents).toBeInstanceOf(AgentsService);
  });
