...
import { useOpenClawdAgents } from '@/hooks/use-openclawd';
...
export function ServerManagement({ open, onOpenChange }: ServerManagementProps) {
  ...
  const { agents } = useOpenClawdAgents();
    ...
    const loadServerAgents = async () => {
      ...
      for (const server of serversData.data.servers) {
        try {
          ...
          const response = await openclawdClient.agents.getAgentsForMessageServer(server.id);
          if (response.success) {
  ...
  const handleAddAgentToServer = async () => {
    ...
    try {
      const openclawdClient = getOpenClawdClient();
      await openclawdClient.agents.addAgentToMessageServer(selectedServerId, selectedAgentId);
  ...
  const handleRemoveAgentFromServer = async (serverId: UUID, agentId: UUID) => {
    ...
    try {
      const openclawdClient = getOpenClawdClient();
      await openclawdClient.agents.removeAgentFromMessageServer(serverId, agentId);
