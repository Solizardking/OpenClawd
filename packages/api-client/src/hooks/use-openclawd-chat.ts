...
import { ChannelType, type UUID, type Media } from '@openclawdsolana/core';
import type { TransportType } from '@openclawdsolana/api-client';
...
interface UseOpenClawdChatOptions {
 ...
 * ```tsx
 * // WebSocket (default)
 * const { messages, sendMessage, isTyping } = useOpenClawdChat({
 *   channelId,
 *   serverId,
 *   agentId,
 * });
 ...
 *
 * // HTTP sync
 * const { messages, sendMessage } = useOpenClawdChat({
 *   channelId,
 *   serverId,
 *   agentId,
 *   transport: 'http',
 ...
 * ```
 */
export function useOpenClawdChat({
  channelId,
  serverId,
  agentId,
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        clientLogger.error('[useOpenClawdChat] Error sending message:', error);
