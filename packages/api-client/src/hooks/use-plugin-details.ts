...
const CORE_PLUGINS = ['@openclawdsolana/plugin-bootstrap', '@openclawdsolana/plugin-sql'];
...
/**
 * Convert plugin name for registry lookup - handles both @openclawd and @openclawd-plugins formats
 */
function getRegistryPluginName(pluginName: string): string {
  // If already in @openclawd-plugins format, return as is
  if (pluginName.startsWith('@openclawd-plugins/')) {
    return pluginName;
  }
  // Convert @openclawd to @openclawd-plugins format
  return pluginName.replace('@openclawdsolana/', '@openclawd-plugins/');
}
...
function getGitHubRepoPath(
  ...
  if (repoMatch) {
    ...
    // Try to parse as a simple owner/repo format (e.g., "openclawd-plugins/plugin-google-genai")
...
async function fetchPluginPackageJson(
  ...
  const packageName = pluginName.replace('@openclawdsolana/', '');
...
function extractRequiredSecrets(
  ...
  // Try to get secrets from openclawd.secrets
  if (packageJson?.openclawd?.secrets) {
    return packageJson.openclawd.secrets.filter((secret) => secret.required);
  }

  // Legacy format - convert string array to PluginSecret array
  if (packageJson?.openclawd?.requiredSecrets) {
    return packageJson.openclawd.requiredSecrets.map((secretName) => ({
      name: secretName,
      required: true,
    }));
  }
