import { Character, Content, Metadata } from '@openclawdsolana/core';
import { usePlugins } from '@/hooks/use-plugins';

const PROVIDER_PLUGIN_MAPPINGS: Record<string, string> = {
  google: '@openclawdsolana/plugin-google-genai',
  llama_local: '@openclawdsolana/plugin-ollama',
  // extend as needed
};
...
const ESSENTIAL_PLUGINS = ['@openclawdsolana/plugin-sql', '@openclawdsolana/plugin-bootstrap'];
...
export function useConvertCharacter() {
  ...
  const matchPlugins = (v1: V1Character): string[] => {
    ...
    if (Array.isArray(v1.clients)) {
      for (const client of v1.clients) {
        ...
        if (mapped && availablePlugins.includes(mapped)) {
          ...
          const constructed = `@openclawdsolana/plugin-${lower}`;
    ...
    if (typeof v1.modelProvider === 'string') {
      ...
      if (!providerMatched) {
        if (availablePlugins.includes('@openclawdsolana/plugin-openai')) {
          matched.add('@openclawdsolana/plugin-openai');
        }
      }
    } else {
