import type { Agent, UUID } from '@openclawdsolana/core';
 ...
 * @example
 * ```tsx
 * const { agent, plugins, isConnected, start, stop } = useOpenClawdAgent(agentId);
 * ```
 */
export function useOpenClawdAgent(agentId: UUID | undefined) {
 ...
 * @example
 * ```tsx
 * const { agents, start, stop } = useOpenClawdAgents();
 * ```
 */
export function useOpenClawdAgents() {
  const { status } = useConnection();
