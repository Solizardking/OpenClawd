...
import { type UUID, ChannelType } from '@openclawdsolana/core';
...
export function useGetOrCreateDmChannel() {
  ...
  return useMutation({
    mutationFn: async (targetUserId: UUID) => {
      ...
      const result = await openclawdClient.messaging.getOrCreateDmChannel({
...
export function useDmChannelsForAgent(
  ...
  return useQuery<MessageChannel[]>({
    ...
    queryFn: async () => {
      ...
      const openclawdClient = getOpenClawdClient();
      const result = await openclawdClient.messaging.getMessageServerChannels(messageServerId);
...
export function useCreateDmChannel() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const openclawdClient = getOpenClawdClient();
      const result = await openclawdClient.messaging.createGroupChannel({
      ...
      return result; // Direct result from OpenClawdClient
    },
