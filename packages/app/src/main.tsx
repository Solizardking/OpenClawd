...
function OpenClawdWrapper() {
  ...
  useEffect(() => {
    ...
    startServer().catch((err: Error) => {
      console.error('Failed to start OpenClawd server:', err);
      setStatus('error');
      setError(`Failed to start OpenClawd server: ${err.message}`);
    });
  ...
  return (
    ...
    >
      {status === 'error' ? (
        ...
        <>
          <h2>Starting OpenClawd Server...</h2>
