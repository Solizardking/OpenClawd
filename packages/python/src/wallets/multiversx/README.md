# MultiversX Wallet for OpenClawd

A Python implementation of a MultiversX wallet for the OpenClawd, providing secure key management and transaction signing capabilities.

## Installation

```bash
poetry add openclawd-wallet-multiversx
```

## Usage

```python
from openclawd_wallets.multiversx import multiversx_wallet
...
```python
from openclawd_wallets.multiversx import send_egld
...
A full working example demonstrating all the capabilities of the MultiversX wallet and plugin using OpenClawd can be found in the gist below
