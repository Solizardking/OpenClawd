...
# Smolagents Adapter for OpenClawd

Integrate the more than +200 onchain tools of OpenClawd with [Smolagents](https://github.com/huggingface/smolagents), a framework for building agentic systems with small language models.

## Installation

```bash
poetry add openclawd-adapter-smolagents
```

## Usage

```python
# --- Setup OpenClawd Wallet and Plugins (Example: Solana + SPL Token) ---
import os
...
# --- Import Smolagents and the OpenClawd Adapter ---
from smolagents import OpenAIServerModel, ToolCallingAgent
from openclawd_adapters.smolagents import get_smolagents_tools

# --- Generate Smolagents Tools from OpenClawd ---
openclawd_smolagents_tools = get_smolagents_tools(
    wallet=wallet,
    plugins=[]
)

# --- Define Smolagents Agent using OpenClawd Tools ---
model = OpenAIServerModel(
    model_id="gpt-4o",
)

agent = ToolCallingAgent(
    tools=openclawd_smolagents_tools,
    model=model,
...
You can also create a Smolagents ToolCollection to manage your OpenClawd tools:

```python
from smolagents import ToolCollection, ToolCallingAgent

# Create the tools first
openclawd_tools = get_smolagents_tools(wallet=wallet, plugins=[])

# Create a custom tool collection with your OpenClawd tools
my_tool_collection = ToolCollection(
    tools=openclawd_tools
)
