...
def get_on_chain_tools(wallet: WalletClientBase, plugins: List[Any]) -> List[BaseTool]:
    """Create LangChain tools from OpenClawd tools.
    ...
    Returns:
        List of LangChain Tool instances configured with the OpenClawd tools
    """
    ...
    langchain_tools = []
    for t in tools:
        # Create a LangChain Tool for each OpenClawd tool
        tool = StructuredTool(
            name=t.name,
