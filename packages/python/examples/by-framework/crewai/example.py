...
from openclawd_wallets.solana import solana, SPL_TOKENS

# Load environment variables
load_dotenv()

# --- Setup OpenClawd Wallet and Plugins ---
print("Initializing Solana wallet and OpenClawd plugins...")
...
try:
    ...
    # Get CrewAI-compatible tools from OpenClawd adapter
    openclawd_crewai_tools = get_crewai_tools(wallet=wallet, plugins=[])
    if not openclawd_crewai_tools:
        print("Warning: No OpenClawd tools were loaded. Check adapter/plugin setup or tool compatibility.")
        # Optionally exit if tools are essential
        # exit(1)
    else:
        print(f"Successfully loaded {len(openclawd_crewai_tools)} OpenClawd tools for CrewAI.")
...
crypto_analyst = Agent(
    ...
    tools=openclawd_crewai_tools,  # Pass the generated OpenClawd tools
