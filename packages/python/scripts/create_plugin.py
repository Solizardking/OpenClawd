...
openclawd = "^0.1.0"'''
    ...
    if is_evm:
        dependencies += '\nopenclawd-wallet-evm = "^0.1.0"'
    elif is_solana:
        dependencies += '\nopenclawd-wallet-solana = "^0.1.0"'
    
    # Dev dependencies
    dev_dependencies = '''ruff = "^0.8.6"
openclawd = { path = "../../openclawd", develop = true }'''
    
    # Add chain-specific dev dependencies
    if is_evm:
        dev_dependencies += '\nopenclawd-wallet-evm = { path = "../../wallets/evm", develop = true }'
    elif is_solana:
        dev_dependencies += '\nopenclawd-wallet-solana = { path = "../../wallets/solana", develop = true }'
    
    toml_content = f'''[tool.poetry]
name = "openclawd-plugin-{plugin_name}"
version = "0.1.0"
description = "OpenClawd plugin for {plugin_name}"
authors = ["Your Name <your_email@example.com>"]
readme = "README.md"
keywords = ["openclawd", "sdk", "agents", "ai", "{plugin_name}"]
homepage = "https://openclawd.dev/"
repository = "https://github.com/openclawd/openclawd"
packages = [
    {{ include = "openclawd_plugins/{plugin_name}" }},
]
...
[tool.poetry.urls]
"Bug Tracker" = "https://github.com/openclawd/openclawd/issues"
...
def create_parameters_file(openclawd_plugins_dir: Path, plugin_name: str) -> None:
    """Create the parameters.py file with example parameters in the openclawd_plugins directory."""
...
'''
    ...
    with open(openclawd_plugins_dir / "parameters.py", "w") as f:
...
def create_service_file(openclawd_plugins_dir: Path, plugin_name: str, is_evm: bool, is_solana: bool = False) -> None:
    ...
    service_content = '''from openclawd.decorators.tool import Tool
from .parameters import ExampleQueryParameters, ExampleActionParameters
'''

    # Add chain-specific imports if needed
    if is_evm:
        service_content += '''from openclawd_wallets.evm import EVMWalletClient

'''
    elif is_solana:
        service_content += '''from openclawd_wallets.solana import SolanaWalletClient
...
'''
    ...
    with open(openclawd_plugins_dir / "service.py", "w") as f:
        f.write(service_content)

def create_init_file(openclawd_plugins_dir: Path, plugin_name: str, is_evm: bool, is_solana: bool = False) -> None:
...
'''.format(
    ...
    with open(openclawd_plugins_dir / "__init__.py", "w") as f:
        f.write(init_content)

def main():
    """Main function to create a new OpenClawd plugin.
    
    This script generates a new plugin package for the OpenClawd with a standardized structure.
    ...
    2. openclawd_plugins directory with:
    ...
    """
    parser = argparse.ArgumentParser(description="Create a new OpenClawd plugin.")
    ...
    # Create openclawd_plugins directory and its contents
    openclawd_plugins_dir = plugin_dir / "openclawd_plugins" / plugin_name
    openclawd_plugins_dir.mkdir(parents=True, exist_ok=True)
    
    # Create all plugin files
    create_parameters_file(openclawd_plugins_dir, plugin_name)
    create_service_file(openclawd_plugins_dir, plugin_name, is_evm, is_solana)
    create_init_file(openclawd_plugins_dir, plugin_name, is_evm, is_solana)
    ...
    readme_content = f"""# {plugin_name} Plugin for OpenClawd

A plugin for the OpenClawd that provides {plugin_name} functionality.

## Installation

```bash
# Install the plugin
poetry add openclawd-plugin-{plugin_name}
...
```python
from openclawd_plugins.{plugin_name} import {python_name}, {class_base}PluginOptions
