...
export interface HonchoWebhookEntry {
  index: 1 | 2 | 3 | 4;
  url: string;
  secret: string;
  workspace?: string;
...
function envReasoning(name: string, fallback: ReasoningLevel): ReasoningLevel {
...
function loadWebhook(i: 1 | 2 | 3 | 4): HonchoWebhookEntry | null {
  const url =
    process.env[`HONCHO_WEBHOOK${i}_URL`] ||
    process.env[`HONCHO_WEBHOOK_URL${i}`] || // legacy
    "";
  const secret =
    process.env[`HONCHO_WEBHOOK${i}_SECRET`] ||
    process.env[`HONCHO_WEBHOOKSECRET${i}`] || // legacy (no underscore)
    process.env[`HONCHO_WEBHOOK_URL${i}_SECRET`] || // legacy
    "";
  const workspace =
    process.env[`HONCHO_WEBHOOK${i}_WORKSPACE`] || undefined;
  if (!url) return null;
 ...
 */
export function loadHonchoConfig(overrides: Partial<HonchoConfig> = {}): HonchoConfig {
  ...
  return {
    ...
    contextSummary:
      overrides.contextSummary ?? envBool("HONCHO_CONTEXT_SUMMARY", true),
    syncMessages:
      overrides.syncMessages ?? envBool("HONCHO_SYNC_MESSAGES", true),
    webhookSecret:
      overrides.webhookSecret ?? process.env.HONCHO_WEBHOOK_SECRET ?? "",
    webhooks:
      overrides.webhooks ??
      ([1, 2, 3, 4] as const)
        .map(loadWebhook)
        .filter((w): w is HonchoWebhookEntry => w !== null),
  };
}
 ...
 */
export function assertHonchoUsable(cfg: HonchoConfig): void {
  if (!cfg.enabled) {
    throw new Error(
      "Honcho is disabled. Set HONCHO_ENABLED=true and HONCHO_API_KEY to enable.",
    );
  }
  if (!cfg.apiKey) {
    throw new Error(
      "HONCHO_API_KEY is not set. Get one at https://app.honcho.dev (free $100 credits).",
    );
  }
  if (!cfg.workspaceId) {
...
export function pickWebhookForWorkspace(
  cfg: HonchoConfig,
  workspaceId: string,
): HonchoWebhookEntry | null {
