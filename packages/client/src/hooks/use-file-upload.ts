...
import { getContentTypeFromMimeType } from '@openclawdsolana/core';
import { UUID, Media, ChannelType } from '@openclawdsolana/core';
        ...
        try {
          const uploadResult =
            chatType === ChannelType.DM && agentId
              ? await openclawdClient.media.uploadAgentMedia(agentId, {
                  file: fileData.file,
                  filename: fileData.file.name,
                })
              : await openclawdClient.media.uploadChannelMedia(channelId!, fileData.file);
