...
export function useAddAgentToMessageServer() {
  ...
  return useMutation({
    mutationFn: async ({ messageServerId, agentId }: { messageServerId: UUID; agentId: UUID }) => {
      ...
      return await openclawdClient.agents.addAgentToMessageServer(messageServerId, agentId);
    },
...
export function useRemoveAgentFromMessageServer() {
  ...
  return useMutation({
    mutationFn: async ({ messageServerId, agentId }: { messageServerId: UUID; agentId: UUID }) => {
      const openclawdClient = getOpenClawdClient();
      return await openclawdClient.agents.removeAgentFromMessageServer(messageServerId, agentId);
    },
    onSuccess: (_data, variables) => {
      // Invalidate server agents query
      queryClient.invalidateQueries({
