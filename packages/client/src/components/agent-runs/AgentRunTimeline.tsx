...
import type { UUID } from '@openclawdsolana/core';
import React, { useMemo } from 'react';
import { openclawdSpanAdapter } from '@/lib/openclawd-span-adapter';
import { Loader2 } from 'lucide-react';
import { useQueries } from '@tanstack/react-query';
import { getOpenClawdClient } from '@/lib/api-client-config';
import type { RunDetail } from '@openclawdsolana/api-client';
...
export const AgentRunTimeline: React.FC<AgentRunTimelineProps> = ({ agentId }) => {
  ...
  const runDetailQueries = useQueries({
    queries: runs.map((run) => ({
      ...
      queryFn: async () => openclawdClient.runs.getRun(agentId, run.runId),
      enabled: Boolean(agentId && run.runId),
      staleTime: 30000,
    })),
  });

  // Convert OpenClawd runs to Agent Prism format
  const traceViewerData: TraceViewerData[] = useMemo(() => {
    return runs
      .map((run, index) => {
        ...
        return {
          traceRecord: openclawdSpanAdapter.convertRunSummaryToTraceRecord(run),
          spans: openclawdSpanAdapter.convertRunDetailToTraceSpans(detailQuery.data as RunDetail),
        };
      })
