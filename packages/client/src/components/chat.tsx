...
import { useOpenClawdChat } from '@/hooks/use-openclawd-chat';
...
import type { Agent, Media, UUID } from '@openclawdsolana/core';
import {
  AgentStatus,
  ChannelType,
...
export default function Chat({
    ...
    async (agentIdForNewChannel: UUID | undefined) => {
      ...
      if (latestChannel) {
        try {
          ...
          const latestMessages = await openclawdClient.messaging.getChannelMessages(latestChannel.id, {
            limit: 30,
          });
  ...
  const handleDeleteCurrentDmChannel = useCallback(() => {
    ...
    confirm(
      ...
      async () => {
        ...
        try {
          const openclawdClient = getOpenClawdClient();
          await openclawdClient.messaging.deleteChannel(channelToDelete.id);
  ...
  const updateChatTitle = async () => {
    ...
    const openclawdClient = getOpenClawdClient();
    const data = await openclawdClient.messaging.generateChannelTitle(
      finalChannelIdForHooks,
      contextId
    );

    const title = data?.title;
    const participants = await openclawdClient.messaging.getChannelParticipants(
      chatState.currentDmChannelId
    );
    if (title && participants) {
      ...
      await openclawdClient.messaging.updateChannel(finalChannelIdForHooks, {
        name: title,
                    ...
                    onClick: () => {
                      ...
                      confirm(
                        ...
                        async () => {
                          try {
                            const openclawdClient = getOpenClawdClient();
                            await openclawdClient.messaging.deleteChannel(channelIdToDelete);
                            toast({
                              title: 'Group Deleted',
