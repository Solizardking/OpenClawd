...
The official web client for OpenClawd agents, providing a modern React-based interface for interacting with OpenClawd agents.
...
```bash
bun add @openclawdsolana/client
```
...
```bash
# Run all tests with the OpenClawd UI automatically started
bun test
...
- Verifies that the OpenClawd web interface loads
...
The default configuration starts the OpenClawd UI automatically as part of the test run. This is handled by the `webServer` configuration in `playwright.config.ts`:
...
webServer: {
  ...
  timeout: 60000, // OpenClawd might take some time to start
},
```
...
2. Run tests in another terminal:
   ```bash
   cd openclawd/packages/client/ui-tests
...
- [OpenClawd API Documentation](https://docs.openclawd.com)
- [UI Component Library](https://ui.openclawd.com)
