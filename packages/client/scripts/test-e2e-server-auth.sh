...
if [ "$CI" = "true" ]; then
  ...
  while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if docker-compose -f docker-compose.test.yml exec -T postgres-test pg_isready -U openclawd_test >/dev/null 2>&1; then
  ...
  if docker-compose -f docker-compose.test.yml exec -T postgres-test psql -U openclawd_test -d openclawd_test -c "SELECT 1" >/dev/null 2>&1; then
...
export POSTGRES_URL="postgresql://openclawd_test:openclawd_test_password@localhost:5433/openclawd_test"
export OPENCLAWD_NONINTERACTIVE=true
export LOG_LEVEL=${LOG_LEVEL:-error}
# Enable data isolation and authentication for E2E tests
export ENABLE_DATA_ISOLATION=true
export OPENCLAWD_SERVER_ID="test-server-e2e"
