...
export type { SkillMetadata, SkillInstallSpec }
...
export function parseClawdisMetadata(frontmatter: ParsedSkillFrontmatter) {
  ...
  try {
    ...
    const config = parseAgentConfigSpec(clawdisObj.config)
    ...
    return parseArk(SkillMetadataSchema, metadata, 'Clawdis metadata')
  } catch {
    return undefined
  }
}
...
function parseAgentConfigSpec(input: unknown): AgentConfigSpec | undefined {
  if (!input || typeof input !== 'object') return undefined
  const raw = input as Record<string, unknown>
 ...
 */
function parseFrontmatterLevelDeclarations(frontmatter: ParsedSkillFrontmatter): SkillMetadata | undefined {
  const metadata: SkillMetadata = {}
  ...
  return Object.keys(metadata).length > 0
    ? parseArk(SkillMetadataSchema, metadata, 'Clawdis metadata')
    : undefined
}
