...
export interface ClawdRouterConfig {
  ...
  // ── Local model upstreams (Ollama / llama.cpp) ────────────────
  // Models prefixed `ollama/*`, `local/*`, or `llamacpp/*` bypass the
  // OpenRouter path and go straight to these OpenAI-compatible servers.
  localEnabled: boolean;
  localOllamaBaseUrl: string;       // Default http://127.0.0.1:11434/v1
  localOllamaApiKey: string;        // Usually empty; some proxies want "ollama"
  localLlamacppBaseUrl: string;     // Default http://127.0.0.1:8080/v1
  localLlamacppApiKey: string;

  // ── Network binding + Tailscale remote access ─────────────────
  // `bind` controls which interface the HTTP server listens on.
  //   loopback — 127.0.0.1 only (default; pairs with tailscale serve/funnel)
  //   all      — bind to 0.0.0.0 (LAN/VPN direct)
  bind: 'loopback' | 'all';
  tailscale: {
  ...
  // ── Tunnel hub (hosted mode only; fly.dev) ─────────────────────
  // When `hub.enabled` is true, this process attaches a tunnel hub to
  // its HTTP server at `hub.path` and accepts authenticated WS spokes.
  // Verification is delegated to the CF control plane (`hub.controlUrl`)
  // using the shared `hub.hubSecret`.
  hub: {
    enabled: boolean;
    controlUrl: string;       // e.g. https://clawdrouter.x402.workers.dev
    hubSecret: string;        // shared with the CF worker
