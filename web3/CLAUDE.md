# CLAUDE CODE CONFIGURATION - OPENCLAWD PROJECT

This file contains project-specific configuration and preferences for Claude Code when working with the OpenClawd codebase.

---

## PROJECT INFORMATION

- **Working Directory:** `/Users/{user}/Documents/GitHub/openclawd`
- **Git Repository:** Yes
- **Main Branch:** `develop`
...
OpenClawd is organized as a monorepo with the following key packages:

### Core Packages

- **`packages/core`** - `@openclawdsolana/core` - Foundation runtime, types, agents, and database
- **`packages/cli`** - `@openclawdsolana/cli` - Command-line interface and agent runtime
...
- **`packages/api-client`** - `@openclawdsolana/api-client` - Type-safe API client for OpenClawd server
...
- **`packages/create-openclawd`** - Project scaffolding tool
...
- Use `bun` for global installs: `bun install -g @openclawdsolana/cli`

### Workspace Dependencies

- **ALWAYS USE `workspace:*` FOR ALL `@openclawdsolana/` PACKAGE DEPENDENCIES**
- **NEVER USE HARDCODED VERSIONS** for internal monorepo packages
- **Example of CORRECT usage:**
  ```json
  {
    "dependencies": {
      "@openclawdsolana/core": "workspace:*",
      "@openclawdsolana/plugin-sql": "workspace:*",
      "@openclawdsolana/server": "workspace:*"
    }
  }
  ```
- **Example of INCORRECT usage:**
  ...
  {
    "dependencies": {
      "@openclawdsolana/core": "1.4.2", // ❌ Don't use hardcoded versions
      "@openclawdsolana/plugin-sql": "^1.4.0", // ❌ Don't use version ranges
      "@openclawdsolana/server": "latest" // ❌ Don't use version tags
    }
  }
  ```
...
- **The `openclawd` CLI** is built from `packages/cli`
- **INTENDED FOR:** Production use by developers/users of the project
- **DO NOT USE THE `openclawd` CLI WITHIN THE `openclawd` MONOREPO ITSELF**
- **The `openclawd` CLI is for external consumers, NOT internal monorepo development**
- **For monorepo development:** Use `bun` commands directly

### OpenClawd Test Command

The `openclawd test` command runs tests for OpenClawd projects and plugins:

```bash
openclawd test [path]           # Run all tests (component + e2e)
openclawd test -t component     # Run only component tests
openclawd test -t e2e          # Run only e2e tests
openclawd test --name "test"   # Filter tests by name (case sensitive)
openclawd test --skip-build    # Skip building before tests
openclawd test --port <port>   # Port for e2e tests
openclawd test --skip-type-check  # Skip TypeScript validation
```

**Test Types:**

- **Component Tests:** Unit tests via `bun test` - test individual modules/components in isolation
- **E2E Tests:** Full integration tests via OpenClawd TestRunner - test complete agent runtime with server, database, and plugins

**Context Support:**

- Works in both monorepo packages and standalone projects created with `openclawd create`
- Automatically detects project type and adjusts paths accordingly
- For plugins: Creates default OpenClawd character as test agent
- For projects: Uses agents defined in project configuration

**Note:** The test command does NOT run Cypress or other UI tests - only OpenClawd-specific tests
...
- **Central Dependency:** Everything depends on `@openclawdsolana/core` or `packages/core`
- **No Circular Dependencies:** Core cannot depend on other packages
- **Import Pattern:** Use `@openclawdsolana/core` in package code, `packages/core` in internal references
...
### CRITICAL: OpenClawd Component Clarifications
...
3. **Import Errors:** Verify correct use of `@openclawdsolana/core` vs `packages/core`
...
_This configuration file should be referenced at the start of any OpenClawd development session to ensure proper setup and adherence to project standards._
