...
/**
 * Creates a standardized Bun build configuration for OpenClawd packages
 */
export async function createOpenClawdBuildConfig(options: OpenClawdBuildOptions): Promise<BuildConfig> {
  ...
  // OpenClawd workspace packages that should typically be external
  const openclawdExternals = [
    '@openclawdsolana/core',
    '@openclawdsolana/server',
    '@openclawdsolana/client',
    '@openclawdsolana/api-client',
    '@openclawdsolana/plugin-*',
  ];
  ...
  const config: BuildConfig = {
    ...
    sourcemap,
    minify,
    external: [...nodeExternals, ...openclawdExternals, ...cleanExternals],
    plugins,
    naming: {
      entry: '[dir]/[name].[ext]',
      chunk: '[name]-[hash].[ext]',
...
export async function runBuild(options: BuildRunnerOptions & { isRebuild?: boolean }) {
  ...
  const config = await createOpenClawdBuildConfig(buildOptions);
