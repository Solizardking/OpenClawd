# OpenClawd Agent Project Development Guide for Claude

> **Optimized for Claude LLM** - Complete reference for building OpenClawd agent projects
...
| **Runtime**         | OpenClawd with plugin ecosystem |
| **Configuration**   | Character-based agent setup   |
| **Architecture**    | Plugin composition pattern    |

## 🏗️ Project Architecture

OpenClawd projects are **character-driven agent systems** that compose functionality through plugins:
...
└── 🚀 Runtime Orchestration (OpenClawd)
```
...
```typescript
// src/character.ts
import { Character } from '@openclawdsolana/core';
...
- Ask clarifying questions when requests are ambiguous`,
  ...
  plugins: [
    ...
    '@openclawdsolana/plugin-bootstrap', // Essential actions & handlers
    '@openclawdsolana/plugin-sql', // Memory & database management

    // REQUIRED: Model provider (choose one or more)
    '@openclawdsolana/plugin-openai', // GPT-4, GPT-3.5, etc.
    // "@openclawdsolana/plugin-anthropic", // Claude models
    // "@openclawdsolana/plugin-groq",      // Fast inference

    // OPTIONAL: Communication channels
    // "@openclawdsolana/plugin-discord",   // Discord integration
    // "@openclawdsolana/plugin-twitter",   // Twitter/X integration
    // "@openclawdsolana/plugin-telegram",  // Telegram bot

    // OPTIONAL: Specialized capabilities
    // "@openclawdsolana/plugin-solana",    // Solana blockchain
    // "@openclawdsolana/plugin-evm",       // Ethereum/EVM chains
  ],
...
export const cryptoCharacter: Character = {
  ...
  plugins: [...baseCharacter.plugins, '@openclawdsolana/plugin-solana', '@openclawdsolana/plugin-evm'],
};
```
...
| --------------------------- | ------------------------------ | ------------ |
| `@openclawdsolana/plugin-bootstrap` | Core actions, message handling | **REQUIRED** |
| `@openclawdsolana/plugin-sql`       | Memory, database management    | **REQUIRED** |

### Model Provider Plugins (Choose One or More)

| Plugin                      | Models                   | Use Case                       |
| --------------------------- | ------------------------ | ------------------------------ |
| `@openclawdsolana/plugin-openai`    | GPT-4, GPT-3.5, GPT-4o   | General purpose, reliable      |
| `@openclawdsolana/plugin-anthropic` | Claude 3.5 Sonnet, Haiku | Reasoning, analysis            |
| `@openclawdsolana/plugin-groq`      | Llama, Mixtral           | Fast inference, cost-effective |
| `@openclawdsolana/plugin-llama`     | Local Llama models       | Privacy, offline operation     |

### Communication Plugins (Optional)

```bash
# Social platforms
bun add @openclawdsolana/plugin-discord      # Discord bot integration
bun add @openclawdsolana/plugin-twitter      # Twitter/X posting & monitoring
bun add @openclawdsolana/plugin-telegram     # Telegram bot functionality

# Web interfaces
bun add @openclawdsolana/plugin-web          # Web UI for agent interaction
bun add @openclawdsolana/plugin-rest         # REST API endpoints
```
...
```bash
# Blockchain & Crypto
bun add @openclawdsolana/plugin-solana       # Solana transactions & data
bun add @openclawdsolana/plugin-evm          # Ethereum & EVM chains

# Data & Tools
bun add @openclawdsolana/plugin-web-search   # Web search capabilities
bun add @openclawdsolana/plugin-image        # Image generation & analysis
```
...
openclawd start --character characters/development.json
```
...
{
  "scripts": {
    "start": "openclawd start",
    "dev": "openclawd start --dev",
    "test": "openclawd test",
...
openclawd start --character characters/development.json --dev

# Start with debug logging
LOG_LEVEL=debug openclawd start --dev
```
...
openclawd test --filter "action-name"

# Test with specific character
openclawd test --character characters/test.json
```
...
bun run build
NODE_ENV=production openclawd start --character characters/production.json
```
...
```typescript
// src/plugin.ts
import { Plugin, Action, ActionResult, Service } from '@openclawdsolana/core';
...
pm2 start "openclawd start --character characters/production.json" --name "my-agent"
```
...
openclawd start --character characters/crypto.json
```
...
openclawd test --filter "openai" --verbose

# Check plugin loading
openclawd start --dry-run --verbose

# Database troubleshooting
openclawd db:status
openclawd db:migrate
```
...
Share useful patterns and plugins with the OpenClawd community.
...
**🎉 Ready to build your OpenClawd agent!** Start with `openclawd start --dev` and let your agent evolve with your needs.
