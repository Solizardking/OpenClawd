...
describe('Project Starter Character Plugin Ordering', () => {
  ...
  describe('Core Plugin Ordering', () => {
    it('should always include SQL plugin first', () => {
      expect(character.plugins[0]).toBe('@openclawdsolana/plugin-sql');
    });

    it('should include bootstrap plugin by default (not ignored)', () => {
      expect(character.plugins).toContain('@openclawdsolana/plugin-bootstrap');
    });
    ...
    it('should exclude bootstrap plugin when IGNORE_BOOTSTRAP is set', () => {
      ...
      // The actual dynamic behavior is tested in the CLI tests with getOpenClawdCharacter()
      expect(character.plugins).toContain('@openclawdsolana/plugin-bootstrap');
  ...
  describe('Plugin Structure and Ordering', () => {
    it('should structure embedding plugins after text-only plugins', () => {
      ...
      const sqlIndex = plugins.indexOf('@openclawdsolana/plugin-sql');
  ...
  describe('Plugin Categories and Ordering', () => {
    it('should categorize plugins correctly', () => {
      ...
      // Core plugins
      expect(plugins).toContain('@openclawdsolana/plugin-sql');

      // Text-only AI plugins (no embedding support)
      const textOnlyPlugins = ['@openclawdsolana/plugin-anthropic', '@openclawdsolana/plugin-openrouter'];

      // Embedding-capable AI plugins
      const embeddingPlugins = [
        '@openclawdsolana/plugin-openai',
        '@openclawdsolana/plugin-ollama',
        '@openclawdsolana/plugin-google-genai',
      ];
      ...
      const platformPlugins = [
        ...
        '@openclawdsolana/plugin-telegram',
      ];

      // Bootstrap plugin
      const bootstrapPlugin = '@openclawdsolana/plugin-bootstrap';
      ...
      expect(plugins).toContain('@openclawdsolana/plugin-bootstrap');
    });
    ...
    it('should maintain proper ordering between plugin categories', () => {
      ...
      const sqlIndex = plugins.indexOf('@openclawdsolana/plugin-sql');
      const bootstrapIndex = plugins.indexOf('@openclawdsolana/plugin-bootstrap');
  ...
  describe('Environment Variable Integration', () => {
    it('should have conditional logic for all AI providers', () => {
      ...
      const hasAiProviders = plugins.some((plugin) =>
        [
          ...
          '@openclawdsolana/plugin-openai',
          '@openclawdsolana/plugin-openrouter',
          '@openclawdsolana/plugin-ollama',
          '@openclawdsolana/plugin-google-genai',
        ].includes(plugin)
      );
    ...
    it('should include proper conditional checks for Twitter', () => {
      ...
      expect(plugins).not.toContain('@openclawdsolana/plugin-twitter');
    });
  ...
  describe('Plugin Logic Validation', () => {
    it('should follow the expected plugin ordering pattern', () => {
      ...
      expect(plugins[0]).toBe('@openclawdsolana/plugin-sql'); // SQL always first
      expect(plugins).toContain('@openclawdsolana/plugin-bootstrap'); // Bootstrap present
    ...
    it('should have valid plugin names', () => {
      ...
      plugins.forEach((plugin) => {
        ...
        expect(plugin).toMatch(/^@openclawd\/plugin-/);
      });
    });
