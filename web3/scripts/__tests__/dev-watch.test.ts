...
const DEV_WATCH_CONTENT = await Bun.file(DEV_SCRIPT).text();
const readDevWatchScript = () => DEV_WATCH_CONTENT;
const TEST_DIR = path.join(tmpdir(), `openclawd-dev-watch-test-${Date.now()}`);
...
describe('Hot Reload Functionality', () => {
  ...
  test('watched directories should include all CLI dependencies', () => {
    ...
    const devWatchContent = readDevWatchScript();
