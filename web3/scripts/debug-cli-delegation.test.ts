...
describe('CLI Delegation Debug Tool', () => {
  ...
  describe('Local CLI Detection', () => {
    it('should detect when local CLI exists', async () => {
      ...
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts'], {
        cwd: '/home/runner/work/openclawd/openclawd',
      ...
      expect(stdout).toContain('node_modules/@openclawdsolana/cli/dist/index.js');
    });
  ...
  describe('Environment Variable Detection', () => {
    ...
    it('should detect problematic OPENCLAWD_TEST_MODE', async () => {
      mockExistsSync.mockReturnValue(true);
      process.argv = ['bun', 'debug-cli-delegation.ts'];
      ...
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts'], {
        cwd: '/home/runner/work/openclawd/openclawd',
        env: { ...process.env, OPENCLAWD_TEST_MODE: 'true' },
        stdout: 'pipe',
        stderr: 'pipe',
      });
      ...
      expect(stdout).toContain('OPENCLAWD_TEST_MODE: true');
      expect(stdout).toContain('Test/CI environment detected');
    });
  ...
  describe('Process Arguments Detection', () => {
    it('should detect test command', async () => {
      ...
      const { spawn } = await import('bun');
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts', 'test'], {
        cwd: '/home/runner/work/openclawd/openclawd',
        env: { ...process.env, NODE_ENV: undefined },
        stdout: 'pipe',
        stderr: 'pipe',
    ...
    it('should detect version commands', async () => {
      ...
      const { spawn } = await import('bun');
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts', '--version'], {
        cwd: '/home/runner/work/openclawd/openclawd',
        env: { ...process.env, NODE_ENV: undefined },
        stdout: 'pipe',
        stderr: 'pipe',
  ...
  describe('Package.json Analysis', () => {
    it('should analyze package.json correctly', async () => {
      ...
      expect(stdout).toContain('Has openclawd dependency: ✅');
    });
  ...
  describe('Help Display', () => {
    it('should display help with --help flag', async () => {
      const { spawn } = await import('bun');
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts', '--help'], {
        cwd: '/home/runner/work/openclawd/openclawd',
        stdout: 'pipe',
        stderr: 'pipe',
      });
      ...
      expect(stdout).toContain('OpenClawd CLI Delegation Debug Tool');
    ...
    it('should display help with -h flag', async () => {
      const { spawn } = await import('bun');
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts', '-h'], {
        cwd: '/home/runner/work/openclawd/openclawd',
        stdout: 'pipe',
        stderr: 'pipe',
      });
  ...
  describe('Auto-fix Mode', () => {
    it('should attempt to install CLI when --fix is used', async () => {
      ...
      const { spawn } = await import('bun');
      const proc = spawn([BUN_BIN, 'scripts/debug-cli-delegation.ts', '--fix'], {
        cwd: '/home/runner/work/openclawd/openclawd',
        env: { ...process.env, NODE_ENV: undefined },
        stdout: 'pipe',
        stderr: 'pipe',
      ...
      expect(stdout).toContain('Installing @openclawdsolana/cli locally');
    });
  });
  ...
  describe('Recommendations', () => {
    it('should recommend bun install when no local CLI', async () => {
      ...
      expect(stdout).toContain('bun install @openclawdsolana/cli');
    });
  ...
  describe('Quick Test', () => {
    it('should show quick test when delegation should work', async () => {
      ...
      expect(stdout).toContain('openclawd --help');
      expect(stdout).toContain('Using local @openclawdsolana/cli installation');
    });
  });
