...
metadata: { "openclawd": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## OpenClawd Runtime Mapping

- **Task tool** in the upstream spec == OpenClawd `sessions_spawn`
- **File I/O** == OpenClawd `read`/`write`
- **Remote fetch** == OpenClawd `web_fetch` (or `exec` with curl when POST is required)
