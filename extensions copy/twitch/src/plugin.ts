/**
 * Twitch channel plugin for OpenClawd.
 *
...
import { buildChannelConfigSchema } from "openclawd/plugin-sdk";
import type {
 ...
 * for OpenClawd. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenClawdConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenClawdConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenClawdConfig): boolean => {
