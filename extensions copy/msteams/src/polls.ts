...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openclawdPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openclawd", "pollId"]) ??
    readNestedString(value, ["openclawd", "poll", "id"]) ??
    readNestedString(value, ["data", "openclawdPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openclawd", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openclawdPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openclawd poll vote",
          displayText: "Vote recorded",
          value: { openclawdPollId: pollId, pollId },
        },
      },
    ],
  };
