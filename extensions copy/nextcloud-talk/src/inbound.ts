...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OpenClawdConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OpenClawdConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OpenClawdConfig);
