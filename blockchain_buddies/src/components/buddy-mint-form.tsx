...
export function BuddyMintForm() {
  ...
  const [synergyRole, setSynergyRole] = useState("companion");
  const [automationLevel, setAutomationLevel] = useState("watcher");
  const [stakingStatus, setStakingStatus] = useState("planned");
  const [stakeAmountLamports, setStakeAmountLamports] = useState("");
  const [buddyWallet, setBuddyWallet] = useState("");
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-medium text-foreground">
          ...
          <select
            value={synergyRole}
            onChange={(event) => setSynergyRole(event.target.value)}
            className="h-11 rounded-2xl border border-border-base bg-background px-4 text-sm outline-none transition focus:border-brand"
          >
            <option value="companion">Companion</option>
            <option value="trader">Trader buddy</option>
            <option value="scanner">Scanner buddy</option>
            <option value="analyst">Analyst buddy</option>
            <option value="guardian">Guardian buddy</option>
          </select>
        </label>
        <label className="grid gap-2 text-sm font-medium text-foreground">
          Automation level
          ...
          >
            ...
            <option value="executor">Executor gated</option>
          </select>
        </label>
        ...
        <label className="grid gap-2 text-sm font-medium text-foreground">
          ...
          <select
            value={stakingStatus}
            onChange={(event) => setStakingStatus(event.target.value)}
            className="h-11 rounded-2xl border border-border-base bg-background px-4 text-sm outline-none transition focus:border-brand"
          >
            <option value="none">No staking</option>
            <option value="planned">Stake planned</option>
            <option value="staked">Already staked</option>
          </select>
        </label>
        <label className="grid gap-2 text-sm font-medium text-foreground">
          $CLAWD stake intent
          <input
            value={stakeAmountLamports}
            onChange={(event) => setStakeAmountLamports(event.target.value)}
            className="h-11 rounded-2xl border border-border-base bg-background px-4 text-sm outline-none transition focus:border-brand"
            inputMode="numeric"
            placeholder="Optional lamports/base units"
          />
        </label>
        <label className="grid gap-2 text-sm font-medium text-foreground md:col-span-2">
          Buddy wallet / execution wallet
          <input
            value={buddyWallet}
            onChange={(event) => setBuddyWallet(event.target.value)}
            className="h-11 rounded-2xl border border-border-base bg-background px-4 text-sm outline-none transition focus:border-brand"
            placeholder="Optional Solana wallet for delegated missions"
          />
          <span className="text-xs font-normal text-muted-3">
            This records staking and execution intent only. Real transfers stay
            behind your wallet and permission gates.
          </span>
        </label>
      </div>
      ...
      {minted ? (
        <div className="mt-5 rounded-2xl border border-brand/30 bg-brand-tint/70 p-4 text-sm dark:bg-brand-tint-dark/60">
          ...
          <p className="mt-2 text-xs text-muted-2">
            Role: {minted.synergyRole} / Automation: {minted.automationLevel} /
            Staking: {minted.stakingStatus}
          </p>
