...
export async function POST(req: Request): Promise<Response> {
  ...
  const synergyRole = cleanChoice(
    body.synergyRole,
    ["companion", "trader", "scanner", "analyst", "guardian"] as const,
    "companion",
  );
  const automationLevel = cleanChoice(
    body.automationLevel,
    ["watcher", "operator", "executor"] as const,
    "watcher",
  );
  const stakingStatus = cleanChoice(
    body.stakingStatus,
    ["none", "planned", "staked"] as const,
    "none",
  );
  const stakeAmountLamports = cleanString(body.stakeAmountLamports, 32);
  const buddyWallet = cleanString(body.buddyWallet, 80);
  ...
  try {
    ...
    await db.insert(schema.blockchainBuddies).values({
      ...
      automationLevel,
      stakingStatus,
      stakeAmountLamports: stakeAmountLamports || null,
      buddyWallet: buddyWallet || ownerWallet || null,
