...
export const blockchainBuddies = pgTable(
  "blockchain_buddies",
  {
    ...
    synergyRole: text("synergy_role").notNull().default("companion"),
    automationLevel: text("automation_level").notNull().default("watcher"),
    stakingStatus: text("staking_status").notNull().default("none"),
    stakeAmountLamports: text("stake_amount_lamports"),
    buddyWallet: text("buddy_wallet"),
  ...
  (table) => ({
    ...
    stakingStatusIdx: index("blockchain_buddies_staking_status_idx").on(
      table.stakingStatus,
    ),
    automationLevelIdx: index("blockchain_buddies_automation_level_idx").on(
      table.automationLevel,
    ),
  }),
);
