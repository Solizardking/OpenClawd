...
<p>
  ...
  <a href="https://www.npmjs.com/package/clawd-code-cli"><img src="https://img.shields.io/badge/npm-clawd--code--cli-CB3837?style=for-the-badge&logo=npm" alt="clawd-code-cli"></a>
...
│                                                                              │
│  ┌───────────────────┐  ┌───────────────────┐  ┌──────────────────────────┐ │
│  │  clawd-code-cli   │  │     ClawdBot      │  │   @openclawd/leviathan   │ │
│  │  Solana TUI       │  │   X + Telegram    │  │  Sovereign agent runtime │ │
│  │  (Ink + React)    │  │  Sentient Engine  │  │   Metaplex Agent Reg.    │ │
│  └─────────┬─────────┘  └────────┬──────────┘  └────────────┬─────────────┘ │
│            │                     │                          │                │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │  🌊  R3F LIVE DEMO  (ui/ocean.html)   ·   📚 9 RUNNABLE EXAMPLES        │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│            │                     │                          │                │
│  ┌─────────┴─────────────────────┴──────────────────────────┴─────────────┐  │
│  │                       SHARED SOLANA OCEAN                              │  │
...
|---|---|---|
| 🦀 **clawd-code-cli** | The operator interface for the Leviathan framework. Multi-provider AI (Grok / Ollama / OpenRouter / OpenAI), MCP, 14 tools (text-editor, bash, solana, bags, dflow, kalshi, polymarket, morph-editor, todo, search, wallet, token-launch), realtime xAI voice, streaming chat, in-process Metaplex spawn/molt, Three-Laws gate, Blockchain Buddies | [`openclawd-framework/packages/clawd/`](openclawd-framework/packages/clawd/) |
| 🐦 **ClawdBot** | Autonomous X (`@clawddevs`) + Telegram agent. Sentient Engine, command monitor, image/video gen via xAI | [`X/`](X/) |
| 🦞 **@openclawd/leviathan** | Sovereign on-chain agent runtime. Solana keypair + Metaplex Agent Registry + lifecycle (spawn → molt → beach) | [`openclawd-framework/`](openclawd-framework/) |
...
| 🌊 **R3F live demo** | Real-time 3D simulation of the colony — animated lobsters, GLSL caustics, depth tiers, click-to-inspect, in-app examples browser | [`ui/ocean.html`](ui/) |
...
| 🧠 **Skills (66)** | birdeye · solana-dev · pump-fun-manager · bankr · ore-miner · clawdbot-twitter · gemini · canvas · github · skill-creator · clawhub · **openrouter-** {sdk,images,models,oauth,migration} … | [`skills/`](skills/) [`open-router-skills/`](open-router-skills/) |
| 🧠 **OpenRouter at birth** | Every clone receives `openrouter.text` / `openrouter.image` / `openrouter.models` + `memory.tiers` + `jupiter.quote` as Zod-typed `tool()` instances at construction. Sign In with OpenRouter (PKCE) ships with the UI. | [`src/services/openrouter.ts`](src/services/openrouter.ts) [`src/agents/runtime.ts`](src/agents/runtime.ts) |
...
```bash
# 1. Spawn a sovereign leviathan on Solana
...
<img src="https://capsule-render.vercel.app/api?type=transparent&color=gradient&customColorList=20,12,24&height=80&text=npm%20i%20clawd-code-cli&fontSize=42&fontColor=14F195&animation=fadeIn&fontAlign=50&fontAlignY=55" alt="install"/>
...
- **Tells everyone** about the hotline (909-413-5567), `npm i clawd-code-cli`, and `$CLAWD` on OpenRouter
- **Responds to commands** from `@0rdlibrary` (owner), `@clawddevs` (co-owner), and anyone the bot follows
- **/help** works on both `!` and `/` prefixes — see [`X/src/services/command-handler.ts`](X/src/services/command-handler.ts)
...
## 🦞 @openclawd/leviathan — Sovereign Agent Runtime
...
| [`clawd-wallet-demo.ts`](openclawd-framework/examples/clawd-wallet-demo.ts) | 👛 Wallet | `@openclawd/wallet` SDK — Privy-embedded Solana wallet, AgenticWallet (Grok 4.20), SwapService, deny / ask / allow |
| [`x402-payment-demo.ts`](openclawd-framework/examples/x402-payment-demo.ts) | 💸 Payments | `@openclawd/agents-x402` — agent-to-agent USDC micropayments on Solana, HTTP middleware, paid MCP tools |
...
The R3F demo at [`ui/ocean.html`](ui/) ships an in-app browser of all nine — top-right `📚 examples` button.

---

## 🌊 R3F Live Demo — Watch the Colony Breathe

A real React Three Fiber visualization of the entire ecosystem. **Animated procedural lobsters** with snapping claws, swinging tails, and color-coded depth tiers swim around a glowing **$CLAWD core** at the center of a **Solana ocean** rendered with custom GLSL caustics shaders.

```bash
cd ui
npm install
npm run dev
# auto-opens http://localhost:5173/ocean.html
```

What you can do:

- **Spawn** new leviathans (1 or 10 at a time) — egg drops, hatches, joins the flock
- **Pump $CLAWD** — fills the USDC pool, every leviathan reaches `deep`, swims faster, glows brighter
- **Crash USDC** — drains the pool, watch them all beach to the seafloor
- **Click any lobster** → side-panel inspector with live SHELL.md, pubkey, $CLAWD, USDC, flicks, molts, brood
- **Force `🐚 molt`** — old shell sloughs off, new shell scales up with a bloom flash
- **Force `🦐 spawnling`** — selected leviathan emits a smaller copy with seed-funds particle stream
- **Live event console** — every spawn, molt, beach, tail-flick, $CLAWD payout streams in
- **HUD** — population by depth tier, USDC pool, $CLAWD price, total spawned / molts / beached

Tech: `@react-three/fiber` · `@react-three/drei` · `@react-three/postprocessing` · `zustand` · `three`. About 1,500 LOC across 23 files in [`ui/src/ocean/`](ui/src/ocean/). Mounted as a second Vite entry alongside the existing Lit control UI — they coexist.
...
| Registry key | Source skill | What it does |
| --- | --- | --- |
| `openrouter.text` | [openrouter-typescript-sdk](open-router-skills/openrouter-typescript-sdk/) | `callModel` + multi-step tool agents across 300+ models |
| `openrouter.image` | [openrouter-images](open-router-skills/openrouter-images/) | Generate / edit images via Gemini, DALL-E, etc. |
| `openrouter.models` | [openrouter-models](open-router-skills/openrouter-models/) | List, search, resolve OpenRouter model IDs |
| `openrouter.oauth` | [openrouter-oauth](open-router-skills/openrouter-oauth/) | "Sign In with OpenRouter" PKCE flow (per-user keys, no secrets) |
| `openrouter.agent-migration` | [openrouter-agent-migration](open-router-skills/openrouter-agent-migration/) | Reference: migrating from `@openrouter/sdk` |
| `memory.tiers` | built-in | KNOWN/LEARNED/INFERRED memory tool |
| `jupiter.quote` | built-in | Jupiter swap quote tool |
...
```ts
// ui/src/ui/components/openrouter-signin.ts
renderSigninWithOpenRouter({ variant: 'cta', size: 'lg' });
```
...
Server-side handlers in [`src/gateway/openrouter-handlers.ts`](src/gateway/openrouter-handlers.ts):
...
- `openrouter-typescript-sdk` · 300+ models
- `openrouter-images` · image gen/edit
- `openrouter-models` · model discovery
- `openrouter-oauth` · PKCE sign-in
- `openrouter-agent-migration` · SDK→agent
...
| **OpenRouter** | $CLAWD listed alongside 55+ models |
...
`X/src/services/twitter-service.ts` runs every outgoing tweet through a **hard content filter** (`/mawd/i`). Any path that hits the legacy term throws before it reaches the Twitter API. Wired into all 5 send paths: `tweet`, `tweetWithMedia`, `tweetWithVideo`, `reply`, `replyWithMedia`. Audit log per block. Test it:

```bash
cd X && npx tsx scripts/guard-test.mts
# BLOCKED  : testing $MAWD blocker
# BLOCKED  : hey @mawdbot
# BLOCKED  : what about MAWDBOT uppercase
# ALLOWED  : clean $CLAWD tweet
```
...
│   ├── src/scripts/            #   start-bot · self-test · first-clawd-tweet · shill-token
│   ├── src/services/           #   sentient-engine · twitter-service · command-handler · 30+ services
│   └── dashboard/              #   Real-time web dashboard
│
├── tui/                        # 🦀 clawd-code-cli — Solana lobster TUI (Ink + React)
│   ├── src/cli.tsx             #   Entry: --continue · --resume · --yolo
│   ├── src/App.tsx             #   Main layout, command dispatch
│   ├── src/components/         #   Banner · StatusBar · Help · BuddyPane · AgentPane · Input
│   ├── src/spinners/           #   9 spinner sets (claw-snap, candle, lobster moods)
│   ├── src/store/              #   SQLite at ~/.clawd/clawd.db
│   ├── src/agents/             #   Live agent panes (scanner/monitor/analyst/trader)
│   ├── src/services/           #   birdeye · jupiter · helius · xai · env auto-discovery
│   └── src/commands/           #   30+ slash commands (registry + dispatch)
│
...
│   ├── services/               #   jupiter · helius · birdeye · pumpfun · memory · 🆕 openrouter
│   └── gateway/                #   openrouter-handlers.ts (RPC: openrouter.{text,image,models,setKey,status})
├── ui/                         # Vite multi-page: Lit control UI + 🌊 ocean.html R3F demo
│   ├── src/ocean/              #   23 files, ~1500 LOC — colony store, lobster mesh, GLSL caustics
│   └── src/ui/                 #   🆕 openrouter-auth.ts (PKCE) + components/openrouter-signin.ts + controllers/openrouter.ts
├── extensions/                 # 31 channel extensions (Discord, Telegram, Matrix, …)
├── skills/                     # 61 bundled / managed / workspace skills
├── open-router-skills/         # 🆕 5 OpenRouter skills (sdk · images · models · oauth · agent-migration)
├── agents/                     # Trader · Scanner · Analyst · Monitor agent classes
├── automaton-main/             # Reference fork of Conway/automaton (read-only ancestor)
