...
declare global {
  interface Window {
    __OPENCLAWD_ASSISTANT_NAME__?: string;
    __OPENCLAWD_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__OPENCLAWD_ASSISTANT_NAME__,
    avatar: window.__OPENCLAWD_ASSISTANT_AVATAR__,
  });
}
